...
|------------|-------------|------------|-------|-------------|-------------|
| \textbf{Classical SMC} | Discontinuous (smoothed) | $C^0$ | 6 | 18.5 μs | Boundary layer chattering reduction |
| \textbf{STA SMC} | 2nd-order sliding mode | $C^1$ | 2 + sliding | 24.2 μs | Finite-time convergence, continuous |
| \textbf{Adaptive SMC} | Adaptive gain | $C^0$ | 5 + $K(t)$ | 31.6 μs | Online parameter estimation |
| \textbf{Hybrid STA} | Mode-switching | $C^0$ | 8 + mode | 26.8 μs | Combines STA + Adaptive |
| \textbf{Swing-Up SMC} | Energy + SMC | $C^0$ | 3 + stabilizer | Variable | Global control (swing-up + stabilize) |
| \textbf{MPC} | Optimal control | $C^{\infty}$ | N/A (weights) | >>100 μs | Constrained optimization |
...
| \textbf{STA SMC} | $V = \|s\| + \frac{1}{2K\textit{2}z^2$ | Finite-time | Finite: $T < \frac{2\|s}0\|^{1/2}}{K\textit{1 - \sqrt{2K}2\bar{d}}}$ | $K\textit{1 > \frac{2\sqrt{2\bar{d}}}{\sqrt{\beta}}$, $K}2 > \frac{\bar{d}}{\beta}$ |
...
3. \textbf{Real-time feasibility:} Control law computation time <50 μs (validated post-optimization, Section 7.1)
...
3. \textbf{Compute time measurement:} Verify control law meets <50 μs real-time constraint
...
3. \textbf{Real-Time Feasibility:} Control law compute times 18.5-31.6 μs (Section 7.1) << 10 ms time step, leaving 99.7-99.8% CPU headroom.
...
\textbf{Physical Interpretation:} Determines real-time feasibility. For 10 kHz control loop (100 μs period), $t_{\text{compute}} < 50$ μs required (50% duty cycle budget).
...
|----------|--------|--------|-------|-------------------|---------|
| \textbf{Computational} | Compute time | $t_{\text{compute}}$ | μs | <50 (10 kHz loop) | 7.1 |
...
\textbf{Key Finding:} All controllers meet hard real-time constraints (<50 μs budget for 100 μs cycle). Classical SMC provides fastest computation (18.5 μs baseline), suitable for resource-constrained embedded systems. STA and Hybrid add 31-45% overhead but remain well within real-time feasibility.
...
- \textbf{Rationale:} Lowest compute time (18.5 μs), deterministic, simple implementation
...
- \textbf{Tradeoff:} +31% compute overhead vs Classical (but still <50 μs budget)
...
- \textbf{Recommendation:} Hybrid Adaptive STA SMC
- \textbf{Rationale:} Near-optimal on all dimensions (1.95s settling, 3.5% overshoot, 26.8 μs compute)
...
- \textbf{Classical SMC:} Fastest compute (18.5 μs, suitable for embedded systems)
...
- \textbf{Embedded systems:} Classical SMC (18.5 μs compute)
