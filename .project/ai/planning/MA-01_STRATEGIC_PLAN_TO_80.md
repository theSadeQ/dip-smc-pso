...
- Keep original files in `.project/archive/`
...
python .artifacts/qa_audits/MA-01_GUIDES_AUDIT_2025-11-10/collect_guides_inventory.py
python .artifacts/qa_audits/MA-01_GUIDES_AUDIT_2025-11-10/batch_guides_metrics.py | tail -20

# Detailed file breakdown
python -c "import json; data=json.load(open('.artifacts/qa_audits/MA-01_GUIDES_AUDIT_2025-11-10/guides_metrics.json'));
print('\n'.join([f'{m[\"file\"]}: {m[\"overall_score\"]:.1f}' for m in sorted(data['per_file_metrics'], key=lambda x: x['overall_score'])]))"
```

### Progress Tracking
Create `.artifacts/qa_audits/MA-01_GUIDES_AUDIT_2025-11-10/PROGRESS_LOG.md`:
