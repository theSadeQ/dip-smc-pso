...
- **Configuration**: `.project/ai/config/testing_standards.md` (85/95/100 targets)
...
**Script Location**: `.artifacts/test_audit/analyze_module_coverage.py`

**Commands**:
```bash
# Create analysis script
cat > .artifacts/test_audit/analyze_module_coverage.py << 'EOF'
...
if __name__ == '__main__':
    ...
    # Save to JSON
    with open('.artifacts/test_audit/module_coverage.json', 'w') as f:
...
python .artifacts/test_audit/analyze_module_coverage.py
```

**Output**: `.artifacts/test_audit/module_coverage.json` with per-file:
- Line coverage %
- Branch coverage %
...
**Script Location**: `.artifacts/test_audit/categorize_modules.py`

**Commands**:
```bash
cat > .artifacts/test_audit/categorize_modules.py << 'EOF'
import json
from pathlib import Path
...
if __name__ == '__main__':
    with open('.artifacts/test_audit/module_coverage.json') as f:
        modules = json.load(f)

    tiers = analyze_by_tier(modules)

    # Save categorized data
    with open('.artifacts/test_audit/coverage_by_tier.json', 'w') as f:
...
python .artifacts/test_audit/categorize_modules.py
```

**Output**: `.artifacts/test_audit/coverage_by_tier.json` with:
...
**Script Location**: `.artifacts/test_audit/build_integration_matrix.py`

**Commands**:
```bash
cat > .artifacts/test_audit/build_integration_matrix.py << 'EOF'
import ast
import json
from pathlib import Path
from collections import defaultdict
...
if __name__ == '__main__':
    ...
    # Save matrix
    with open('.artifacts/test_audit/integration_matrix.json', 'w') as f:
...
python .artifacts/test_audit/build_integration_matrix.py
```

**Output**: `.artifacts/test_audit/integration_matrix.json` with:
...
radon cc tests/ -a -s -j > .artifacts/test_audit/test_complexity.json

# Maintainability index
radon mi tests/ -j > .artifacts/test_audit/test_maintainability.json

# Analyze results
python .artifacts/test_audit/analyze_test_complexity.py
```
...
**Output**: `.artifacts/test_audit/complexity_analysis.json`
...
**Commands**:
```bash
interrogate tests/ -v -o .artifacts/test_audit/docstring_coverage.txt
python .artifacts/test_audit/parse_docstring_coverage.py
```

**Output**: `.artifacts/test_audit/docstring_coverage.json`
...
**Output**: `.artifacts/test_audit/assertion_quality.json`
...
**Commands**:
```bash
python -m pytest tests/ --json-report --json-report-file=.artifacts/test_audit/pytest_run1.json -q
python -m pytest tests/ --json-report --json-report-file=.artifacts/test_audit/pytest_run2.json -q
python -m pytest tests/ --json-report --json-report-file=.artifacts/test_audit/pytest_run3.json -q

python .artifacts/test_audit/detect_flaky_tests.py
```

**Output**: `.artifacts/test_audit/flaky_tests.json`
...
**Output**: `.artifacts/test_audit/slow_tests.json` (top 20 slowest)
...
**Output**: `.artifacts/test_audit/structural_issues_catalog.json`
...
python .artifacts/test_audit/analyze_duplicate_tests.py
```

**Decision Point**: Keep both (different purposes) or consolidate?

**Output**: `.artifacts/test_audit/duplicate_test_analysis.json`
...
**Output**: `.artifacts/test_audit/init_files_plan.json`
...
**Output**: `.artifacts/test_audit/gap_prioritization_matrix.json`
...
**Output**: `.artifacts/test_audit/missing_integration_tests.json`
...
**Output**: `.artifacts/test_audit/effort_estimates.json`
...
**Output**: `.artifacts/test_audit/CONSOLIDATED_FINDINGS.md`
...
**Output**: `.artifacts/test_audit/visualizations/` (PNG files)
...
**Output**: `.artifacts/test_audit/EXECUTIVE_SUMMARY.md`
...
- All outputs to `.artifacts/test_audit/` (isolated)
