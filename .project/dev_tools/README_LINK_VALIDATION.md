...
```bash
python .project/dev_tools/validate_docs_links.py --fast
```

### Full Mode (All Except External URLs, ~30 seconds)
```bash
python .project/dev_tools/validate_docs_links.py --full --verbose
```

### External Mode (Includes HTTP Validation, ~3 minutes)
```bash
python .project/dev_tools/validate_docs_links.py --external
```
...
```bash
python .project/dev_tools/validate_docs_links.py --full --fix --dry-run
```

### Apply Fixes
```bash
python .project/dev_tools/validate_docs_links.py --full --fix --apply
```
...
### List Orphaned Files
```bash
python .project/dev_tools/validate_docs_links.py --full --no-orphans
```

### Interactive Cleanup
```bash
python .project/dev_tools/validate_docs_links.py --full --cleanup-orphans
```
...
### JSON Export
```bash
python .project/dev_tools/validate_docs_links.py --full --output validation_report.json
```
...
```bash
#!/bin/bash
python .project/dev_tools/validate_docs_links.py --fast --staged-only
```

### CI/CD (GitHub Actions)
```yaml
- name: Validate Documentation Links
  run: python .project/dev_tools/validate_docs_links.py --full --fail-fast
```
...
sphinx-build -M html docs docs/_build && \
  python .project/dev_tools/validate_docs_links.py --full
```
...
```bash
python .project/dev_tools/validate_docs_links.py --fast --verbose | grep CRITICAL
```
...
```bash
python .project/dev_tools/validate_docs_links.py --external --output report.json
...
- [Documentation Build System](.project/ai/config/documentation_build_system.md)
