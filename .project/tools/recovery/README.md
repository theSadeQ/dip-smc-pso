# .project/tools/recovery - 30-Second Recovery System
...
**State File**: `.project/state/project_state.json`
...
python roadmap_tracker.py --roadmap .project/ai/planning/research/ROADMAP_EXISTING_PROJECT.md
```
...
3. `python .project/tools/checkpoints/analyze_checkpoints.py`
...
**See**: `.project/tools/multi_account/MULTI_ACCOUNT_RECOVERY_GUIDE.md`
...
python .project/tools/checkpoints/launch_checkpoint_task.py --resume LT-4
```
...
**Hook Location**: `.git/hooks/pre-commit` (via `.project/tools/automation/git-hooks/`)

### Recovery Script Dependencies
```
recover_project.sh
├─ .project/state/project_state.json (via project_state_manager.py)
├─ git log (recent commits)
├─ git status (current branch, changes)
└─ .project/checkpoints/*.json (agent checkpoints)
```
...
**2025-12-18**: Consolidated from `.project/dev_tools/` to `.project/tools/recovery/`
...
**State File Migration**: `.project/ai/config/project_state.json` -> `.project/state/project_state.json`
