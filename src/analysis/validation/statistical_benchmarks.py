...
class StatisticalBenchmarks:
    ...
    def run_benchmark(
        ...
        for i, config in enumerate(configurations):
            ...
            from .core import TrialConfiguration
            ...
            # Run trials
            batch = run_multiple_trials(benchmark_function, trial_config)
            results[config_name] = {
                'batch': batch,
            ...
            if batch.successful_results:
                # Get all available metrics from first successful result
                sample_result = batch.successful_results[0]
                for metric_name in sample_result.metrics.keys():
                    values = batch.get_metric_values(metric_name)
                    if values:
                        stats = batch.get_summary_statistics(metric_name)
                        ci = compute_basic_confidence_intervals(np.array(values))
                        results[config_name]['metrics'][metric_name] = {
                            'statistics': stats,
    ...
    def compare_configurations(
        ...
        for i in range(len(config_names)):
            for j in range(i + 1, len(config_names)):
                ...
                if (metric_name in results[name1]['metrics'] and
                    metric_name in results[name2]['metrics']):

                    batch1 = results[name1]['batch']
                    batch2 = results[name2]['batch']

                    from .core import compare_trial_batches
                    comparison = compare_trial_batches(batch1, batch2, metric_name)
