...
The double-inverted pendulum (DIP) system consists of a cart of mass $m\textit{0$ moving horizontally on a track, with two pendulum links (masses $m_1$, $m\textit{2$; lengths $L_1$, $L\textit{2$) attached sequentially to form a double-joint structure. The system is actuated by a horizontal force $u$ applied to the cart, with the control objective to stabilize both pendulums in the upright position ($\theta}1 = \theta_2 = 0$).
...
- $M\textit{{12} = M}{21} = (m\textit{1 r}1 + m\textit{2 L_1)\cos\theta\textit{1 + m}2 r\textit{2 \cos\theta}2$
- $M\textit{{13} = M}{31} = m\textit{2 r}2 \cos\theta_2$
- $M\textit{{22} = m}1 r\textit{1^2 + m}2 L\textit{1^2 + I}1$
- $M\textit{{23} = M}{32} = m\textit{2 L_1 r\textit{2 \cos(\theta}1 - \theta\textit{2) + I}2$
...
0 \\
-(m\textit{1 r}1 + m\textit{2 L_1)g\sin\theta_1 \\
...
Pendulum 1 inertia & $I_1$ & 0.0081 & kg$\cdot$m² \\
Pendulum 2 inertia & $I_2$ & 0.0034 & kg$\cdot$m² \\
Gravity & $g$ & 9.81 & m/s² \\
Cart friction & $b_0$ & 0.2 & N$\cdot$s/m \\
...
\begin{equation}
\label{eq:3_19}
E = \frac{1}{2}m\textit{0 \dot{x}^2 + \frac{1}{2}I}1 \dot{\theta}\textit{1^2 + \frac{1}{2}I}2 \dot{\theta}\textit{2^2 - m}1 g r\textit{1 \cos\theta}1 - m\textit{2 g (L_1 \cos\theta\textit{1 + r}2 \cos\theta_2)
\end{equation}
...
\textbf{Inertia weight} & $w = 0.7$ & Balanced exploration (early iterations) and exploitation (late iterations). Linearly decreasing $w$ (0.9 $\rightarrow$ 0.4) tested but showed no benefit \\
...
1. \textbf{Monte Carlo robustness test:} 100 runs with random initial conditions ($\pm$0.05 rad range)
2. \textbf{Model uncertainty sweep:} $\pm$10% and $\pm$20% parameter perturbations (masses, lengths, inertias)
...
\midrule
Nominal & 3 & $\pm$0.05 rad (~$\pm$3°) & 20% & Maintain baseline performance comparable to standard PSO \\
Moderate & 4 & $\pm$0.15 rad (~$\pm$9°) & 30% & Intermediate robustness for state estimation errors \\
Large & 8 & $\pm$0.30 rad (~$\pm$17°) & 50% & Real-world disturbances, startup transients, severe noise \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key Findings:}
1. \textbf{Substantial Overfitting Reduction:} 7.5x improvement in generalization (144.59x $\rightarrow$ 19.28x degradation)
2. \textbf{Absolute Performance:} 94% chattering reduction on realistic conditions (115k $\rightarrow$ 6.9k)
...
\textbf{Pydantic} & 2.0+ & Configuration validation (YAML $\rightarrow$ structured config) \\
...
1. \textbf{Nyquist Criterion:} Sample at >2$\times$ highest system frequency. DIP natural frequencies $\omega_n \approx 2\pi \times 5$ rad/s $\rightarrow$ minimum sample rate 10 Hz. Using 100 Hz provides 10$\times$ safety margin.
...
\textbf{Analysis:}
1. \textbf{Overfitting to Narrow Scenario:} PSO optimized parameters (ε_min=0.00250, $\alpha$=1.21) for $\pm$0.05 rad initial conditions
2. \textbf{Catastrophic Failure at Scale:} 6x larger perturbations ($\pm$0.3 rad, realistic disturbances) cause 50.4x chattering increase
...
1. \textbf{Substantial Generalization Improvement:} 7.5x reduction in overfitting (144.59x $\rightarrow$ 19.28x degradation)
2. \textbf{Absolute Performance:} 94% chattering reduction on realistic conditions (115k $\rightarrow$ 6.9k)
...
- Validation: Test on full $\pm$0.3 rad initial condition range
