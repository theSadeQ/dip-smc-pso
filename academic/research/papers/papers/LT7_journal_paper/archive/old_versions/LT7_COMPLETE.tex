...
- Studies with robustness analysis: 25percent (5/20; typically +/-10percent uncertainty only)
...
- Friction: Asymmetric viscous + Coulomb friction introducing model uncertainty (+/-15percent typical variation)

These terms stress-test SMC robustness to: (a) parametric uncertainty (+/-20percent in masses, lengths, inertias), (b) unmodeled dynamics (friction, flexibility), and (c) external disturbances (step, impulse, sinusoidal 0.5-5 Hz).
...
- Section 2: System model (6D state space, full nonlinear Euler-Lagrange dynamics with inertia matrix M(q), Coriolis C(q,q̇), gravity G(q)) and control objectives (5 formal requirements: asymptotic stability, settling time <=3s, overshoot <=10percent, control bounds |u|<=100N, real-time feasibility <100mus)
...
- Pendulum 1: Rigid link pivoting at cart position, free to rotate 360 deg (+/-π rad)
...
- Configuration-Dependent Inertia:
   ...
   - This creates state-dependent effective mass, making control gains tuned at theta=0 potentially ineffective at theta=+/-0.3 rad

- Trigonometric Nonlinearity in Gravity:
   - For small angles: sin(theta) ~= theta (linear approximation, error <2percent for |theta|<0.25 rad)
...
- 10percent overshoot: Prevents excessive pendulum swing that could violate +/-π workspace limits
...
- Robustness: Maintain performance under:
   - Model parameter uncertainty (+/-10-20percent in masses, lengths, inertias)
...
- Classical SMC (boundary layer)
- Super-Twisting Algorithm (STA-SMC)
- Adaptive SMC (parameter estimation)
- Hybrid Adaptive STA-SMC (mode-switching)
...
- Overflow Prevention: Clip intermediate calculations: u eq limited to +/-100N before adding switching term
...
- Chattering from small epsilon: Setting epsilon<0.01 causes high-frequency switching (>50 Hz). Stay above epsilon>=0.02 for dt=0.01s.
...
- Instability from violating Lyapunov conditions: Ensure K₁² >= 2K₂d̄ where d̄ is disturbance bound (~1.0 for DIP)
...
- Simplest: Classical SMC (6 scalar gains)
- Moderate: STA SMC (2 gains + Lyapunov conditions), Adaptive SMC (5 gains + adaptation law)
...
- Classical SMC (4.9 mus control law):
...
- Adaptive SMC (17.8 mus control law):
...
- Swing-Up SMC (8.5 mus control law):
   ...
   - SMC stabilizer: 2.5 mus (simplified Classical SMC)
...
- Worst-case timing (Adaptive SMC): 31.6 mus << 10 ms deadline (0.32percent utilization)
...
Choose K init = 1.2·d̄ (similar to Classical SMC switching gain)
...
Typical Results:
- Classical SMC: K=15.0, epsilon=0.02, k d=2.0 -> 18percent better than manual tuning
...
- Despite bound looseness, finite-time convergence confirmed: s->0 in <1s (much faster than Classical SMC's exponential ~2s)
...
Tolerance Analysis:
- +/-10percent parameter errors -> switching term compensates -> stability preserved
- +/-20percent errors -> steady-state error increases, chattering may worsen
- +/-30percent errors -> risk of instability (equivalent control degrades)

DIP Validation (Section 8.1):
- Mass errors (+/-10percent): Settling time +8percent, overshoot +12percent -> Stable ✓
- Length errors (+/-10percent): Settling time +5percent, overshoot +8percent -> Stable ✓
- Combined (+/-10percent): Settling time +15percent, overshoot +18percent -> Stable ✓
...
DIP Validation:
- Mass errors (+/-15percent): Settling time +6percent, overshoot +9percent -> Stable ✓
- Length errors (+/-15percent): Settling time +4percent, overshoot +7percent -> Stable ✓
...
DIP Validation (Section 8.1):
- Mass errors (+/-20percent): K(t) adapts +18percent, overshoot +5percent -> Stable ✓
- Predicted: +/-15percent tolerance from gain adaptation analysis
...
- Classical SMC Gain Condition: K > d̄
...
Key Insights:
- STA-SMC best balance: excellent disturbance rejection, good parameter tolerance, highest phase margin
- Adaptive SMC best for uncertain models: +/-20percent parameter tolerance via online adaptation
- Classical SMC largest gain margin but relies on accurate model (u eq)
...
- Classical SMC: 96.2percent of samples show $\dot{V} < 0$ (consistent with asymptotic stability)
- STA SMC: Fastest settling (1.82s), validating finite-time advantage
- Adaptive SMC: Bounded gains in 100percent of runs, confirming Theorem 4.3
...
For robust optimization, fitness evaluated across multiple physics realizations with parameter perturbations (+/-5percent in masses, lengths, inertias):
...
Classical SMC (6 parameters: $[k 1, k 2, \lambda 1, \lambda 2, K, k d]$):
...
Adaptive SMC (5 parameters: $[k 1, k 2, \lambda 1, \lambda 2, \gamma]$):
...
- Monte Carlo robustness test: 100 runs with random initial conditions (+/-0.05 rad range)
- Model uncertainty sweep: +/-10percent and +/-20percent parameter perturbations (masses, lengths, inertias)
...
- Inertia Weight (w) - High Sensitivity:
   ...
   - Impact: +/-20percent change in w -> +/-10percent change in final cost

- Cognitive Coefficient (c₁) - Low Sensitivity:
   - Optimal range: c₁ ∈ [1.5, 2.5]
   - Impact moderate: +/-50percent change in c₁ -> +/-2percent change in cost
...
- Social Coefficient (c₂) - Moderate Sensitivity:
   ...
   - Impact: +/-50percent change in c₂ -> +/-5percent change in cost
...
- Inertia w: +/-20percent -> +/-10percent cost change (High sensitivity)
- Social c₂: +/-50percent -> +/-5percent cost change (Moderate sensitivity)
- Cognitive c₁: +/-50percent -> +/-2percent cost change (Low sensitivity)
...
Maximum percentage deviation of pendulum angles beyond initial perturbation. Computed separately for $\theta 1, \theta 2$; reported as maximum across both angles. Target: OS < 10percent (standard second-order system spec).
...
- Moderate (Classical SMC): 12.4 J (+5percent)
- High (Adaptive SMC): 13.6 J (+15percent)
...
- Sort bootstrap distribution and extract 2.5th and 97.5th percentiles
...
Reporting Format: Mean +/- SD [95percent CI]
...
- Comparable performance (runtimes within +/-20percent on similar hardware)
...
- Classical SMC: 2.15s (+18percent vs STA)
- Adaptive SMC: 2.35s (+29percent vs STA)
...
- Classical: 5.8percent +/- 0.8percent -> 95percent CI [5.0, 6.6]percent
- STA: 2.3percent +/- 0.4percent -> 95percent CI [1.9, 2.7]percent
...
- Classical: 12.4 +/- 1.2J -> 95percent CI [11.2, 13.6]J
- STA: 11.8 +/- 0.9J -> 95percent CI [10.9, 12.7]J
...
Variability Sources:
- Stochastic disturbances: Random sensor noise, friction variations (+/-10percent settling time)
- Numerical integration: RK45 adaptive step size introduces +/-2percent variability
- PSO optimization: Different random seeds produce slightly different gains (+/-5percent performance)
...
- Classical SMC: 0.50x10 + 0.20x6 + 0.15x5 + 0.10x7 + 0.05x6 = 8.6/10 (BEST)
...
- Classical SMC: 0.10x10 + 0.20x6 + 0.35x5 + 0.30x7 + 0.05x6 = 6.5/10
...
- Adaptive SMC: 0.15x5 + 0.20x4 + 0.15x3 + 0.10x4 + 0.40x10 = 6.4/10 (BEST)
...
- Recommendation: Measure compute budget first (critical constraint)
  ...
  - If budget <30mus: Classical SMC (only feasible option)
...
- [ ] Controller tested with +/-10percent parameter variations
...
- If compute budget issues -> fallback to Classical SMC
...
- Lyapunov analysis uses worst-case assumptions:
  ...
  - Parameter uncertainty: +/-20percent assumed (actual system +/-5percent variation)
...
- Example: Adaptive SMC tolerates 16percent parameter error (predicted 20percent) -> still robust, just not quite as generous as theory suggested
...
- Theoretical +/-20percent conservative -> could refine Lyapunov analysis with tighter assumptions
- Adaptive SMC actual tolerance +/-16percent -> suggests adaptation law could be more aggressive
...
- Resonance Suppression: Classical SMC shows small resonance peak (+2 dB at 2 Hz), while STA nearly flat (+/-0.5 dB), validating finite-time convergence advantage.
...
- Classical SMC: Good (87percent attenuation, 0.83s recovery) - Acceptable for moderate disturbances
- Adaptive SMC: Moderate (78percent attenuation, 1.12s recovery) - Not recommended for fast-varying disturbances
...
Analysis:
- Overfitting to Narrow Scenario: PSO optimized parameters (epsilon min=0.00250, alpha=1.21) for +/-0.05 rad initial conditions
- Catastrophic Failure at Scale: 6x larger perturbations (+/-0.3 rad, realistic disturbances) cause 50.4x chattering increase
...
- $m 0 \in [0.84, 1.16]$ kg (+/-0.16 kg)
- $m 1 \in [0.42, 0.58]$ kg (+/-0.08 kg)
- $L 1 \in [0.252, 0.348]$ m (+/-0.048 m, +/-4.8 cm)
- $I 1 \in [0.0168, 0.0232]$ kg·m² (+/-0.0032 kg·m²)
...
- Friction coefficient varies: different surface (carpet vs tile) -> +/-16percent friction force
- All variations occur simultaneously (worst-case), controller still stable

Contrast with Lower Tolerance Controllers:
- Classical SMC (12percent tolerance): 16percent payload -> instability (settling time >10s, overshoot >50percent, eventually diverges)
...
- Scenario: Industrial robot arm, nominal payload 50 kg +/- 10percent (45-55 kg spec)
- Reality: Workers occasionally load 58 kg (16percent over nominal)
- Classical SMC: Fails at 56 kg (12percent tolerance -> 12percent over 50 kg = 56 kg limit)
...
- Recommendation: Classical SMC (12percent tolerance) marginal -> Use STA SMC (better attenuation) or Hybrid (better tolerance)
...
- Measure your application requirements (uncertainty percent, disturbance N, recovery time s)
...
- Symptoms:
  - System diverges (angles exceed +/-45 deg within 5-10s)
...
- Settling time: 5.2 +/- 2.8s (survivors only, +167percent)
- Overshoot: 24 +/- 11percent (+586percent)
- Chattering: 31 +/- 18 index (+474percent)
...
- Use robust PSO (Section 8.3) with +/-5percent variation around actual values
...
- Expected improvement: Tolerance 16percent -> 22percent (+6 percentage points)
...
Deployment Reality (+/-0.3 rad Initial Conditions, 6x Larger):
- Chattering index: 107.61 +/- 5.48 (50.4x worse)
...
- Weight: 20percent nominal +/-0.05 rad, 30percent moderate +/-0.15 rad, 50percent large +/-0.3 rad
...
- Completed Work (MT-8):
  ...
  - ✓ Adaptive Gain Scheduling: Validated state-magnitude-based scheduling across 4 controllers (320 simulations) + HIL (120 trials). Classical SMC: 28–41percent chattering reduction. Critical limitation: +354percent overshoot for step disturbances. See Section 8.2 for complete analysis.
...
Key Findings:
- Classical SMC: 96.2percent of state trajectory samples exhibit negative Lyapunov derivative (V̇ < 0), confirming asymptotic stability proof
...
- Adaptive SMC: Adaptive gains remain within prescribed bounds in 100percent of Monte Carlo runs, confirming bounded adaptation law
...
- 16percent parameter tolerance (Hybrid) = +/-16percent simultaneous variations in all plant parameters (Section 8.5.2)
...
- Best practice: Include 50percent of trials at large perturbations (+/-0.3 rad for DIP), 30percent moderate (+/-0.15 rad), 20percent nominal (+/-0.05 rad)
...
- High risk: Classical SMC generalization (90.2percent MT-7 failure) -> REQUIRE robust PSO validation
...
- 16percent faster settling than Classical SMC (1.82s vs 2.15s)
...
- Classical SMC: Poor generalization (90.2percent failure rate under large perturbations)
...
- 96.2percent of samples confirm Lyapunov stability (V̇ < 0 for Classical SMC)
...
- 11–41percent chattering reduction achieved for Classical SMC (320 simulation + 120 HIL trials)
...
- Embedded systems: Classical SMC (18.5 mus compute)
...
- Result (Classical SMC): 11-40.6percent chattering reduction depending on disturbance type (see Section 8.2)
