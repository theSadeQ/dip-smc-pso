...
\definecolor{sevlow}{RGB}{100, 100, 100}
% --- Severity / status macros (audit findings) ---
\newcommand{\sevC}{\textbf{\textcolor{blocked}{Critical}}}
\newcommand{\sevI}{\textbf{\textcolor{pending}{Important}}}
\newcommand{\sevL}{\textcolor{sevlow}{Minor}}
\newcommand{\statU}{\textbf{\textcolor{blocked}{Unresolved}}}
\newcommand{\statR}{\textbf{\textcolor{done}{Resolved}}}
\newcommand{\statA}{\textcolor{headblue}{Acknowledged}}
...
\paragraph{Gravity and friction model:}
\begin{align}
...
\end{equation}
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  \item Friction scope: all DOFs $i\in\{x,\theta_1,\theta_2\}$; dead zone
  $|\dot{q}_i|>10^{-6}$ avoids sign discontinuity at rest.
  \item Viscous coefficients $(0.2,\,0.005,\,0.004)$; Coulomb coefficients
  from config, Monte Carlo variation $\pm10\%$.
  \item Simulation model: full viscous + Coulomb form \eqref{eq:friction};
  PSO speedup uses reduced coupling (0.7--0.8) and no gyroscopic term.
  \item Linearization: numerical $(\mathbf{A},\mathbf{B})$ at
  $\vq^\star=\mathbf{0},\dot{\vq}^\star=\mathbf{0}$ with $\varepsilon=10^{-8}$;
  $\Delta t=0.01$~s, $u\in[-150,150]$~N.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Sliding dynamics, stability, and tuning:}
  \begin{equation}
    \sigma=0 \;\Rightarrow\; \dot{e}_i = -\frac{\lambda_i}{k_i}e_i,\quad
    \lambda_i>0,\;k_i>0,\; i\in\{1,2\}
    \label{eq:stability_cond}
  \end{equation}
  Time constants: $\tau_i = k_i/\lambda_i$ (direct tuning meaning).
  MT-8 robust Classical example: $(\lambda_1,\lambda_2,k_1,k_2)=(5.51,3.49,23.07,12.85)$
  gives $\tau_1\approx4.19$~s and $\tau_2\approx3.68$~s.
  ...
  \item \textbf{Relative degree, robustness, and practical limitation:}
  \begin{equation}
    \dot{\sigma} = \phi(\vx,t) + b(\vx)u + b(\vx)d(t), \qquad
    b(\vx)=\mathbf{L}\mathbf{M}^{-1}\mathbf{B},\quad
    \mathbf{L}=[0,\;k_1,\;k_2]
  \end{equation}
  Reachability: select $K>\bar{d}+\eta$ when $|d(t)|\le\bar{d}$.
  Limitation: surface excludes cart $x$; mitigated by saturation $u\in[-150,150]$~N
  and optional outer-loop cart reference shaping.
\end{itemize}
...
Energy $= 10^6$ and chattering $= 0$ in MT-5 are sentinel values: the PSO
instability penalty ($P = 10^6$) triggered on every run.  The simulation did
not abort; the controller was flagged as internally unstable.

The root cause is mode-switching instability: adaptive rates $\gamma_1{=}0.1$,
$\gamma_2{=}0.05$ combined with a 5.0/s rate limiter are too aggressive for
the rapid IC variations in the MT-5 distribution ($\pm0.05$~rad).  Gain
spikes breach actuator saturation before the simulation converges.  Resolution:
reduce rates to $\gamma \le 0.01$ and lower the rate limiter to $\le1.0$/s,
then re-run MT-5 with constrained robust PSO.

Contradicting evidence confirms the Hybrid \emph{can} stabilise: robust PSO
returned $J_\text{robust}{=}8.87$ (best of four controllers, MT-8, 15
constrained scenarios) and 16\% mismatch tolerance (best, LT-6, $\pm20\%$
parametric uncertainty, small nominal ICs).  Online gain adaptation compensates
unknown bounds when ICs are not extreme; failure occurs only in the full MC
distribution.

\medskip\noindent\textbf{Controller comparison:}
\begin{center}
\resizebox{\columnwidth}{!}{%
...
\noindent\textit{Success criteria and statistics:}
...
\noindent All 31 audit findings documented for advisor reference.
Color key: \sevC\ $\mid$ \sevI\ $\mid$ \sevL\ $\mid$
\statU\ $\mid$ \statR\ $\mid$ \statA.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Priority} & \textbf{Count} & \textbf{Open} & \textbf{Required action} \\
\midrule
\sevC  & 14 & 1 & Fix before submission \\
\sevI  & 15 & 0 & Expand before advisor review \\
\sevL  & 2  & 0 & Optional / out of scope \\
\midrule
...
% --- Submission blocker alert ---
\medskip
\begingroup
\setlength{\fboxsep}{6pt}
\noindent\fcolorbox{blocked}{blocked!6}{%
\begin{minipage}{\columnwidth-2\fboxsep-2\fboxrule-2pt}
\small
\textbf{\textcolor{blocked}{[SUBMISSION BLOCKER] --- Finding \#20 --- \S3.5 / Appendix~A}}

\smallskip
STA robust PSO gains: $K_1{=}2.02$, $K_2{=}6.67$.
Report states hard constraint $K_1{>}K_2$ (Moreno-Osorio finite-time
convergence condition).  Values are inverted: $K_1 < K_2$.

\smallskip
\textbf{Resolution:} Rerun constrained robust PSO with explicit
$K_1{>}K_2$ bound; \textit{or} re-derive whether $K_1{<}K_2$ is
valid for this DIP formulation and document the derivation.
\end{minipage}%
}
\endgroup

% --- Critical findings ---
\subsection*{\textcolor{blocked}{Critical Findings (14) --- Advisor will ask}}

\begin{center}
\small
\begin{tabularx}{\columnwidth}{@{}rp{0.85cm}Xp{1.85cm}@{}}
\toprule
\textbf{\#} & \textbf{Sect.} & \textbf{Issue} & \textbf{Status} \\
\midrule
1  & \S1.2   & Full $3{\times}3$ inertia matrix never shown (unique terms only) & \statA \\
2  & \S1.2   & $C(\mathbf{q},\dot{\mathbf{q}})$ referenced but never written out & \statA \\
3  & \S1.2   & Gravity vector missing explicit $G_1{=}0$ for cart DOF & \statA \\
5  & \S1.4   & Linearised $(\mathbf{A},\mathbf{B})$ used by PSO never shown & \statA \\
6  & \S1.2   & Friction: text says ``asymmetric Coulomb'' but equation is linear viscous only & \statA \\
9  & \S2     & No relative-degree check of $\sigma$ w.r.t.\ $u$ & \statA \\
10 & \S2     & Matching condition for disturbances never stated & \statA \\
11 & \S2.2.1 & $u_\text{eq}$ derivation from $\dot{\sigma}{=}0$ absent; only final form shown & \statA \\
20 & \S3.5   & STA gains $K_1{=}2.02 < K_2{=}6.67$ violate $K_1{>}K_2$ & \statU \\
21 & \S4     & Lyapunov constants ($\beta,\eta,c_1,c_2$) symbolic; not numerically instantiated & \statA \\
24 & \S5.1   & MC IC distributions, failure criteria, seed protocol not defined & \statA \\
26 & \S5.2   & Hybrid ``Failed'' in primary tables but has valid CI/FFT stats---contradictory & \statA \\
27 & \S5.4   & No normality checks; no multiple-comparison correction; Cohen's $d$ formula absent & \statA \\
29 & \S7.1   & ``Mismatch tolerance'' (8--16\%) undefined; ${\pm}20\%$ claim inconsistent & \statA \\
\bottomrule
\end{tabularx}
\end{center}

% --- Important findings ---
\subsection*{\textcolor{pending}{Important Findings (15) --- Strongly recommended}}
...
\textbf{\#} & \textbf{Sect.} & \textbf{Issue} & \textbf{Status} \\
\midrule
4  & \S1.2   & Nonlinear coupling terms not tied to derivation check & \statA \\
7  & \S2.1   & Sliding surface choice not justified vs integral/terminal alternatives & \statA \\
8  & \S2.1   & Stability constraints ($\lambda_i{>}0$, $k_i{>}0$) not stated as formal conditions & \statA \\
12 & \S2.2   & $\sat(\sigma/\varepsilon)$ piecewise definition never shown & \statA \\
13 & \S2.2.2 & Finite-time bound $T_f(K_1,K_2,x_0)$ not provided & \statA \\
14 & \S2.2.3 & $\delta_\text{leak}{=}0.01$, $d_z{=}0.05$ without sensitivity study & \statA \\
15 & \S5.1   & MT-5 inclusion/exclusion criteria for 4 of 7 controllers not stated & \statA \\
17 & \S3.1   & PSO convergence curve absent; stop-condition not defined & \statA \\
18 & \S3.2   & Normalisation thresholds $[10,100,1000,1]$ not scale-calibrated & \statA \\
19 & \S3.4   & 15-scenario split (3+4+8) without rationale or sensitivity analysis & \statA \\
22 & \S4.1   & 3.8\% of Lyapunov samples violate $\dot{V}{<}0$---cause unexplained & \statA \\
23 & \S4.5   & Swing-up Zeno prevention: no switching-time lower bound shown & \statA \\
28 & \S6     & Only 3 $\varepsilon$ values; 66.5\%$\to$3.7\% correction qualitative only & \statA \\
30 & General & Missing: lit benchmarking, sim-to-hardware gap, future work & \statA \\
31 & General & Chattering 3.088 (MT-5 RMS) vs 2.1 (FFT) without metric label & \statA \\
\bottomrule
\end{tabularx}
\end{center}

% --- Minor findings ---
\subsection*{\textcolor{sevlow}{Minor Findings (2) --- Out of Scope}}

\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  \item \textbf{\#16} (\S3.3): No PSO hyperparameter ablation over $N_p$, $w$---literature
  defaults used without sensitivity study.  Out of scope; acknowledged.
  \item \textbf{\#25} (\S5.1): Inclusion/exclusion criteria for the 4-of-7 controller
  subset in MT-5 not stated.  One sentence on Swing-Up/MPC exclusion suffices.
\end{itemize}

% --- Controller functional failures ---
\subsection*{Controller Functional Failures}

\begin{center}
\small
\begin{tabularx}{\columnwidth}{@{}p{1.65cm}p{1.75cm}Xp{1.5cm}@{}}
\toprule
\textbf{Controller} & \textbf{Failure} & \textbf{Root cause / evidence} & \textbf{Status} \\
\midrule
Hybrid Adaptive STA &
  100\% MC failure; Energy$=10^6$ on all 100 runs &
  $\gamma_1{=}0.1$, $\gamma_2{=}0.05$ + 5.0/s rate limiter too aggressive
  for ${\pm}0.05$~rad ICs.  Positive feedback loop: chattering$\to$large
  $|\theta|\to$conservative gains$\to$more chattering
  (Cohen's $d{=}1.33$, $p{<}0.001$).  Recovery: $\gamma{\le}0.01$,
  rate limiter ${\le}1.0$/s. &
  \textcolor{blocked}{\textbf{Not recommended}} \\
\addlinespace
Swing-Up &
  Diverges in MT-5 &
  Designed for large-angle recovery; phase transition fails for
  near-upright ICs (${\pm}0.05$~rad). &
  Excluded \\
\addlinespace
MPC &
  No results &
  \texttt{cvxpy} not integrated in batch runner. &
  Out of scope \\
\bottomrule
\end{tabularx}
\end{center}
...
% --- Resolved findings ---
\subsection*{\textcolor{done}{Resolved Findings}}

\begin{center}
\small
\begin{tabularx}{\columnwidth}{@{}rp{0.85cm}p{2.8cm}X@{}}
\toprule
\textbf{\#} & \textbf{Sect.} & \textbf{Issue} & \textbf{Resolution} \\
\midrule
R1 & \S5.3 & Two chattering metrics gave opposite rankings &
  FFT power $>$10~Hz adopted as official metric.  By this measure
  STA-SMC is 74\% better than Classical, not worse. \\
R2 & \S5.1 & MT-5 random seed undocumented &
  Seed 42 confirmed from \texttt{mt8\_repro\_seed42\_*.json} files. \\
\bottomrule
\end{tabularx}
\end{center}
