...
\noindent\textit{Table 1 omitted - see Markdown version for complete data}
...
\begin{itemize}
\item \textbf{vs. Single Pendulum}: DIP adds coupled nonlinear dynamics (inertia matrix coupling M_1_2, M_1_3, M_2_3; Coriolis forces $\propto$ $\theta$_1$\theta$_2) absent in single pendulum, requiring multi-variable sliding surfaces ($\sigma$ = $\lambda$_1$\theta$_1 + $\lambda$_2$\theta$_2 + k_1$\theta$_1 + k_2$\theta$_2) and coordinated gain tuning across 4-6 parameters.
...
\item \textbf{Coriolis matrix C(q,q)}: Velocity-dependent with centrifugal ($\propto$ $\theta$ᵢ²) and Coriolis ($\propto$ $\theta$ᵢ$\theta$ⱼ) terms
\item \textbf{Gravity vector G(q)}: Strongly nonlinear (sin$\theta$_1, sin$\theta$_2) with unstable equilibrium requiring active stabilization
...
\begin{itemize}
\item \textbf{Section 2}: System model (6D state space, full nonlinear Euler-Lagrange dynamics with inertia matrix M(q), Coriolis C(q,q), gravity G(q)) and control objectives (5 formal requirements: asymptotic stability, settling time $\leq$3s, overshoot $\leq$10\%, control bounds |u|$\leq$100N, real-time feasibility <100$\mu$s)
...
\begin{itemize}
\item $M_{11} = m_0 + m_1 + m_2$
\item $M_{12} = M_{21} = (m_1 r_1 + m_2 L_1)\cos\theta_1 + m_2 r_2 \cos\theta_2$
\item $M_{13} = M_{31} = m_2 r_2 \cos\theta_2$
\item $M_{22} = m_1 r_1^2 + m_2 L_1^2 + I_1$
\item $M_{23} = M_{32} = m_2 L_1 r_2 \cos(\theta_1 - \theta_2) + I_2$
\item $M_{33} = m_2 r_2^2 + I_2$
...
\end{enumerate}
   - Coriolis terms $\propto$ $\theta$1·$\theta$2 create \textbf{cross-coupling} between pendulum motions
   - During fast transients ($\theta$1 > 2 rad/s), Coriolis forces can exceed 20\% of gravity torque
...
\noindent\textit{Table 2 omitted - see Markdown version for complete data}
...
where:
\begin{itemize}
\item $\lambda_1, \lambda_2 > 0$ - position error weights
\item $k_1, k_2 > 0$ - velocity error weights
...
\noindent\textit{Table 3 omitted - see Markdown version for complete data}
...
where:
\begin{itemize}
\item $L = [0, k_1, k_2]$ - sliding surface gradient vector
...
\noindent\textit{Table 4 omitted - see Markdown version for complete data}
...
\item \textbf{Derivative Estimation:} Use filtered backward difference for $\theta$ (Butterworth 2nd-order, 20 Hz cutoff) to reduce noise amplification

\end{itemize}
\textbf{Computational Breakdown (18.5 $\mu$s total):}


\noindent\textit{Table 5 omitted - see Markdown version for complete data}
...
\item \textbf{Instability from large k\_d:} Derivative gain k\_d>5.0 can cause oscillations due to noise amplification in $\theta$ estimates.
...
\textbf{Signal Flow:}
\begin{enumerate}
\item Measure state x = [x, $\theta$_1, $\theta$_2, ẋ, $\theta$_1, $\theta$_2]ᵀ
\item Compute sliding surface $\sigma$ = $\lambda$_1$\theta$_1 + $\lambda$_2$\theta$_2 + k_1$\theta$_1 + k_2$\theta$_2
...
where:
\begin{itemize}
\item $K_1, K_2 > 0$ - STA algorithm gains (satisfy Lyapunov conditions)
...
\noindent\textit{Table 6 omitted - see Markdown version for complete data}
...
\item Compute sliding surface $\sigma$ = $\lambda$_1$\theta$_1 + $\lambda$_2$\theta$_2 + k_1$\theta$_1 + k_2$\theta$_2
\item Compute equivalent control u\_eq (model-based feedforward)
\item Compute proportional term: -K_1|$\sigma$|^(1/2)·sign($\sigma$)
\item Compute integral state: ż = -K_2·sign($\sigma$)
\item Sum STA terms: u\_STA = -K_1|$\sigma$|^(1/2)·sign($\sigma$) + z
...
\begin{enumerate}
\item \textbf{Instability from violating Lyapunov conditions:} Ensure K_1² $\geq$ 2K_2d̄ where d̄ is disturbance bound (~1.0 for DIP)
...
\item \textbf{Slow convergence from small K_1:} If K_1<8.0, reaching time increases beyond acceptable limits (>5s)
...
\noindent\textit{Table 7 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 8 omitted - see Markdown version for complete data}
...
\end{enumerate}
   - STA mode: u\_sw = -K_1|$\sigma$|^(1/2)·sign($\sigma$) + z
...
\item Energy pumping: Adds energy when $\cos(\theta_1) \dot{\theta}_1 > 0$ (constructive phase)
...
\noindent\textit{Table 9 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 10 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 11 omitted - see Markdown version for complete data}


\textbf{Convergence Guarantees:}


\noindent\textit{Table 12 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 13 omitted - see Markdown version for complete data}
...
\end{enumerate}
   ...
   - Swing-up term: 1.4 $\mu$s (k\_swing \textit{ cos($\theta$_1) } $\theta$_1)
...
\noindent\textit{Table 14 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 15 omitted - see Markdown version for complete data}
...
\begin{itemize}
\item \textbf{Mass ratios:} m_1/m_0, m_2/m_0 (affects inertia coupling)
\item \textbf{Length ratios:} L_1/L\_cart, L_2/L_1 (affects angular dynamics)
\item \textbf{Natural frequencies:} $\omega$_1 $\approx$ $\sqrt{}$(g/L_1), $\omega$_2 $\approx$ $\sqrt{}$(g/L_2) (sets response timescales)
...
\textbf{Step 1: Design Sliding Surface ($\lambda$_1, $\lambda$_2, k_1, k_2)}
...
\begin{enumerate}
\item Choose K_2 to dominate disturbances:

\end{enumerate}
   For d̄=1.0, $\varepsilon$=0.01 $\to$ K_2 > 200
   Practical choice: K_2 = 250 (25\% margin)

\begin{enumerate}
\item Choose K_1 to satisfy stability:

\end{enumerate}
   For K_2=250, d̄=1.0 $\to$ K_1 > $\sqrt{}$(500) $\approx$ 22.4
   Practical choice: K_1 = 30 (34\% margin)

\textbf{Step 3: Tune for Performance}

\begin{enumerate}
\item Start with Lyapunov-based values (K_1=30, K_2=250)
\item If convergence too slow $\to$ increase K_1 by 20\%
\item If chattering observed $\to$ decrease K_1 by 10\%, increase $\varepsilon$
\item Final gains typically: K_1 $\in$ [12, 20], K_2 $\in$ [8, 15] (after PSO optimization)
...
\noindent\textit{Table 16 omitted - see Markdown version for complete data}
...
\item Choose cost function: J = w_1·t\_settle + w_2·overshoot + w_3·chattering
...
\item STA SMC: K_1=12.0, K_2=8.0, $\varepsilon$=0.01 $\to$ 22\% better performance
...
\noindent\textit{Table 17 omitted - see Markdown version for complete data}
...
\item STA gains: K_1 = 12.0, K_2 = 8.0
...
\noindent\textit{Table 18 omitted - see Markdown version for complete data}
...
\begin{itemize}
\item \textbf{STA mode} ($|s| > \sigma_{\text{switch}}$): $\dot{V} \leq -c_1\|\xi\|^{3/2}$ (Theorem 4.2)
...
\noindent\textit{Table 19 omitted - see Markdown version for complete data}
...
\end{enumerate}
   \textbf{Configuration 1: Upright ($\theta$_1=0, $\theta$_2=0):}


   \textbf{Configuration 2: Large angle ($\theta$_1=0.2 rad, $\theta$_2=0.15 rad):}


   \textbf{Configuration 3: Near-singular ($\theta$_1=π/2, $\theta$_2=π/4):}
...
\noindent\textit{Table 20 omitted - see Markdown version for complete data}
...
\begin{itemize}
\item \textbf{Immediate:} Restrict operating range (limit |$\theta$_1|, |$\theta$_2| < 0.3 rad)
\item \textbf{Redesign sliding surface:} Adjust k_1, k_2 to maximize $\beta$
...
\noindent\textit{Table 21 omitted - see Markdown version for complete data}
...
\item \textbf{Conservative design:} Add 20-50\% safety margins to all bounds (d̄, $\varepsilon$_0, L)
...
\textbf{DIP Example:}
\begin{itemize}
\item Nominal: K_1 = 12.0, K_2 = 8.0
\item Minimums: K_1\_min = 3.2, K_2\_min = 1.28
\item Margins: GM\_K_1 = 12/3.2 = 3.75 (375\%), GM\_K_2 = 8/1.28 = 6.25 (625\%)
...
\noindent\textit{Table 22 omitted - see Markdown version for complete data}
...
\textbf{DIP Example:}
\begin{itemize}
\item Nominal: K_2 = 8.0, $\beta$ = 0.78 $\to$ d\_reject = 6.24 N
\item Actual: d̄ = 1.0 N
...
\noindent\textit{Table 23 omitted - see Markdown version for complete data}
...
Equivalent control $u_{eq}$ depends on accurate M, C, G. Parameter errors $\Delta$$\theta$ affect:
...
\noindent\textit{Table 24 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 25 omitted - see Markdown version for complete data}
...
\begin{enumerate}
\item \textbf{STA Lyapunov Conditions:} K_1 > 3.2, K_2 > 1.28
\end{enumerate}
   - PSO-optimized: K_1 = 12.0, K_2 = 8.0
   - \textbf{Conservatism factor: 3.75$\times$ (K_1), 6.25$\times$ (K_2)}
...
\noindent\textit{Table 26 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 27 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 28 omitted - see Markdown version for complete data}
...
\item Balanced cognitive-social influence ($c_1 \approx c_2$)
...
\noindent\textit{Table 29 omitted - see Markdown version for complete data}
...
\textit{Figure 5.1: PSO Convergence Curves}
...
\noindent\textit{Table 30 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 31 omitted - see Markdown version for complete data}
...
\textit{Figure 5.2: MT-6 PSO Convergence Comparison}
...
\textbf{Objective:} Optimize 6 gains [k_1, k_2, $\lambda$_1, $\lambda$_2, K, k\_d] for Classical SMC to minimize multi-objective cost (Eq. 5.2).
...
\noindent\textit{Table 32 omitted - see Markdown version for complete data}
...
\end{enumerate}
   - Increased $\lambda$_1, $\lambda$_2 (5.0$\to$10.5, 5.0$\to$8.3): Faster convergence rates
   - Increased K (0.5$\to$1.5): Stronger switching action (robustness)
   - Decreased k_1, k_2 (5.0$\to$5.2, 5.0$\to$3.1): Gentler sliding surface (less aggressive)
...
While Section 5.4 specifies standard PSO hyperparameters (w=0.7, c_1=c_2=2.0), this section quantifies the impact of hyperparameter variations on optimization performance.

\textbf{Table 5.1: PSO Hyperparameter Sensitivity Study (Classical SMC, 6 parameters)}


\noindent\textit{Table 33 omitted - see Markdown version for complete data}
...
\begin{enumerate}
\item \textbf{Cognitive Coefficient (c_1) - Low Sensitivity:}
\end{enumerate}
   - Optimal range: c_1 $\in$ [1.5, 2.5]
   - Impact moderate: $\pm$50\% change in c_1 $\to$ $\pm$2\% change in cost
   - Personal memory less critical than social learning for this problem

\begin{enumerate}
\item \textbf{Social Coefficient (c_2) - Moderate Sensitivity:}
\end{enumerate}
   - Optimal range: c_2 $\in$ [1.5, 2.5]
   - Higher c_2 (3.0) slightly beneficial (-2.1\% cost) but risks premature convergence
   - Impact: $\pm$50\% change in c_2 $\to$ $\pm$5\% change in cost

\begin{enumerate}
\item \textbf{Balance Critical:}
\end{enumerate}
   - Unbalanced c_1=1.0, c_2=3.0 causes swarm collapse (+23\% cost degradation)
   - Recommendation: maintain c_1 $\approx$ c_2 (equal cognitive-social influence)
...
\item \textbf{Social c_2:} $\pm$50\% $\to$ $\pm$5\% cost change (Moderate sensitivity)
\item \textbf{Cognitive c_1:} $\pm$50\% $\to$ $\pm$2\% cost change (Low sensitivity)

\end{enumerate}
\textbf{Practical Recommendation:}

\textbf{Stick with standard values (w=0.7, c_1=c_2=2.0)} for SMC gain tuning:
\begin{itemize}
...
\begin{itemize}
\item \textbf{Convergence too slow (>200 iterations):} Decrease w to 0.6, increase c_2 to 2.5
...
\noindent\textit{Table 34 omitted - see Markdown version for complete data}
...
\end{enumerate}
   - PSO works well with w=0.7, c_1=c_2=2.0 (no custom tuning needed)
...
\noindent\textit{Table 35 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 36 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 37 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 38 omitted - see Markdown version for complete data}
...
\item \textbf{Independent draws:} Each Monte Carlo trial uses independent random draw from $\mathcal{U}(-\theta_{\max}, +\theta_{\max})$
...
\textbf{Rationale:}
\begin{itemize}
\item Welch's t-test more robust than Student's t-test when variances unequal ($s_1^2 \neq s_2^2$)
\item Does not assume equal sample sizes ($n_1 \neq n_2$ permitted)
\item Approximately normal for $n \geq 30$ (Central Limit Theorem applies for our sample sizes 100-500)

\end{itemize}
\textbf{Decision Rule:}
\begin{itemize}
\item Reject null hypothesis $H_0: \mu_1 = \mu_2$ if $p < 0.05$
...
\item Cost function: $J = w_1 t_s + w_2 \text{OS} + w_3 E_{\text{control}}$ (Section 6.2)
...
\item \textbf{Robustness Score:} $R = p_{\text{conv}} \times (1 - \text{OS}_{\max}/180$^\circ$)$ (higher is better)
...
\noindent\textit{Table 39 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 40 omitted - see Markdown version for complete data}
...
\textbf{Linux:}
\begin{itemize}
\item Verify BLAS backend: `ldd $(python -c 'import numpy; print(numpy.__file__)') | grep blas`
...
\noindent\textit{Table 41 omitted - see Markdown version for complete data}
...
\item PSO hyperparameters (w, c_1, c_2) - Validated in Section 5.7
...
\item Solution: Set `OMP_NUM_THREADS=1` environment variable for strict reproducibility
...
\noindent\textit{Table 42 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 43 omitted - see Markdown version for complete data}
...
\textit{Figure 7.1: Computational Efficiency Comparison}
...
\noindent\textit{Table 44 omitted - see Markdown version for complete data}
...
\textit{Figure 7.2: Transient Response Performance}
...
\noindent\textit{Table 45 omitted - see Markdown version for complete data}
...
\end{itemize}
\textit{Figure 7.3: Chattering Characteristics}
...
\noindent\textit{Table 46 omitted - see Markdown version for complete data}
...
\textit{Figure 7.4: Control Energy Consumption}
...
\noindent\textit{Table 47 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 48 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 49 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 50 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 51 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 52 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 53 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 54 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 55 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 56 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 57 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 58 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 59 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 60 omitted - see Markdown version for complete data}
...
\end{itemize}
\textit{Figure 8.1: Model Uncertainty Tolerance}
...
\noindent\textit{Table 61 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 62 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 63 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 64 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 65 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 66 omitted - see Markdown version for complete data}
...
\textit{Figure 8.2: Disturbance Rejection Performance}
...
\noindent\textit{Table 67 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 68 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 69 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 70 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 71 omitted - see Markdown version for complete data}
...
\end{itemize}
\textit{Figure 8.3: PSO Generalization Analysis}
...
\textit{Figure 8.4a: MT-7 Chattering Distribution}
...
\textit{Figure 8.4b: MT-7 Per-Seed Variance}
...
\textit{Figure 8.4c: MT-7 Success Rate Distribution}
...
\textit{Figure 8.4d: MT-7 Worst-Case Scenario Analysis}
...
\noindent\textit{Table 72 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 73 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 74 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 75 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 76 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 77 omitted - see Markdown version for complete data}


\textbf{Controller Robustness Report Card (Section 8 Data):}


\noindent\textit{Table 78 omitted - see Markdown version for complete data}
...
\begin{itemize}
\item Control energy unbounded: $\int$_0ᵀ |u(t)|dt increases linearly with T
\item Expected: Bounded integral (system settles $\to$ u$\to$0, integral plateaus)
\item Manifested as: Integral grows $\propto$ T (linear, not saturating)
...
\item Energy: 47J after 10s (unbounded, linear growth $\propto$ time)
...
\noindent\textit{Table 79 omitted - see Markdown version for complete data}
...
\noindent\textit{Table 80 omitted - see Markdown version for complete data}


\textbf{Key Findings:}
\begin{enumerate}
\item \textbf{Classical SMC:} 96.2\% of state trajectory samples exhibit negative Lyapunov derivative (V < 0), confirming asymptotic stability proof
...
\noindent\textit{Table 81 omitted - see Markdown version for complete data}
...
\end{itemize}
\textbf{Controller Architecture Generalizes, Parameters Do Not:} The insight is that STA's integral action (z-term) provides superior disturbance rejection applies broadly, but K_1=15, K_2=8.3 are DIP-specific.
...
\begin{itemize}
\item 96.2\% of samples confirm Lyapunov stability (V < 0 for Classical SMC)
