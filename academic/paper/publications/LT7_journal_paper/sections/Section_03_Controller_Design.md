...
For stability, gains must satisfy (simplified form assuming β ≈ 1):
...
**Note:** The rigorous gain conditions (accounting for controllability scalar β = LM⁻¹B ≈ 0.78) are:
```math
K_1 > \frac{2\sqrt{2\bar{d}}}{\sqrt{\beta}}, \quad K_2 > \frac{\bar{d}}{\beta}
```

See Section 4.2 (Theorem 4.2) for complete derivation. For the DIP system with β = 0.78, this increases required gains by ~13% (K₁) and ~28% (K₂) compared to the simplified conditions above. The PSO-tuned gains (K₁ = 12.0, K₂ = 8.0) satisfy both conditions with safety margin.
...
```math
\dot{K}(t) = \begin{cases}
\gamma \beta |\sigma| - \beta_{\text{leak}} (K - K_{\text{init}}) & |\sigma| > \delta \\
-\beta_{\text{leak}} (K - K_{\text{init}}) & |\sigma| \leq \delta
\end{cases}
```

where β ≈ 0.78 is the controllability scalar (not the leak rate β_leak = 0.1). Note the variable naming: β_leak is renamed here to avoid confusion with the Lyapunov controllability parameter β.

**Practical Implementation:**

1. **Rigorous Approach:** Use β-scaled adaptation law above with β computed from system parameters (Section 4.6.2)
2. **Simplified Approach:** Retain original law but compensate with gain margin K_design ≥ 1.45 × d̄ (Section 4.3, Implementation Note)

The PSO-tuned gains in this paper (K_init = 10.0, γ = 5.0, Section 7) were empirically optimized across 400+ trials and implicitly compensate for β ≠ 1. See Section 4.3 for detailed mathematical analysis and recommendations.
...
**IMPORTANT: β Scaling Considerations**

This hybrid controller inherits β-related considerations from both constituent controllers:

1. **STA Mode (Section 3.3):** Rigorous gain conditions depend on controllability scalar β ≈ 0.78, increasing required K₁, K₂ by 13-28% (see Section 3.3 note)
2. **Adaptive Mode (Section 3.4):** Adaptation law assumes β = 1; for β ≠ 1, use corrected law or gain compensation (see Section 3.4 note and Section 4.3)

**Note on Variable Naming:** In the table above, "β = 0.1" refers to the leak rate (β_leak) in Adaptive mode, NOT the controllability scalar β = LM⁻¹B ≈ 0.78. See Section 3.4 for clarification of this naming collision.

The PSO-tuned hybrid gains (Section 7) satisfy all theoretical requirements with safety margin.
