...
\begin{itemize}
    \item \textbf{1.} \textbf{WHERE} to put code (directory structure)
    \item \textbf{2.} \textbf{HOW} to name things (files, classes, functions)
    \item \textbf{3.} \textbf{WHAT} patterns to use (factory, not if-else chains)
    \item \textbf{4.} \textbf{WHEN} to deviate (documented exceptions only)
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
    \item \textbf{1.} Exported in \_\_init\_\_.py? → Production (\texttt{src/})
    \item \textbf{2.} Imported by production code? → Production (\texttt{src/})
    \item \textbf{3.} Framework/infrastructure? → Production (\texttt{src/})
    \item \textbf{4.} Has \texttt{test\_*} name AND only for pytest? → Test (\texttt{tests/})
\end{itemize}
\end{warning}
...
\begin{tikzpicture}[node distance=2cm, auto]
    \node[block, fill=primary!30] (need) {\textbf{Need}\\Architecture\\decision};
    \node[decision, below=of need] (type) {\textbf{Type?}\\What to\\create};
    \node[process, below left=1.5cm and 2cm of type] (factory) {\textbf{Factory}\\Controller\\creation};
    \node[process, below=of type] (strategy) {\textbf{Strategy}\\Swappable\\dynamics};
    \node[process, below right=1.5cm and 2cm of type] (di) {\textbf{DI}\\Testable\\code};

    \draw[arrow] (need) -- (type);
    \draw[arrow] (type) -| node[left, near start] {Objects} (factory);
    \draw[arrow] (type) -- node[right] {Behavior} (strategy);
    \draw[arrow] (type) -| node[right, near start] {Testing} (di);
\end{tikzpicture}
\end{center}
