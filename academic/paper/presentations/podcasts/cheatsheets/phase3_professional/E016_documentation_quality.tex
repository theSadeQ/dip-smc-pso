...
\begin{itemize}
  \item \textbf{1.} Scan all existing docs: \texttt{python scripts/docs/detect\_ai\_patterns.py --dir docs/}
  \item \textbf{2.} Categorize files by severity: LOW (<5), MEDIUM (5-10), HIGH (>10)
  \item \textbf{3.} Prioritize: Fix HIGH files first, MEDIUM next, LOW last
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \texttt{scripts/docs/detect\_ai\_patterns.py} - Pattern detection (50+ patterns)
  \item \textbf{2.} \texttt{scripts/docs/validate\_links.py} - Cross-reference integrity
  \item \textbf{3.} \texttt{scripts/docs/check\_evidence.py} - Verify claims have citations/benchmarks
  \item \textbf{4.} \texttt{scripts/docs/measure\_density.py} - Calculate facts-per-paragraph ratio
\end{itemize}
...
\begin{tikzpicture}[node distance=2cm]
    \node[block, fill=primary!30] (scan) {\textbf{Scan}\\detect\_ai\\<5 patterns};
    \node[process, right=of scan] (verify) {\textbf{Verify}\\Evidence\\benchmarks};
    \node[process, right=of verify] (measure) {\textbf{Measure}\\Density\\5+ facts/para};
    \node[block, fill=secondary!30, right=of measure] (approve) {\textbf{Approve}\\Quality\\gates};

    \draw[arrow] (scan) -- (verify);
    \draw[arrow] (verify) -- (measure);
    \draw[arrow] (measure) -- (approve);
\end{tikzpicture}
\end{center}
\begin{itemize}
