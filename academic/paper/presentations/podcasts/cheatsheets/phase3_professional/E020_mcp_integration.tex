...
\begin{itemize}
  \item \textbf{1.} \textbf{filesystem}: File operations (read, write, list, search)
  \item \textbf{2.} \textbf{github}: Issues, PRs, checks, releases
  \item \textbf{3.} \textbf{sequential-thinking}: Planning, debugging, reasoning
  \item \textbf{4.} \textbf{puppeteer}: UI testing, browser automation
\end{itemize}

\subsection{Development Servers (4)}
\begin{itemize}
  \item \textbf{1.} \textbf{pytest-mcp}: Test debugging, coverage analysis
  \item \textbf{2.} \textbf{git-mcp}: Advanced Git operations (rebase, cherry-pick, blame)
  \item \textbf{3.} \textbf{mcp-analyzer}: Code quality, linting, static analysis
  \item \textbf{4.} \textbf{mcp-debugger}: API testing, HTTP requests
\end{itemize}

\subsection{Data Science Servers (3)}
\begin{itemize}
  \item \textbf{1.} \textbf{sqlite-mcp}: PSO optimization database queries
  \item \textbf{2.} \textbf{pandas-mcp}: Data analysis, CSV/JSON processing
  \item \textbf{3.} \textbf{numpy-mcp}: Numerical compute, matrix operations
\end{itemize}

\subsection{Audit Server (1)}
\begin{itemize}
  \item \textbf{1.} \textbf{lighthouse-mcp}: Accessibility audits (WCAG compliance)
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \textbf{Reproduce}: Trigger failing MCP operation
  \item \textbf{2.} \textbf{Check Logs}: Review .ai\_workspace/logs/mcp/*.log
  \item \textbf{3.} \textbf{Test Manually}: Run MCP server standalone (\texttt{npx @modelcontextprotocol/server-*})
  \item \textbf{4.} \textbf{Validate Config}: Run \texttt{validate\_servers.py}
  \item \textbf{5.} \textbf{Update Servers}: \texttt{npx -y @modelcontextprotocol/server-*} (latest versions)
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \textbf{Planning (sequential-thinking)}:
  \begin{lstlisting}[style=bashstyle]
  ...
  \item \textbf{2.} \textbf{Data Collection (pandas-mcp + sqlite-mcp)}:
  \begin{lstlisting}[style=bashstyle]
  ...
  \item \textbf{3.} \textbf{Figure Generation (pandas-mcp + numpy-mcp)}:
  \begin{lstlisting}[style=bashstyle]
  ...
  \item \textbf{4.} \textbf{LaTeX Creation (filesystem + sequential-thinking)}:
  \begin{lstlisting}[style=bashstyle]
  ...
  \item \textbf{5.} \textbf{Git + GitHub (git-mcp + github)}:
  \begin{lstlisting}[style=bashstyle]
...
\begin{itemize}
  \item \textbf{1.} \textbf{latex-mcp}: Compile LaTeX, generate PDFs, extract citations
  \item \textbf{2.} \textbf{sphinx-mcp}: Build docs, check cross-references, validate links
  \item \textbf{3.} \textbf{streamlit-mcp}: UI component testing, WCAG audits
  \item \textbf{4.} \textbf{benchmark-mcp}: Run pytest-benchmark, compare baselines
\end{itemize}
...
\begin{tikzpicture}[node distance=2cm, auto]
    \node[block, fill=primary!30] (config) {\textbf{Configure}\\12 servers\\.mcp.json};
    \node[process, right=of config] (validate) {\textbf{Validate}\\11/12\\operational};
    \node[decision, right=of validate] (ready) {\textbf{Ready?}\\Env vars\\set};
    \node[process, below=of ready] (chain) {\textbf{Chain}\\3-5 MCPs\\workflows};
    \node[process, left=of chain] (optimize) {\textbf{Optimize}\\Parallel\\execution};
    \node[block, fill=secondary!30, left=of optimize] (monitor) {\textbf{Monitor}\\Usage\\stats};

    \draw[arrow] (config) -- (validate);
    \draw[arrow] (validate) -- (ready);
    \draw[arrow] (ready) -- node[right] {Yes} (chain);
    \draw[arrow] (chain) -- (optimize);
    \draw[arrow] (optimize) -- (monitor);
\end{tikzpicture}
\end{center}
