...
\begin{itemize}
  \item \textbf{1.} \textbf{Unbounded Histories}: Controller stores ALL past states (10K+ arrays)
  \item \textbf{2.} \textbf{Circular References}: Controller $\leftrightarrow$ Dynamics back-refs
  \item \textbf{3.} \textbf{Event Listeners}: Callbacks hold references to large objects
  \item \textbf{4.} \textbf{Cache Bloat}: Memoization caches grow unbounded
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \textbf{tracemalloc}: Built-in Python memory profiler
  \item \textbf{2.} \textbf{memory\_profiler}: Line-by-line memory usage
  \item \textbf{3.} \textbf{objgraph}: Visualize object reference graphs
  \item \textbf{4.} \textbf{gc module}: Detect circular references
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \textbf{Reproduce}: Run simulation for 100K+ steps
  \item \textbf{2.} \textbf{Monitor}: Track memory usage via tracemalloc
  \item \textbf{3.} \textbf{Profile}: Identify leak source with memory\_profiler
  \item \textbf{4.} \textbf{Visualize}: Use objgraph to find circular refs
  \item \textbf{5.} \textbf{Fix}: Apply weakref or cleanup patterns
  \item \textbf{6.} \textbf{Validate}: Re-run with memory monitoring
\end{itemize}
...
\begin{itemize}
  \item \textbf{1.} \textbf{Phase 4.3}: Automated quality gate enforcement (CI/CD integration)
  \item \textbf{2.} \textbf{Phase 4.4}: Coverage measurement fixes (pytest-cov issues)
  \item \textbf{3.} \textbf{Phase 4.5}: Multi-threaded stress testing (100+ concurrent simulations)
\end{itemize}
...
\begin{tikzpicture}[node distance=1.8cm, auto]
    \node[block, fill=primary!30] (weakref) {\textbf{Weakref}\\Back-refs\\safe};
    \node[process, below=of weakref] (cleanup) {\textbf{Cleanup}\\Methods\\explicit};
    \node[process, below=of cleanup] (history) {\textbf{History}\\Limit\\1000 max};
    \node[decision, below=of history] (thread) {\textbf{Thread?}\\11/11\\tests};
    \node[process, below left=1.5cm and 2cm of thread] (memory) {\textbf{Memory}\\Monitor\\tracemalloc};
    \node[decision, below=of thread] (gates) {\textbf{Gates?}\\7/8\\passing};
    \node[block, fill=secondary!30, below=of gates] (ready) {\textbf{Ready}\\Production\\deploy};
    \node[block, fill=warning!30, right=2.5cm of gates] (fail) {\textbf{Block}\\Fix\\issues};

    \draw[arrow] (weakref) -- (cleanup);
    \draw[arrow] (cleanup) -- (history);
    \draw[arrow] (history) -- (thread);
    \draw[arrow] (thread) -| node[left, near start] {Pass} (memory);
    \draw[arrow] (memory) -- (gates);
    \draw[arrow] (thread) -- node[right] {Pass} (gates);
    \draw[arrow] (gates) -- node[right] {8/8} (ready);
    \draw[arrow] (gates) -- node[above] {<8/8} (fail);
\end{tikzpicture}
\end{center}
