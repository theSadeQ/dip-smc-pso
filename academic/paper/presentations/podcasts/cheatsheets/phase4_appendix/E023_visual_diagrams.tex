...
\begin{enumerate}
  \item \textbf{Config Entry}: User edits \texttt{config.yaml}
  \item \textbf{Validation}: Pydantic validates all parameters (catch errors pre-runtime)
  \item \textbf{Splitting}: Config splits into controller settings + dynamics parameters
  \item \textbf{Factory}: Controller factory instantiates requested controller type
  \item \textbf{Simulation Loop}:
    ...
    \end{itemize}
  \item \textbf{Results}: Visualization, analysis, export
\end{enumerate}

\subsection{PSO Optimization Flow}
\begin{enumerate}
  \item \textbf{Swarm Init}: 50 particles, random initial positions in parameter space
  \item \textbf{Iteration Loop (100 iterations)}:
    \begin{itemize}
    ...
    \end{itemize}
  \item \textbf{Convergence}: Return global best after 100 iterations
  \item \textbf{Output}: Save optimized gains to JSON file
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Start Broad}: Project structure diagram (understand 4 layers)
  \item \textbf{Follow Data}: Control flow diagram (trace execution path)
  \item \textbf{Zoom In}: Controller factory (understand instantiation)
  \item \textbf{Cross-Reference}: Match diagram nodes to code files
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Layered Design}: Core (L1) -> Controllers/Optimization (L2) -> Infrastructure (L3) -> Specialized (L4)
  \item \textbf{Intentional Patterns}: Compatibility layers, re-export chains, model variants (documented in CLAUDE.md Section 25)
  \item \textbf{Interface Abstraction}: Swap implementations without changing dependents
  \item \textbf{Peer File Structure}: Every src/ file has tests/ peer
\end{enumerate}
