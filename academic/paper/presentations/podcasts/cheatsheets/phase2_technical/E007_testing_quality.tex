...
\begin{warning}
\begin{enumerate}
    \item \textbf{What to test?} One input or all possible inputs? (All is impossible $\Rightarrow$ sample strategically)
    \item \textbf{When to stop?} Can always write more tests - what's "good enough"?
    \item \textbf{What to validate?} Implementation details (break on refactor) vs behavior (stable contract)?
\end{enumerate}
...
\begin{tabular}{p{4cm}|p{2.5cm}|p{2cm}|p{2.5cm}}
\tableheadcolor
...
\begin{enumerate}
    \item \textbf{Overall project:} 85\% minimum (aggregate across all files)
    \item \textbf{Critical modules:} 95\% minimum (controllers, dynamics, PSO)
    \item \textbf{Safety-critical:} 100\% required (saturation, validation, monitoring)
\end{enumerate}
\end{keypoint}
...
\begin{enumerate}
    \item Add 100 lines to critical module
    \item Must add tests to maintain 95\%+ coverage
    \item If coverage drops below 95\% $\Rightarrow$ \textbf{Build FAILS}
    \item Cannot merge until tests added
\end{enumerate}
...
\begin{enumerate}
    \item Zero critical bugs (all P0 issues resolved)
    \item 100\% test pass rate (4,563/4,563 tests)
    \item Memory validated (10,000 sims, zero growth)
    \item Thread-safe (11/11 parallel PSO tests pass)
    \item Zero high-priority issues
\end{enumerate}

\textbf{Gates We FAIL (Production Blockers):}
\begin{enumerate}
    \setcounter{enumi}{5}
    \item Coverage measurement broken (reports 2.86\%, real is 89\%)
    \item No production CI/CD (dev pipelines only)
    \item No hardware validation (never run on actual robot/PLC)
\end{enumerate}
\end{tcolorbox}
