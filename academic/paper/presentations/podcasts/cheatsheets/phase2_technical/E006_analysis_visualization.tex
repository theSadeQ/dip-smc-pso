...
\begin{warning}
\begin{itemize}
    \item \textbf{1.} \textbf{Choosing representation}: Time series vs phase portraits vs frequency domain vs distributions
    \item \textbf{2.} \textbf{Avoiding misleading scales}: Y-axis at zero? Log vs linear?
    \item \textbf{3.} \textbf{Managing complexity}: 6 states + control + metrics + disturbances = overwhelming
\end{itemize}
...
\begin{itemize}
    \item \textbf{1.} \textbf{Settling Time ($t_{\text{settle}}$)}: How long until equilibrium?

    Min time where $|\theta_1| < \epsilon$ AND $|\theta_2| < \epsilon$ for all future $t$

    \item \textbf{2.} \textbf{Overshoot}: How far past equilibrium before settling?

    $\text{Overshoot} = \max(\theta) - \theta_{\text{desired}}$

    \item \textbf{3.} \textbf{Energy Consumption}: Total control effort

    $E = \int_0^T u^2 \, dt$ (lower is better)

    \item \textbf{4.} \textbf{Chattering Frequency}: High-frequency oscillations (via FFT)
...
\textbf{Method:}
\begin{itemize}
    \item \textbf{1.} Take your 100 results (names in a hat)
    \item \textbf{2.} Draw 100 names randomly WITH REPLACEMENT (same name can appear multiple times)
    \item \textbf{3.} Create "virtual" set of 100 results
    \item \textbf{4.} Repeat 10,000 times
\end{itemize}
...
\begin{itemize}
    \item \textbf{1.} Axes labeled with units
    \item \textbf{2.} Legends clear and complete
    \item \textbf{3.} Error bars/confidence intervals shown
    \item \textbf{4.} Statistical significance indicated
\end{itemize}

\textbf{Visual Design:}
\begin{itemize}
    \item \textbf{1.} Colorblind-friendly palette
    \item \textbf{2.} High-resolution (300 DPI for print)
    \item \textbf{3.} Consistent fonts/sizes
    \item \textbf{4.} No clutter or overlapping text
\end{itemize}

\textbf{Storytelling:}
\begin{itemize}
    \item \textbf{1.} Caption explains key insight
    \item \textbf{2.} Figure stands alone (no external context needed)
    \item \textbf{3.} Comparison is fair (same scales, conditions)
    \item \textbf{4.} Conclusion is obvious from visual inspection
\end{itemize}
\end{summary}
