...
\textbf{What It Does:}
\begin{itemize}
    \item \textbf{1.} Reads \texttt{conf.py}
    \item \textbf{2.} Parses markdown \& reST files
    \item \textbf{3.} Generates API from docstrings (autodoc)
    \item \textbf{4.} Builds cross-reference links
    \item \textbf{5.} Outputs searchable HTML
\end{itemize}
...
\begin{itemize}
    \item \textbf{1.} What does this function do?
    \item \textbf{2.} What inputs does it need? (Parameters section)
    \item \textbf{3.} What does it return? (Returns section)
    \item \textbf{4.} What errors can it raise? (Raises section)
\end{itemize}
...
\textbf{Audit Process:}
\begin{itemize}
    \item \textbf{1.} Script lists files in \texttt{src/} without corresponding docs in \texttt{sphinx\_docs/}
    \item \textbf{2.} List goes on backlog
    \item \textbf{3.} Prioritize based on usage:
    \begin{itemize}
...
\begin{tikzpicture}[node distance=2.5cm]
    \node[cloud] (source) {.md/.rst\\Source\\706 files};
    \node[block, fill=primary!30, right=of source] (sphinx) {\textbf{Sphinx}\\Builder\\30s};
    \node[process, above right=1cm and 1.5cm of sphinx] (html) {\textbf{HTML}\\Docs\\GitHub Pages};
    \node[process, below right=1cm and 1.5cm of sphinx] (pdf) {\textbf{PDF}\\Manual\\LaTeX};

    \draw[arrow] (source) -- (sphinx);
