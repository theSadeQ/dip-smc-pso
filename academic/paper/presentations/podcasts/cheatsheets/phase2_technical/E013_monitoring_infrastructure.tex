...
\textbf{Algorithm:}
\begin{enumerate}
    \item Maintain circular buffer of last k outcomes (PASS/MISS)
    \item On each cycle: Add new outcome, drop oldest
    \item Count misses in current window
    \item If misses > m → Constraint violated!
\end{enumerate}
...
\begin{enumerate}
    \item Thread safety: 11/11 tests passing
    \item Memory: Zero growth over 10,000 simulations
    \item Timing: < 5\% deadline miss rate
    \item Weakly-hard: Zero (2, 10) violations in 24-hour test
\end{enumerate}
