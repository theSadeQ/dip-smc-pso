...
\begin{enumerate}
    ...
    \item First pendulum angle ($\theta_1$)
    \item Second pendulum angle ($\theta_2$)
    \item Cart velocity ($\dot{x}$)
    \item First pendulum velocity ($\dot{\theta}_1$)
    \item Second pendulum velocity ($\dot{\theta}_2$)
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Mass matrix}: How heavy everything is, how mass is distributed
    \item \textbf{Coriolis \& centrifugal}: How rotation causes forces
    \item \textbf{Gravity terms}: How gravity pulls pendulums down
    \item \textbf{Control input}: The push force we apply
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Nudge it}: Marble rolls to the side
    \item \textbf{Gravity pulls down}: Rolls toward bottom
    \item \textbf{Overshoots}: Has momentum, goes up other side
    \item \textbf{Friction slows}: Each oscillation loses energy
    \item \textbf{Settles}: Eventually MUST stop at bottom
\end{enumerate}
\end{keypoint}
...
\begin{enumerate}
    \item Small deviations stay small (stable)
    \item Deviations actually go to zero over time (asymptotic)
\end{enumerate}
...
\textbf{Strategy:}
\begin{enumerate}
    \item \textbf{Reaching Phase}: Get TO the guardrail path (don't care about efficiency, just reach it fast)
    \item \textbf{Sliding Mode}: Follow the path - guardrail geometry guides you safely to bottom
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Dramatically reduced chattering}: Control signal is continuous (no buzzing!)
    \item \textbf{Finite-time convergence}: Still reaches surface in finite time
    \item \textbf{Robust to smooth disturbances}: Handles Lipschitz disturbances
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Integral term}: Gradually builds force from accumulated error
    \item \textbf{Proportional term with $\sqrt{|s|}$}: Square root provides automatic gain scheduling
\end{enumerate}
...
\begin{summary}
\begin{enumerate}
    \item Compute proportional term: $K_2 \cdot \text{sign}(s) \cdot \sqrt{|s|}$
    \item Update integral term: Accumulate signed error over time, scaled by $K_1$
    \item Sum them: $u = u_{\text{integral}} + u_{\text{proportional}}$
\end{enumerate}
...
\begin{summary}
\begin{enumerate}
    \item \textbf{Dead Zone}: If $|s| < $ threshold, don't adapt (ignore noise)
    \item \textbf{Gain Leak}: Slowly decrease gains in dead zone (prevents ratcheting)
    \item \textbf{Bounded Adaptation}: Enforce min/max limits (don't go to zero or infinity)
\end{enumerate}
