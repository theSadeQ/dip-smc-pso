...
\begin{itemize}
    \item \textbf{1.} $x$ - Cart position (left/right)
    \item \textbf{2.} $\theta_1$ - First pendulum angle (from vertical)
    \item \textbf{3.} $\theta_2$ - Second pendulum angle (from vertical)
\end{itemize}
...
\begin{itemize}
    \item \textbf{1.} Free-body diagrams for cart + both pendulums
    \item \textbf{2.} All internal forces (pin forces, constraint forces)
    \item \textbf{3.} Coupled force balance equations
    \item \textbf{4.} Solving for dozen+ variables you don't care about!
\end{itemize}
...
\textbf{Recipe:}
\begin{itemize}
    \item \textbf{1.} Calculate total kinetic energy (motion)
    \item \textbf{2.} Calculate total potential energy (gravity)
    \item \textbf{3.} Form Lagrangian: $\mathcal{L} = T - V$
    \item \textbf{4.} Apply Euler-Lagrange equations (systematic calculus)
    \item \textbf{5.} Get equations of motion - NO internal forces!
\end{itemize}
\end{keypoint}
...
\textbf{How We Handle:}
\begin{itemize}
    \item \textbf{1.} Check $\kappa$ before inverting $\mathbf{M}$
    \item \textbf{2.} If $\kappa > 10^8$: Switch to pseudoinverse with regularization
    \item \textbf{3.} Prevents division-by-near-zero crashes
\end{itemize}
...
\begin{tikzpicture}[node distance=2.5cm]
    ...
    \node[block, fill=warning!30, right=of input] (simplified) {\textbf{Sprinter}\\Fast\\450 sims/s};
    \node[process, right=of simplified] (full) {\textbf{Simulator}\\Accurate\\8 sims/s};
    \node[block, fill=secondary!30, right=of full] (lowrank) {\textbf{Efficient Pro}\\Balanced\\95 sims/s};

    \draw[arrow] (input) -- node[above] {Linear} (simplified);
    \draw[arrow] (input) -- node[above] {Full NL} (full);
    \draw[arrow] (input) -- node[above] {Low-Rank} (lowrank);
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item \textbf{1.} Run Simulator 1000x times
    \item \textbf{2.} Collect data snapshots
    \item \textbf{3.} SVD: Find important patterns
    \item \textbf{4.} Keep signal, discard noise
\end{itemize}
...
\begin{itemize}
    \item \textbf{1.} Unpack state: Extract $q = [x, \theta_1, \theta_2]$ and $\dot{q} = [\dot{x}, \dot{\theta}_1, \dot{\theta}_2]$
    \item \textbf{2.} Build mass matrix: $\mathbf{M}(q)$ based on current angles (trig functions)
    \item \textbf{3.} Calculate Coriolis: $\mathbf{C}(q, \dot{q})$ with velocity coupling
    \item \textbf{4.} Calculate gravity: $\mathbf{G}(q)$ angle-dependent torques
    \item \textbf{5.} Apply control: Force $u$ enters through $\mathbf{B}$
    \item \textbf{6.} Solve for $\ddot{q}$: Invert $\mathbf{M}$ (check $\kappa$ first!)
    \item \textbf{7.} Return derivative: Pack $[\dot{q}, \ddot{q}]$ for integrator
\end{itemize}
