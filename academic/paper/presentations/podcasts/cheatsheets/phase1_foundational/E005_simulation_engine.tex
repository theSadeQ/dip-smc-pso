...
\begin{enumerate}
    \item \textbf{Compute control}: $u = \text{controller.compute}(\text{state}, \text{last\_u}, \text{history})$
    \item \textbf{Actuator saturation}: $u = \text{clip}(u, -50, +50)$ (real motors have limits!)
    \item \textbf{Compute dynamics}: $\dot{x} = \text{plant.compute\_dynamics}(\text{state}, u)$
    \item \textbf{Integration}: $\text{state}_{\text{new}} = \text{integrator.step}(\text{state}, \dot{x}, dt)$
    \item \textbf{Instability check}: If $|\theta_1| > 45°$ or $|\theta_2| > 45°$ $\Rightarrow$ early exit
    \item \textbf{Logging}: Store $t$, state, $u$ in pre-allocated arrays
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}
    \item Analyze Python bytecode
    \item Infer types from input arguments
    \item Generate LLVM intermediate representation
    \item Compile to machine code (x86/ARM)
    \item Cache compiled function
\end{enumerate}
Time: ~500ms (one-time cost)

\textbf{Subsequent calls (blazing fast):}
\begin{enumerate}
    \item Load cached machine code
    \item Execute directly on CPU (no Python interpreter!)
\end{enumerate}
