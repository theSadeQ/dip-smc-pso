...
\fourkeypatterns
{\textbf{Abstract Base Class (ABC)}: Enforces contract via Python's \texttt{@abstractmethod}}
{\textbf{Factory Pattern}: Registry-based instantiation decouples creation from usage}
{\textbf{Weak Reference Pattern}: Prevents memory leaks from circular references}
{\textbf{Strategy Pattern}: Controllers are interchangeable strategies for same problem}
