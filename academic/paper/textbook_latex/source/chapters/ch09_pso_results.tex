...
Chapter 8 demonstrated that default controller gains from \texttt{config.yaml} produce suboptimal performance: classical SMC\index{sliding mode control|see{SMC}}\index{sliding mode control!classical}\index{sliding mode control|see{SMC}}\index{sliding mode control!classical} consumes 20$\times$ more energy than necessary, hybrid controller fails to converge,and adaptive SMC\index{sliding mode control!adaptive}\index{sliding mode control!adaptive} exhibits 8.2\% overshoot. Manual gain tuning is impractical for multi-input systems (classical SMC has 6 gains, adaptive SMC has 5) with nonlinear coupling between parameters.
...
\begin{enumerate}
    ...
    \item \textbf{RQ2 (Effectiveness):} What performance improvements does PSO achieve (settling time\index{performance metrics!settling time}\index{performance metrics!settling time}, overshoot, energy, chattering\index{chattering}\index{chattering})?
...
\begin{itemize}
    ...
    \item \textbf{Heterogeneous bounds:} Switching gains ($k_4$-$k_6$) typically smaller than sliding surface\index{sliding surface}\index{sliding surface} gains ($k_1$-$k_3$) based on theoretical analysis.
\end{itemize}
...
Total simulation\index{simulation} time & -- & 4.17 hours (1500 evals $\times$ 10 s) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Inertia Weight Scheduling:} Linearly decreases from 0.9 to 0.4 to balance exploration\index{Particle Swarm Optimization!exploration} (early iterations) and exploitation\index{Particle Swarm Optimization!exploitation} (late iterations):
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Batch acceleration:} Without Numba\index{Numba optimization}\index{Numba optimization} JIT, runtime would be ~7.4 hours (10$\times$ slower).
...
\begin{itemize}
    ...
    \item Controllers tested: Classical SMC only (with MT-6\index{research tasks!MT-6}\index{research tasks!MT-6} boundary layer optimization)
...
\begin{itemize}
    ...
    \item \textbf{Violate sliding mode theory:} Gains may not satisfy Lyapunov\index{Lyapunov stability}\index{Lyapunov stability} stability\index{stability} conditions for large sliding variable magnitudes.
\end{itemize}

\textbf{Secondary Contributing Factors:}
\begin{enumerate}
    \item \textbf{Fitness function deficiency:} Equation \ref{eq:cost_function} penalizes chattering but not robustness. Need worst-case\index{worst-case analysis} penalty.
...
\begin{enumerate}
    ...
    \item \textbf{Online PSO:} Adapt gains in real-time\index{real-time control} as system parameters drift (e.g., load mass changes, actuator degradation). Requires faster PSO (reduce iterations to 10-20) or meta-learning approach.
...
For practical deployment examples applying these PSO-optimized controllers to industrial automation scenarios, robotics applications, and hardware-in-the-loop\index{hardware-in-the-loop} experiments, see \cref{ch:case_studies}.

\textbf{Next Chapter:} Chapter 10 evaluates controller robustness under model uncertainty\index{uncertainty}\index{model uncertainty}\index{model uncertainty} ($\pm 10\%$, $\pm 20\%$ parameter errors) and systematic disturbance rejection tests.
