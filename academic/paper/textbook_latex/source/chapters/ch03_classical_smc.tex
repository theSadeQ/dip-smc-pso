...
Sliding mode control is a robust nonlinear control technique that forces the system trajectory to converge to a predetermined sliding surface in the state space and remain there for all subsequent time. The fundamental idea, introduced by Utkin in 1977 \cite{Utkin1977}, is to decompose the control design into two phases:
...
where:
\begin{itemize}
    \item $u_{\text{eq}}$ is the \textbf{equivalent control}, a model-based feedforward term that maintains motion along the sliding surface.
...
\end{equation}
\coderef{src/controllers/smc/core/sliding_surface.py}{82}
...
\begin{itemize}
    \item \textbf{Larger $k_1, k_2$}: Faster convergence on the sliding surface but increased control effort and potential chattering.
    \item \textbf{Larger $\lambda_1, \lambda_2$}: Stronger coupling between angles, faster transient response.
    \item \textbf{Trade-off}: Balance settling time (requires high gains) against chattering amplitude (prefers moderate gains).
\end{itemize}

Particle Swarm Optimization (PSO) can systematically explore this trade-off space, as demonstrated in \cref{ch:pso}.
...
The equivalent control $u_{\text{eq}}$ is the control input required to maintain the system on the sliding surface once it has been reached. Mathematically, it is derived by enforcing $\dot{s} = 0$ along the nominal trajectory.
...
If the denominator $\vect{L} \mat{M}^{-1} \vect{B}$ approaches zero, the system is uncontrollable in the direction normal to the sliding surface. To avoid division by near-zero values, we introduce a controllability threshold:
...
where $\sat(\cdot)$ is a saturation function. Two common choices are:
...
\begin{itemize}
    ...
    \item \textbf{Large $\epsilon$}: Smooth control, minimal chattering, but larger steady-state tracking error (proportional to $\epsilon$).
\end{itemize}

\begin{theorem}[Ultimate Boundedness with Boundary Layer]
\label{thm:ultimate_boundedness}
For a classical SMC with boundary layer thickness $\epsilon$, the sliding variable $s(t)$ is ultimately bounded within $|s| \leq \mathcal{O}(\epsilon)$ for all $t \geq T_r$, where $T_r$ is the reaching time.
\end{theorem}
...
\begin{itemize}
    ...
    \item Convergence to the sliding manifold (reaching condition)
...
Benchmarking shows classical SMC achieves $12 \pm 2$ $\mu$s per control cycle on modern CPU, enabling real-time control at 10 kHz sampling rate (see \cref{ch:benchmarking}, Figure 8.1).
...
\begin{itemize}
    ...
    \item \textbf{Boundary layer}: $\epsilon = 0.3$ (MT-6 optimized)
...
\begin{itemize}
    ...
    \item \textbf{Adaptive SMC} (\cref{ch:adaptive_smc}): Online gain adaptation improves robustness to model uncertainty (92\% success rate vs. 85\% for classical SMC under 20\% parameter variations).
...
\begin{enumerate}
    ...
    \item \textbf{Robustness}: Matched disturbance rejection when $K > \sup |d(t)| + \eta$
\end{enumerate}
\end{keybox}
