...
\begin{remark}
Underactuation introduces fundamental constraints on controllability. Unlike fully actuated systems where every degree of freedom can be directly controlled, underactuated systems require careful exploitation of system dynamics (e.g., gravitational potential, centrifugal forces) to achieve desired motions.
\end{remark}
...
\begin{enumerate}
    ...
    \item \textbf{Complex Stabilization}: Maintaining equilibrium requires continuous feedback (unlike fully actuated systems where static feedback may suffice)
...
Consider the cart-pole system with state $x = [x_{\text{cart}}, \theta, \dot{x}_{\text{cart}}, \dot{\theta}]^T$. The control input $u$ (cart force) can directly affect $\ddot{x}_{\text{cart}}$, but $\ddot{\theta}$ is only indirectly influenced through the nonlinear coupling term:
\begin{equation}
...
The double-inverted pendulum (DIP) consists of two rigid links connected in series, mounted on a cart that moves horizontally along a rail. This system serves as an ideal benchmark for underactuated control research due to its:
...
\begin{itemize}
    ...
    \item \textbf{Measurable Complexity}: 3 degrees of freedom, 6-dimensional state space, 1 control input
    \item \textbf{Computational Tractability}: Simulation and control implementation are feasible on standard hardware
\end{itemize}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{figures/placeholder_dip_schematic.png}
\caption[Double-Inverted Pendulum Configuration]{Configuration of the double-inverted pendulum system. The cart (mass $M$) moves horizontally along a rail under control force $u$. Two pendula (masses $m_1, m_2$, lengths $L_1, L_2$) are connected in series. Angles $\theta_1$ and $\theta_2$ are measured from the vertical, with the upright equilibrium at $\theta_1 = \theta_2 = 0$. This system has 3 DOF ($x_{\text{cart}}, \theta_1, \theta_2$) but only 1 actuator ($u$), making it underactuated.}
\label{fig:intro:dip_schematic}
\end{figure}

\subsection{Equations of Motion}

The dynamics of the DIP are derived using Lagrangian mechanics (detailed derivation in \cref{ch:mathematical_foundations}). The equations of motion have the form:
...
\begin{itemize}
    ...
    \item $\mat{M}(\vect{q}) \in \Real^{3 \times 3}$ is the configuration-dependent inertia matrix
    \item $\mat{C}(\vect{q}, \dot{\vect{q}}) \in \Real^{3 \times 3}$ captures Coriolis and centrifugal forces
    \item $\vect{G}(\vect{q}) \in \Real^3$ is the gravitational force vector
...
\begin{enumerate}
    ...
    \item \textbf{Rich Behavior}: The system supports multiple control tasks (stabilization, swing-up, trajectory tracking), enabling comprehensive algorithm testing
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The \textit{reaching condition} ($\sigma \dot{\sigma} < 0$) ensures finite-time convergence to the sliding surface
    ...
    \item \textbf{Lyapunov-Based Design (1980s)}: Development of Lyapunov function methods to systematically prove stability and convergence of SMC laws.
\end{itemize}

\subsection{Chattering Problem and Boundary Layer Method (1980s)}

A major limitation of classical SMC emerged: \textit{chattering}---high-frequency oscillations around the sliding surface caused by imperfect control switching. Burton and Zinober~\cite{burton1986continuous} proposed the \textit{boundary layer method}:
...
This continuous approximation eliminates chattering at the cost of a small steady-state tracking error bounded by the boundary layer thickness $\epsilon$.
...
\begin{itemize}
    ...
    \end{align}
    Achieves \textit{finite-time convergence} of both $\sigma$ and $\dot{\sigma}$ to zero, eliminating chattering while maintaining robustness.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Optimization-Based Tuning}: Particle Swarm Optimization (PSO) and genetic algorithms for systematic gain selection~\cite{messina2013multiobjective}
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Measurement Noise}: Sensor noise causes spurious crossings of the sliding surface
...
\begin{table}[htbp]
\centering
\caption{Comparison of Chattering Reduction Methods}
...
\textbf{Key Features}:
\begin{itemize}
    \item Combines STA finite-time convergence with adaptive gain tuning
    \item Unified sliding surface incorporating cart recentering
    \item Self-tapering adaptation law to prevent overshoot
...
\begin{table}[htbp]
\centering
\caption{Controller Selection Guidelines}
...
\begin{itemize}
    ...
    \item \textbf{Performance Optimization}: Numba JIT compilation for simulation-critical code
\end{itemize}
...
\begin{itemize}
    ...
    \item Multi-objective cost functions (tracking error, control effort, chattering)
...
\begin{itemize}
    ...
    \item \textbf{Web Interface}: Streamlit app for interactive parameter tuning
    \item \textbf{Real-Time Animation}: DIPAnimator class for phase-space visualization
    \item \textbf{Batch Analysis}: Monte Carlo simulations for robustness evaluation
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Chapter 9 (Robustness Analysis)}: Model uncertainty, Monte Carlo simulations, worst-case performance
...
\begin{itemize}
    ...
    \item \textbf{Control Theory}: State-space representation, controllability, feedback linearization (introductory level)
\end{itemize}
...
\begin{itemize}
    \item Basic Python programming (functions, classes, NumPy arrays)
    \item Familiarity with scientific computing libraries (NumPy, SciPy, Matplotlib)
    \item Understanding of object-oriented programming (helpful but not essential)
\end{itemize}

\cref{app:python_guide} provides a quick-start guide for control engineers new to Python.
