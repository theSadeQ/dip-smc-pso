...
This chapter bridges theory and practice by presenting the complete Python implementation of the DIP\index{double-inverted pendulum|see{DIP}}\index{double-inverted pendulum|see{DIP}}-SMC-PSO\index{Particle Swarm Optimization|see{PSO}}\index{Particle Swarm Optimization|see{PSO}} framework. Unlike typical textbook examples that show simplified code snippets, all code presented here is \textbf{production-tested} with:

\begin{itemize}
    \item 100\% test coverage for critical safety components
    \item Rigorous benchmarking\index{benchmarking} across multiple operating conditions (Chapter~\ref{ch:benchmarking})
    ...
    \item Real-time performance optimization with Numba\index{Numba optimization}\index{Numba optimization} JIT compilation
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Correctness over performance}: Controllers are validated against theoretical stability\index{stability}\index{stability} proofs before optimization
...
class ControllerProtocol(Protocol):
    ...
    def compute_control(
        ...
        """Compute control input\index{control input}\index{control input}.
...
class ClassicalSMC:
    """Classical sliding mode controller with boundary layer\index{boundary layer}\index{boundary layer}.

    Implements conventional first-order SMC with:
    - Model-based equivalent control (u_eq)
    - Boundary layer for chattering\index{chattering}\index{chattering} reduction
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Validate positivity (Lyapunov\index{Lyapunov stability}\index{Lyapunov stability} stability requirements)
    ...
    def _compute_equivalent_control(self, state: np.ndarray) -> float:
        """Compute model-based feedforward\index{feedforward control} control u_eq.
        ...
        try:
            ...
            # Regularize inertia matrix\index{inertia matrix}\index{inertia matrix}
            M_reg = M + np.eye(3) * self.regularization

            # Check controllability\index{controllability}\index{controllability}: L @ M^-1 @ B
...
def _sta_smc_core(
    ...
    """Numba-accelerated STA\index{Super-Twisting Algorithm|see{STA}}\index{Super-Twisting Algorithm|see{STA}} core.
...
class SuperTwistingSMC:
    ...
    Achieves finite-time convergence\index{convergence} without discontinuous control.
    ...
    def compute_control(self, state, state_vars, history):
        ...
        # Compute sliding surface\index{sliding surface}\index{sliding surface} and saturated sign
        sigma = self._compute_sliding_surface(state)
        from ...utils.control.primitives import saturate
...
Reference Algorithm~\ref{alg:pso} from Chapter~\ref{ch:pso}. The PSO optimizer automates controller gain tuning\index{gain tuning} via multi-objective cost minimization.
...
class PSOTuner:
    ...
    def objective_function(self, gains_array: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_particles):
            ...
            E = results[i]['energy']  # Integrated control effort\index{performance metrics!control effort}\index{performance metrics!control effort}
...
\begin{itemize}
    ...
    \item \textbf{Multi-objective cost}: Balances settling time\index{performance metrics!settling time}\index{performance metrics!settling time}, overshoot, chattering, energy
...
simulation:
  ...
  initial_state:        # [x, theta1, theta2, dx, dtheta1, dtheta2]
    - 0.0
    - 0.1               # 5.7 degrees initial perturbation\index{perturbation}
    - 0.05
    - 0.0
    - 0.0
    - 0.0
...
# Run classical SMC\index{sliding mode control!classical}\index{sliding mode control!classical} with default gains
...
The Streamlit\index{Streamlit}\index{Streamlit} web UI provides interactive parameter tuning and visualization:
...
\begin{enumerate}
    \item \textbf{Plant Server}: Simulates system dynamics in real-time\index{real-time control}
