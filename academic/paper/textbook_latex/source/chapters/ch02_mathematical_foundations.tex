...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figures/placeholder_dip_coordinates.png}
\caption[DIP Coordinate System]{Coordinate system for the double-inverted pendulum. Generalized coordinates: $q = [x_{\text{cart}}, \theta_1, \theta_2]^T$. Positive angles are measured clockwise from the vertical. The cart position $x_{\text{cart}}$ is measured from an arbitrary reference point on the rail. Centers of mass of the pendula are located at $L_i/2$ from their pivot points.}
\label{fig:math:dip_coordinates}
\end{figure}
...
where $\mat{M}(\vect{q}) \in \Real^{3 \times 3}$ is the configuration-dependent inertia matrix (explicit form in \cref{sec:math:mass_matrix}).
...
\begin{remark}
At the upright equilibrium ($\theta_1 = \theta_2 = 0$), the potential energy is:
\begin{equation}
...
\begin{itemize}
    ...
    \item $\mat{C}(\vect{q}, \dot{\vect{q}}) \in \Real^{3 \times 3}$: Coriolis/centrifugal matrix
...
See \pyfile{src/plant/models/full/dynamics.py} and \pyfile{src/plant/core/physics\_matrices.py} for complete Python implementation of the dynamics equations.
...
This property is fundamental for passivity-based control. Define $\mat{N} = \dot{\mat{M}} - 2\mat{C}$. For any vector $\vect{z} \in \Real^3$:
\begin{equation}
...
\caption{Runge-Kutta 4th Order Integration Step}
\label{alg:math:rk4}
...
\textbf{Next Steps}: \cref{ch:classical_smc} applies these foundations to design the first controller: classical sliding mode control with boundary layer.
