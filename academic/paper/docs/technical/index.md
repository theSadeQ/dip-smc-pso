# Technical Documentation

Technical references, integration guides, and configuration specifications for the DIP SMC PSO framework.

## Overview

This section provides detailed technical documentation for system integration, factory patterns, configuration schemas, and workflow specifications.

## Configuration & Schema

```{toctree}
:maxdepth: 1
:caption: Configuration

configuration_schema_reference
```

## Factory Integration

```{toctree}
:maxdepth: 1
:caption: Factory System

controller_factory_integration
factory_integration_fixes_issue6
factory_usage_examples
integration_protocols
```

## Mathematical Foundations

```{toctree}
:maxdepth: 1
:caption: Mathematical Foundations

mathematical_foundations
```

## PSO Integration

```{toctree}
:maxdepth: 1
:caption: PSO Workflows

pso_integration_workflows
```

## Related Documentation

- [API Reference](../api/index.md)
- [Factory System](../factory/README.md)
- [Mathematical Foundations](../mathematical_foundations/index.md)
- [User Guides](../guides/index.md)
---

**Navigation**: Return to [Master Navigation Hub](../NAVIGATION.md) | Browse all [Documentation Categories](../index.md)
