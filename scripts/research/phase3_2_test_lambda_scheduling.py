"""
Phase 3.2: Selective Lambda (Boundary Layer) Scheduling Strategy Testing

Tests whether scheduling lambda parameters (boundary layer rates) is safer than
scheduling c parameters (sliding surface coefficients) tested in Phase 3.1.

Background:
    Phase 3.1 showed that c1/c2 scheduling causes severe feedback instability:
    - c1_only: +0.0% chattering (no degradation)
    - c2_only: +0.0% chattering (no degradation)
    - full (c1+c2): +208% chattering increase

    Hypothesis: The sliding surface coefficients (c1, c2) are too sensitive to
    schedule adaptively. The boundary layer rates (lambda1, lambda2) may be
    safer to schedule since they control convergence speed, not the sliding
    surface geometry itself.

Hypothesis:
    Selective lambda scheduling (lambda1 only OR lambda2 only) will show less
    degradation than full c1/c2 scheduling from Phase 3.1.

    Target: lambda-only modes achieve <50% chattering increase (vs Phase 3.1
    full mode which showed +208%).

Test Design:
    - Condition 1: Baseline (no scheduling, fixed gains)
    - Condition 2: lambda1 only scheduling (lambda2, c1, c2 fixed)
    - Condition 3: lambda2 only scheduling (lambda1, c1, c2 fixed)
    - Condition 4: Full lambda scheduling (both lambda1 and lambda2)
...
Expected Results:
    1. Baseline shows lowest variance (reference)
    2. lambda1_only shows mild degradation (<30%)
    3. lambda2_only shows mild degradation (<30%)
    4. Full lambda scheduling shows moderate degradation (<50%)
    5. All lambda modes perform BETTER than Phase 3.1 full c1/c2 (+208%)

Usage:
    python scripts/research/phase3_2_test_lambda_scheduling.py
    python scripts/research/phase3_2_test_lambda_scheduling.py --trials 100
    python scripts/research/phase3_2_test_lambda_scheduling.py --quick  # 10 trials
"""
...
def run_single_trial_with_variance_logging(
    ...
    Args:
        ...
        scheduler_mode: "none", "lambda1_only", "lambda2_only", "full"
    ...
    for i in range(steps):
        ...
        c1, c2 = robust_gains[0], robust_gains[2]  # c1, c2 from robust gains (FIXED in lambda scheduling)
...
def create_selective_lambda_scheduler(
    base_controller,
    config,
    mode: str,
    robust_gains: List[float]
    ...
    """
    Create a selective lambda scheduler wrapper.

    Args:
        base_controller: Hybrid controller instance
        config: Configuration object
        mode: "none", "lambda1_only", "lambda2_only", "full"
        robust_gains: [c1, lambda1, c2, lambda2]

    Returns:
        Controller with selective lambda scheduling applied
    """
    if mode == "none":
    ...
    # For all lambda modes (including full), create custom wrapper
    class SelectiveLambdaScheduler:
        """Wrapper that applies selective lambda scheduling (lambda1 or lambda2 only)"""
        ...
        def _schedule_gains(self, state):
            """Apply selective lambda scheduling based on mode"""
            ...
            # CRITICAL DIFFERENCE FROM PHASE 3.1:
            # Schedule lambda parameters (indices 1, 3) instead of c parameters (indices 0, 2)

            if self.mode == "lambda1_only":
                # Only scale lambda1 (index 1)
                # c1, c2, lambda2 remain FIXED
                scheduled_gains[1] = self.robust_gains[1] * scale  # lambda1
                if VERIFICATION_MODE:
                    logger.debug(f"  [lambda1_only] lambda1: {scheduled_gains[1]:.4f} (scale={scale:.2f}), lambda2: {scheduled_gains[3]:.4f} (FIXED)")
            elif self.mode == "lambda2_only":
                # Only scale lambda2 (index 3)
                # c1, c2, lambda1 remain FIXED
                scheduled_gains[3] = self.robust_gains[3] * scale  # lambda2
                if VERIFICATION_MODE:
                    logger.debug(f"  [lambda2_only] lambda1: {scheduled_gains[1]:.4f} (FIXED), lambda2: {scheduled_gains[3]:.4f} (scale={scale:.2f})")
            elif self.mode == "full":
                # Scale both lambda1 and lambda2
                # c1 and c2 remain FIXED
                scheduled_gains[1] = self.robust_gains[1] * scale  # lambda1
                scheduled_gains[3] = self.robust_gains[3] * scale  # lambda2
                if VERIFICATION_MODE:
                    logger.debug(f"  [full] lambda1: {scheduled_gains[1]:.4f} (scale={scale:.2f}), lambda2: {scheduled_gains[3]:.4f} (scale={scale:.2f})")

            return scheduled_gains

        def compute_control(self, state, state_vars, history):
            """Compute control with selective lambda scheduling"""
            # Schedule gains
    ...
    return SelectiveLambdaScheduler(base_controller, mode, robust_gains)
...
def run_monte_carlo_condition(
    ...
    """
    Run Monte Carlo trials for a single lambda scheduling condition.
    ...
    for trial in range(num_trials):
        ...
        # Wrap with selective lambda scheduler
        controller = create_selective_lambda_scheduler(
            base_controller,
            config,
            mode,
            robust_gains
...
def compare_conditions(
    ...
    """
    Compare lambda scheduling conditions statistically.

    Args:
        results_dict: {"none": [...], "lambda1_only": [...], "lambda2_only": [...], "full": [...]}
    ...
    for mode in ["lambda1_only", "lambda2_only", "full"]:
...
def create_comparison_plots(
    ...
    """Create comparison plots for all lambda scheduling modes."""

    modes = ["none", "lambda1_only", "lambda2_only", "full"]
    mode_labels = {
        "none": "Baseline\n(No Scheduling)",
        "lambda1_only": "Lambda1 Only\nScheduling",
        "lambda2_only": "Lambda2 Only\nScheduling",
        "full": "Full Lambda\n(L1+L2)"
    }

    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    fig.suptitle("Phase 3.2: Selective Lambda Scheduling Comparison", fontsize=14, fontweight='bold')
    ...
    for idx, (metric, title, ylabel) in enumerate(metrics_to_plot):
        ...
        data_to_plot = []
        labels_to_plot = []
        colors = ['#2ecc71', '#3498db', '#f39c12', '#9b59b6']  # green, blue, orange, purple
        ...
        if idx < 5:  # Only for metrics in comparison
            ...
            for i, mode in enumerate(["lambda1_only", "lambda2_only", "full"]):
    ...
    # Save plot
    output_path = output_dir / "phase3_2_lambda_scheduling_comparison.png"
    ...
    ax.set_title("Phase 3.2: Variance Increase by Lambda Scheduling Mode", fontsize=14, fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3, axis='y')

    plt.tight_layout()
    output_path = output_dir / "phase3_2_variance_ratio_comparison.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 3.2: Selective Lambda Scheduling Testing")
    ...
    # MT-8 robust PSO gains (same as Phase 3.1)
    robust_gains = [10.149, 12.839, 6.815, 2.750]

    # Output directory
    output_dir = Path("benchmarks/research/phase3_2")
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 70)
    logger.info("Phase 3.2: Selective Lambda Scheduling Strategy Testing")
    logger.info("=" * 70)
    ...
    output_json = output_dir / "phase3_2_lambda_scheduling_report.json"
    ...
    logger.info(f"\n{ASCII_OK} Phase 3.2 testing complete!")
