#!/usr/bin/env python3
"""
Phase 4: Classical SMC Energy PSO Optimization
================================================================================

Optimizes Classical SMC controller gains to minimize energy consumption while
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'classical_smc_energy_phase4.log'),
...
def optimize_classical_smc_energy(
    n_particles: int = 30,
    n_iterations: int = 50,
    seed: int = 42
) -> EnergyPSOResult:
    """
    Run energy PSO optimization for Classical SMC.
    ...
    """
    logger.info("=" * 80)
    logger.info("Phase 4: Classical SMC Energy PSO Optimization")
    logger.info("=" * 80)
...
def save_results(result: EnergyPSOResult, output_dir: Path):
    ...
    # 1. Gains JSON
    gains_file = output_dir / "classical_smc_energy_gains.json"
    ...
    # 2. Summary JSON
    summary_file = output_dir / "classical_smc_energy_summary.json"
    with open(summary_file, 'w') as f:
        json.dump(asdict(result), f, indent=2)
    logger.info(f"Saved summary: {summary_file}")

    # 3. CSV for analysis
    csv_file = output_dir / "classical_smc_energy_optimization.csv"
    df = pd.DataFrame([{
        'metric': 'energy',
    ...
    # Save as NPZ
    npz_file = output_dir / "classical_smc_energy_timeseries.npz"
    np.savez(npz_file, time=t_arr, state=x_arr, control=u_arr)
    logger.info(f"Saved time-series: {npz_file}")


if __name__ == "__main__":
    logger.info("[OK] Starting Phase 4: Classical SMC Energy PSO")

    # Run optimization
    result = optimize_classical_smc_energy(
        n_particles=30,
        n_iterations=50,
        seed=42
    )
    ...
    # Save results
    output_dir = project_root / "academic" / "paper" / "experiments" / "classical_smc" / "optimization" / "energy"
    save_results(result, output_dir)

    logger.info("[OK] Phase 4 Classical SMC energy PSO complete!")
