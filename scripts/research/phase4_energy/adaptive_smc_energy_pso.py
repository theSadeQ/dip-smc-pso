#!/usr/bin/env python3
"""
Phase 4: Adaptive SMC Energy PSO Optimization
================================================================================

Optimizes Adaptive SMC controller gains to minimize energy consumption while
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'adaptive_smc_energy_phase4.log'),
...
def optimize_adaptive_smc_energy(
    n_particles: int = 30,
    n_iterations: int = 50,
    seed: int = 42
) -> EnergyPSOResult:
    """
    Run energy PSO optimization for Adaptive SMC.
    ...
    """
    logger.info("=" * 80)
    logger.info("Phase 4: Adaptive SMC Energy PSO Optimization")
    logger.info("=" * 80)
    ...
    controller_orig = create_controller(controller_type='adaptive_smc', config=config)
    dynamics = DIPDynamics(config=config.physics)
    ...
    best_fitness, optimized_gains = optimizer.optimize(
        lambda p: np.array([objective_function(gains, config, 'adaptive_smc') for gains in p]),
        iters=n_iterations
    )
    ...
    temp_config = config.model_copy(deep=True)
    default_ctrl = temp_config.controller_defaults.adaptive_smc
    updated = default_ctrl.model_copy(update={'gains': optimized_gains.tolist()})
    temp_config.controller_defaults.adaptive_smc = updated

    ctrl_config = temp_config.controllers.adaptive_smc
    updated_ctrl = ctrl_config.model_copy(update={'gains': optimized_gains.tolist()})
    temp_config.controllers.adaptive_smc = updated_ctrl

    controller_opt = create_controller(controller_type='adaptive_smc', config=temp_config)
...
def save_results(result: EnergyPSOResult, output_dir: Path):
    ...
    # 1. Gains JSON
    gains_file = output_dir / "adaptive_smc_energy_gains.json"
    ...
    # 2. Summary JSON
    summary_file = output_dir / "adaptive_smc_energy_summary.json"
    with open(summary_file, 'w') as f:
        json.dump(asdict(result), f, indent=2)
    logger.info(f"Saved summary: {summary_file}")

    # 3. CSV for analysis
    csv_file = output_dir / "adaptive_smc_energy_optimization.csv"
    df = pd.DataFrame([{
        'metric': 'energy',
    ...
    # Save as NPZ
    npz_file = output_dir / "adaptive_smc_energy_timeseries.npz"
    np.savez(npz_file, time=t_arr, state=x_arr, control=u_arr)
    logger.info(f"Saved time-series: {npz_file}")


if __name__ == "__main__":
    logger.info("[OK] Starting Phase 4: Adaptive SMC Energy PSO")

    # Run optimization
    result = optimize_adaptive_smc_energy(
        n_particles=30,
        n_iterations=50,
        seed=42
    )
    ...
    # Save results
    output_dir = project_root / "academic" / "paper" / "experiments" / "adaptive_smc" / "optimization" / "energy"
    save_results(result, output_dir)

    logger.info("[OK] Phase 4 Adaptive SMC energy PSO complete!")
