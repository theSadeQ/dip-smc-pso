#!/usr/bin/env python3
"""
Phase 4: Hybrid Adaptive STA Energy PSO Optimization
================================================================================

Optimizes Hybrid Adaptive STA controller gains to minimize energy consumption while
maintaining performance. Uses a multi-objective fitness function:

    fitness = 0.7 * energy_index + 0.3 * RMSE

This balances energy efficiency with tracking accuracy.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'hybrid_adaptive_sta_smc_energy_phase4.log'),
...
@dataclass
class EnergyPSOResult:
    """Results from energy PSO optimization."""
    controller_name: str
...
def compute_energy_index(u_arr: np.ndarray, dt: float = 0.01) -> float:
    ...
    Energy = sqrt(mean(u^2))

    This measures the overall magnitude of control effort required.
    Lower values indicate more energy-efficient control.

    Args:
        u_arr: Control signal array
        dt: Timestep (s) - not used but kept for API compatibility

    Returns:
        Energy index (higher = more energy consumption)
    """
    if len(u_arr) == 0:
        return 0.0

    # RMS of control effort
...
def compute_fitness(
    ...
    """
    Compute multi-objective fitness for energy PSO.
    ...
    Returns:
        Tuple of (total_fitness, energy_index, rmse)
    """
    energy = compute_energy_index(u_arr, dt)
    rmse = compute_rmse(x_arr)

    # Multi-objective fitness (70% energy, 30% performance)
    fitness = 0.7 * energy + 0.3 * rmse

    return fitness, energy, rmse


def objective_function(gains: np.ndarray, config: Any, controller_type: str) -> float:
    """
    PSO objective function for energy optimization.
    ...
    try:
        ...
        # Compute fitness
        fitness, energy, rmse = compute_fitness(t_arr, x_arr, u_arr, dt=0.01)
...
def optimize_hybrid_adaptive_sta_smc_energy(
    n_particles: int = 30,
    n_iterations: int = 50,
    seed: int = 42
) -> EnergyPSOResult:
    """
    Run energy PSO optimization for Hybrid Adaptive STA.
    ...
    Returns:
        EnergyPSOResult with optimization results
    """
    logger.info("=" * 80)
    logger.info("Phase 4: Hybrid Adaptive STA Energy PSO Optimization")
    logger.info("=" * 80)
    ...
    controller_orig = create_controller(controller_type='hybrid_adaptive_sta_smc', config=config)
    dynamics = DIPDynamics(config=config.physics)
    t_orig, x_orig, u_orig = simulate_with_controller(controller_orig, dynamics)
    fitness_orig, energy_orig, rmse_orig = compute_fitness(t_orig, x_orig, u_orig)

    logger.info(f"Original - Energy: {energy_orig:.4f}, RMSE: {rmse_orig:.4f}, Fitness: {fitness_orig:.4f}")
    ...
    best_fitness, optimized_gains = optimizer.optimize(
        lambda p: np.array([objective_function(gains, config, 'hybrid_adaptive_sta_smc') for gains in p]),
        iters=n_iterations
    )
    ...
    temp_config = config.model_copy(deep=True)
    default_ctrl = temp_config.controller_defaults.hybrid_adaptive_sta_smc
    updated = default_ctrl.model_copy(update={'gains': optimized_gains.tolist()})
    temp_config.controller_defaults.hybrid_adaptive_sta_smc = updated

    ctrl_config = temp_config.controllers.hybrid_adaptive_sta_smc
    updated_ctrl = ctrl_config.model_copy(update={'gains': optimized_gains.tolist()})
    temp_config.controllers.hybrid_adaptive_sta_smc = updated_ctrl

    controller_opt = create_controller(controller_type='hybrid_adaptive_sta_smc', config=temp_config)
    t_opt, x_opt, u_opt = simulate_with_controller(controller_opt, dynamics)
    fitness_opt, energy_opt, rmse_opt = compute_fitness(t_opt, x_opt, u_opt)

    logger.info(f"Optimized - Energy: {energy_opt:.4f}, RMSE: {rmse_opt:.4f}, Fitness: {fitness_opt:.4f}")
...
def save_results(result: EnergyPSOResult, output_dir: Path):
    """Save optimization results to files."""
    output_dir.mkdir(parents=True, exist_ok=True)

    # 1. Gains JSON
    gains_file = output_dir / "hybrid_adaptive_sta_smc_energy_gains.json"
    ...
    # 2. Summary JSON
    summary_file = output_dir / "hybrid_adaptive_sta_smc_energy_summary.json"
    with open(summary_file, 'w') as f:
        json.dump(asdict(result), f, indent=2)
    logger.info(f"Saved summary: {summary_file}")

    # 3. CSV for analysis
    csv_file = output_dir / "hybrid_adaptive_sta_smc_energy_optimization.csv"
    df = pd.DataFrame([{
        'metric': 'energy',
        'before': result.energy_before,
        'after': result.energy_after,
        'improvement_pct': ((result.energy_before - result.energy_after) / result.energy_before) * 100
    }, {
        'metric': 'rmse',
    ...
    # Save as NPZ
    npz_file = output_dir / "hybrid_adaptive_sta_smc_energy_timeseries.npz"
    np.savez(npz_file, time=t_arr, state=x_arr, control=u_arr)
    logger.info(f"Saved time-series: {npz_file}")


if __name__ == "__main__":
    logger.info("[OK] Starting Phase 4: Hybrid Adaptive STA Energy PSO")

    # Run optimization
    result = optimize_hybrid_adaptive_sta_smc_energy(
        n_particles=30,
        n_iterations=50,
        seed=42
    )
    ...
    # Save results
    output_dir = project_root / "academic" / "paper" / "experiments" / "hybrid_adaptive_sta_smc" / "optimization" / "energy"
    save_results(result, output_dir)

    logger.info("[OK] Phase 4 Hybrid Adaptive STA energy PSO complete!")
