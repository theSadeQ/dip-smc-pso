#!/usr/bin/env python3
"""
Phase 5: Multi-Objective PSO - Performance vs Chattering Pareto Front
================================================================================

Generates Pareto-optimal solutions for the trade-off between tracking performance
(RMSE) and safety (chattering index) using weighted-sum multi-objective PSO.

Strategy:
- Run PSO with multiple weight combinations: w1*RMSE + w2*chattering
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'phase5_performance_vs_chattering.log'),
...
def objective_function(
    ...
    try:
        ...
        rmse, chattering = compute_objectives(t_arr, x_arr, u_arr, dt=0.01)

        # Weighted sum fitness
        fitness = weight_rmse * rmse + weight_chattering * chattering
...
def is_dominated(solution1: ParetoSolution, solution2: ParetoSolution) -> bool:
    ...
    # For minimization: solution1 is dominated if solution2 is better in all objectives
    rmse_worse = solution1.rmse >= solution2.rmse
    chattering_worse = solution1.chattering >= solution2.chattering
    at_least_one_strictly_worse = (solution1.rmse > solution2.rmse) or (solution1.chattering > solution2.chattering)

    return rmse_worse and chattering_worse and at_least_one_strictly_worse
...
def run_mopso_controller(
    ...
    logger.info("=" * 80)
    logger.info(f"Phase 5: Performance vs Chattering MOPSO - {controller_type}")
    logger.info("=" * 80)
    ...
    weight_combinations = [
        ...
        (0.7, 0.3),   # Phase 2 default
        (0.5, 0.5),   # Balanced
        (0.3, 0.7),   # Safety-focused
        (0.1, 0.9)    # Maximum chattering reduction
    ]
    ...
    for w_rmse, w_chat in weight_combinations:
        logger.info(f"\nRunning PSO with weights: RMSE={w_rmse:.1f}, Chattering={w_chat:.1f}")
        ...
        best_fitness, optimized_gains = optimizer.optimize(
            lambda p: np.array([objective_function(gains, config, controller_type, w_rmse, w_chat) for gains in p]),
            iters=n_iterations
        )
        ...
        rmse, chattering = compute_objectives(t_arr, x_arr, u_arr)
        ...
        solution = ParetoSolution(
            ...
            weight_rmse=w_rmse,
            weight_chattering=w_chat
        )

        solutions.append(solution)
        logger.info(f"  Result: RMSE={rmse:.4f}, Chattering={chattering:.4f}, Fitness={best_fitness:.4f}")
...
def save_pareto_results(
    ...
    df = pd.DataFrame([{
        ...
        'rmse': sol.rmse,
        'chattering': sol.chattering,
        'fitness': sol.fitness,
        'weight_rmse': sol.weight_rmse,
        'weight_chattering': sol.weight_chattering,
        'is_dominated': sol.is_dominated,
        'gains': json.dumps(sol.gains)
    } for sol in solutions])

    csv_file = output_dir / f"{controller_type}_performance_vs_chattering_all.csv"
    df.to_csv(csv_file, index=False)
    logger.info(f"Saved all solutions: {csv_file}")

    # Save Pareto front only
    pareto_df = df[df['is_dominated'] == False]
    pareto_csv = output_dir / f"{controller_type}_performance_vs_chattering_pareto.csv"
    ...
    plt.scatter(dominated['rmse'], dominated['chattering'],
                alpha=0.3, s=50, color='gray', label='Dominated')
    plt.scatter(non_dominated['rmse'], non_dominated['chattering'],
    ...
    plt.plot(pareto_sorted['rmse'], pareto_sorted['chattering'],
             'r--', alpha=0.5, linewidth=2)

    plt.xlabel('RMSE (rad)', fontweight='bold', fontsize=12)
    plt.ylabel('Chattering Index (RMS du/dt)', fontweight='bold', fontsize=12)
    plt.title(f'Performance vs Chattering Pareto Front\n{controller_type.replace("_", " ").title()}',
              fontweight='bold', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    plot_file = output_dir / f"{controller_type}_performance_vs_chattering_pareto.png"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Phase 5: Performance vs Chattering Multi-Objective PSO")
    logger.info("=" * 80)
    ...
    for ctrl in controllers:
        ...
        output_dir = project_root / "academic" / "paper" / "experiments" / "comparative" / "mopso_performance_chattering"
        save_pareto_results(solutions, ctrl, output_dir)

    logger.info("=" * 80)
    logger.info("[OK] Performance vs Chattering MOPSO complete!")
    logger.info("=" * 80)
