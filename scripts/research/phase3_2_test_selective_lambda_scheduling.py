"""
Phase 3.2: Selective λ1/λ2 (Boundary Layer) Scheduling Strategy Testing

Tests different λ1/λ2 scheduling configurations to determine if boundary layer
modulation is safer than c1/c2 scheduling (tested in Phase 3.1).

Background:
    Phase 3.1 showed that full c1/c2 scheduling causes +208% chattering increase,
    while selective scheduling (c1 or c2 only) has no effect.

Hypothesis:
    λ1/λ2 scheduling (boundary layer modulation) may be safer than c1/c2 scheduling
    because it doesn't directly modify the sliding surface definition, only the
    chattering reduction mechanism.

Test Design:
    - Condition 1: Baseline (no scheduling, fixed λ1/λ2)
    - Condition 2: λ1 only scheduling (λ2 fixed)
    - Condition 3: λ2 only scheduling (λ1 fixed)
    - Condition 4: Full scheduling (both λ1 and λ2)
...
Expected Results:
    1. Baseline shows stable performance (reference)
    2. λ1-only and λ2-only show minimal degradation
    3. Full λ1/λ2 scheduling shows LESS degradation than full c1/c2 (Phase 3.1)
    4. Boundary layer modulation is safer than sliding surface modification

Usage:
    python scripts/research/phase3_2_test_selective_lambda_scheduling.py
    python scripts/research/phase3_2_test_selective_lambda_scheduling.py --trials 100
    python scripts/research/phase3_2_test_selective_lambda_scheduling.py --quick  # 10 trials
"""
...
def create_selective_lambda_scheduler(
    ...
    """
    Create a selective λ1/λ2 scheduler wrapper.
    ...
    Returns:
        Controller with selective λ1/λ2 scheduling applied
    """
    if mode == "none":
    ...
    if mode == "full":
        # Full λ1/λ2 scheduling - use AdaptiveGainScheduler
    ...
    class SelectiveLambdaScheduler:
        """Wrapper that applies selective λ1/λ2 gain scheduling"""
        ...
        def _schedule_gains(self, state):
            """Apply selective λ1/λ2 scheduling based on mode"""
            ...
            if self.mode == "lambda1_only":
                # Only scale lambda1 (index 1), keep c1, c2, lambda2 at robust values
                scheduled_gains[1] = self.robust_gains[1] * scale  # lambda1
                # c1, c2, lambda2 remain unchanged
            elif self.mode == "lambda2_only":
                # Only scale lambda2 (index 3), keep c1, c2, lambda1 at robust values
                scheduled_gains[3] = self.robust_gains[3] * scale  # lambda2
                # c1, c2, lambda1 remain unchanged

            return scheduled_gains

        def compute_control(self, state, state_vars, history):
            """Compute control with selective λ1/λ2 gain scheduling"""
            # Schedule gains
...
def run_monte_carlo_condition(
    ...
    for trial in range(num_trials):
        ...
        # Wrap with selective λ1/λ2 scheduler
...
def create_comparison_plots(
    ...
    mode_labels = {
        "none": "Baseline\n(No Scheduling)",
        "lambda1_only": "λ1 Only\nScheduling",
        "lambda2_only": "λ2 Only\nScheduling",
        "full": "Full\n(λ1+λ2)"
    }

    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    fig.suptitle("Phase 3.2: Selective λ1/λ2 Scheduling Comparison", fontsize=14, fontweight='bold')
    ...
    # Save plot
    output_path = output_dir / "phase3_2_selective_lambda_comparison.png"
    ...
    ax.set_title("Phase 3.2: Variance Change by λ1/λ2 Scheduling Mode", fontsize=14, fontweight='bold')
    ax.legend()
...
def main():
    parser = argparse.ArgumentParser(description="Phase 3.2: Selective λ1/λ2 Scheduling Testing")
    ...
    logger.info("=" * 70)
    logger.info("Phase 3.2: Selective λ1/λ2 Scheduling Strategy Testing")
    logger.info("=" * 70)
    ...
    output_json = output_dir / "phase3_2_selective_lambda_report.json"
