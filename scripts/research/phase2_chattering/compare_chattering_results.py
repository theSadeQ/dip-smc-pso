#!/usr/bin/env python3
"""
Phase 2: Comparative Chattering Analysis
================================================================================

Compares chattering PSO results across all controllers:
- Classical SMC
- Adaptive SMC
- Hybrid Adaptive STA
...
output_dir = project_root / "academic" / "paper" / "experiments" / "comparative" / "chattering_pso"
...
def load_controller_results(controller_name: str) -> Dict:
    """Load chattering PSO results for a controller."""
    summary_file = (
        project_root / "academic" / "paper" / "experiments" /
        controller_name / "optimization" / "chattering" /
        f"{controller_name}_chattering_summary.json"
    )
...
def create_comparative_summary():
    ...
    for ctrl in controllers:
        ...
        results.append({
            ...
            'Chattering Before': data['chattering_before'],
            'Chattering After': data['chattering_after'],
            'Chattering Reduction (%)': ((data['chattering_before'] - data['chattering_after']) /
                                          max(data['chattering_before'], 1e-10)) * 100,
    ...
    # Save to CSV
    csv_file = output_dir / "chattering_pso_comparative_summary.csv"
    df.to_csv(csv_file, index=False, float_format='%.4f')
    logger.info(f"Saved summary table: {csv_file}")

    # Save formatted table
    txt_file = output_dir / "chattering_pso_comparative_summary.txt"
    with open(txt_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("Phase 2: Chattering PSO Comparative Analysis\n")
        f.write("=" * 100 + "\n\n")
        ...
        # Best chattering reduction
        best_chat_idx = df['Chattering Reduction (%)'].idxmax()
        f.write(f"1. Best Chattering Reduction: {df.iloc[best_chat_idx]['Controller']}\n")
        f.write(f"   - Reduction: {df.iloc[best_chat_idx]['Chattering Reduction (%)']:.2f}%\n")
        f.write(f"   - From: {df.iloc[best_chat_idx]['Chattering Before']:.4f} -> {df.iloc[best_chat_idx]['Chattering After']:.4f}\n\n")
        ...
        # Baseline comparison
        f.write("3. Baseline Chattering Performance:\n")
        for _, row in df.iterrows():
            f.write(f"   - {row['Controller']}: {row['Chattering Before']:.4f}\n")
...
def create_comparison_plots(df: pd.DataFrame):
    ...
    fig.suptitle('Phase 2: Chattering PSO Comparative Results', fontsize=16, fontweight='bold')

    controllers = df['Controller'].tolist()
    x = np.arange(len(controllers))
    width = 0.35

    # 1. Chattering comparison
    ax = axes[0, 0]
    ax.bar(x - width/2, df['Chattering Before'], width, label='Before', alpha=0.8, color='#e74c3c')
    ax.bar(x + width/2, df['Chattering After'], width, label='After', alpha=0.8, color='#27ae60')
    ax.set_xlabel('Controller', fontweight='bold')
    ax.set_ylabel('Chattering Index (RMS du/dt)', fontweight='bold')
    ax.set_title('Chattering Reduction', fontweight='bold')
    ax.set_xticks(x)
    ...
    # Save plot
    plot_file = output_dir / "chattering_pso_comparison.png"
...
def create_gains_comparison():
    ...
    # Save to CSV
    csv_file = output_dir / "chattering_pso_gains_comparison.csv"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Phase 2: Chattering PSO Comparative Analysis")
    logger.info("=" * 80)
