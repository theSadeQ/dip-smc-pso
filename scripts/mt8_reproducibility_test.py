#!/usr/bin/env python
"""
MT-8 Reproducibility Test: Validate PSO Optimization Reproducibility
================================================================================

Re-runs the MT-8 robust PSO optimization from baseline gains with explicit
random seeds to validate reproducibility of optimization results.

Test Protocol:
- Run optimization with 3 different seeds (42, 123, 456)
- Use identical PSO parameters as original MT-8 (30 particles, 50 iterations)
- Start from baseline gains (pre-MT-8, nominal-only tuning)
- Compare final fitness, optimized gains, and improvement percentages
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('benchmarks/mt8_reproducibility_test.log'),
...
def optimize_controller_robust_pso(
    ...
    """
    logger.info(f"\n{'='*80}")
    logger.info(f"Optimizing: {controller_name} (Robust PSO, Seed={random_seed})")
    logger.info(f"{'='*80}")
    logger.info(f"  Particles: {n_particles}, Iterations: {n_iterations}, Seed: {random_seed}")

    # Set random seed for reproducibility
    np.random.seed(random_seed)
    logger.info(f"  [OK] Random seed set: {random_seed}")

    # Get original gains (from baseline)
    ...
    logger.info("  Evaluating baseline gains...")
    original_fitness = evaluate_robust_fitness(
        np.array(original_gains), controller_name, dynamics
    )
    logger.info(f"    Baseline robust fitness: {original_fitness:.4f}")
    ...
    logger.info(f"  Starting PSO optimization with seed {random_seed}...")
...
def main():
    """Main MT-8 reproducibility test execution."""
    import argparse
    parser = argparse.ArgumentParser(description='MT-8: Reproducibility Test')
    parser.add_argument('--seed', type=int, required=True,
                        help='Random seed for PSO optimization')
    parser.add_argument('--save-prefix', type=str, required=True,
                        help='Prefix for output files (e.g., mt8_repro_seed42)')
    ...
    logger.info(f"Save Prefix: {args.save_prefix}")
    logger.info("="*80)

    # Load config
    config = load_config("config.yaml")
    logger.info("Configuration loaded (baseline gains from controller_defaults)")
    ...
    for controller_idx, controller_name in enumerate(controllers):
        ...
        try:
            ...
            # Save gains immediately with seed suffix
            gains_file = Path(f"optimization_results/{args.save_prefix}_{controller_name}.json")
            ...
            with open(gains_file, 'w') as f:
                json.dump({
                    ...
                    'baseline_gains': result.original_gains,
                    'baseline_cost': result.robust_cost_before
                }, f, indent=2)
    ...
    summary_file = Path(f"optimization_results/{args.save_prefix}_summary.json")
    ...
    # Print summary
    logger.info(f"\n{'='*80}")
    logger.info(f"REPRODUCIBILITY TEST SUMMARY (Seed={args.seed})")
    logger.info(f"{'='*80}")
    logger.info(f"\n{'Controller':<30} {'Baseline Cost':<15} {'Optimized Cost':<15} {'Improvement'}")
    logger.info('-'*80)
    ...
    logger.info(f"{'='*80}")
    logger.info("[OK] Reproducibility test complete!")
