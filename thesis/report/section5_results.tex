...
Baseline performance comparison (manually-tuned gains from Section~\ref{sec:controllers}) shows MPC achieving fastest settling time (1.48~s) and lowest overshoot (1.2\%), followed by STA-SMC (1.82~s settling, 2.3\% overshoot). Classical SMC serves as the baseline with 2.15~s settling time and 5.8\% overshoot. PSO optimization (Section~\ref{sec:pso}) improves these baselines by 25-40\%.
