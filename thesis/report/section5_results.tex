...
% Note: Comprehensive table included in landscape format due to width (18 columns)
\clearpage
\begin{landscape}
\input{tables/comprehensive}
\end{landscape}
\clearpage
...
The comprehensive benchmark table (Table \ref{tab:comprehensive}) includes 95\% confidence intervals for all metrics. Non-overlapping confidence intervals provide visual confirmation of statistical significance.
...
\begin{table}[htbp]
\centering
\caption{Computational Performance (mean $\pm$ std, $N=1000$ control steps)}
\label{tab:computational}
\begin{tabular}{lcccc}
\toprule
Controller & Time ($\mu$s) & Real-Time Factor & 100Hz Suitable & Memory (KB) \\
\midrule
Classical SMC & $18.5 \pm 2.1$ & 541$\times$ & Yes & 2.4 \\
STA-SMC & $24.2 \pm 3.4$ & 413$\times$ & Yes & 3.1 \\
Adaptive SMC & $31.6 \pm 4.8$ & 316$\times$ & Yes & 4.2 \\
Hybrid Adaptive STA & $26.8 \pm 3.9$ & 373$\times$ & Yes & 3.8 \\
\bottomrule
\multicolumn{5}{l}{Real-Time Factor = Available time (10ms) / Compute time}
\end{tabular}
\end{table}
