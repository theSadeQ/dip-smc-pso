...
1. **Logs** → `academic/logs/` directory (hidden, centralized)
   - **Centralized paths:** Use `src/utils/logging/paths.py` (MANDATORY)
   - **Structure:** `academic/logs/{pso,test,monitoring,archive}/`
   - **Format:** `{script}_{timestamp}.log`
   - **Examples:** `academic/logs/pso/pso_classical_20250930.log`, `academic/logs/test/pytest_run_20250930.log`
...
- [x] Move all logs to `academic/logs/` directory (completed Dec 17, 2025)
- [x] Centralize log paths via `src/utils/logging/paths.py` (completed Dec 17, 2025)
- [x] Archive old logs to `academic/logs/archive/YYYY-MM-DD/` (completed Dec 17, 2025)
...
- [ ] Verify `academic/logs/` size: `du -sh academic/logs/` (target: ≤100MB, current: 12MB [OK])
...
- `.archive/`, `.test_artifacts/`, `.ai_workspace/`, `.build/`, `.cache/`, `.coverage/`, `academic/`
- `academic/logs/` (centralized logging), `.git/`, `.github/`
...
| `*.log`, `report.*` | `academic/logs/` | `output.log` → `academic/logs/script_20251009.log` |
...
# CORRECT: Logs in academic/logs/ with timestamps
python optimize.py > academic/logs/optimize_$(date +%Y%m%d_%H%M%S).log
```
...
mv *.log academic/logs/ 2>/dev/null || true
...
academic/logs/benchmarks/           # Log files (9.7 MB, hidden directory)
```
...
- **Backup:** `academic/backups/benchmarks_pre_reorg_20251218.tar.gz`
...
- `logs/` (120 KB) → `academic/logs/` (centralized)
- `monitoring_data/` (56 MB) → `academic/logs/archive/` (compressed to 214 KB)
...
All logs now in `academic/logs/` (hidden directory):
...
**Changes:**
- Moved 13 PSO logs (978 KB) → `academic/logs/pso/`
...
# Automatically resolves to academic/logs/ subdirectories
pso_log = PSO_LOG_DIR / f"{datetime.now():%Y%m%d}_{controller}_pso.log"
```

**Environment Override:**
```bash
export LOG_DIR="/custom/log/path"  # Override default academic/logs/
```

### DO NOT

- Create logs in root directory (use `academic/logs/`)
- Hardcode "logs/" paths (use `src/utils/logging/paths.py`)
- Create visible log directories (always use hidden `academic/logs/`)
- Store PSO logs in `optimization_results/` (use `academic/logs/pso/`)

### See Also

- Log paths config: `src/utils/logging/paths.py`
- Logging handlers: `src/utils/logging/handlers.py`
- Logs README: `academic/logs/README.md`
...
2. **Prepare Monitoring**
   - Set up log directory: `academic/logs/`
...
2. **Progress Tracking**
   - Monitor log files: `tail -f academic/logs/pso_*.log`
...
3. **Moved** logs:
   - `report.log` → `academic/logs/report.log`
   - `logs/` → `academic/logs/` (made hidden)
4. **Moved** scripts:
...
**Updates:**
- Updated `.gitignore` to include `academic/logs/`
...
du -sh academic/logs/                        # Target: <100MB (current: ~8MB)
...
|---------|----------|----------|--------|
| Research artifacts | `academic/` | `artifacts/` | Hidden = not cluttering root |
...
| Development tools | `.ai_workspace/dev_tools/` | `.dev_tools/` (at root) | Centralized config location |
