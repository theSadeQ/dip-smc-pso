# .ai_workspace/tools - Development Tools
...
**See**: `.ai_workspace/orchestration/agent_checkpoint_system.md` for design
...
**Contents**: 200+ automation scripts from legacy `.ai_workspace/dev_tools/`
...
**See**: `academic/research/` for full analysis reports
...
# View project state
python .ai_workspace/tools/recovery/project_state_manager.py show
```

### Checkpoint Workflow
```bash
# Launch task with checkpointing
python .ai_workspace/tools/checkpoints/launch_checkpoint_task.py --task LT-4

# Analyze checkpoint status
python .ai_workspace/tools/checkpoints/analyze_checkpoints.py

# Cleanup test checkpoints
python .ai_workspace/tools/checkpoints/cleanup_test_checkpoints.py
```

### Multi-Account Recovery
```bash
# See comprehensive guide
cat .ai_workspace/tools/multi_account/MULTI_ACCOUNT_RECOVERY_GUIDE.md

# Task wrapper usage
python .ai_workspace/tools/multi_account/task_wrapper.py --help
```

## Migration Notes

**2025-12-18**: Reorganized from 3 legacy locations:
- `.ai_workspace/dev_tools/` -> `tools/automation/` (200+ files)
- `.ai_workspace/recovery/` -> `tools/recovery/` and `tools/checkpoints/`
- `.ai_workspace/config/tools/` -> `tools/misc/`
