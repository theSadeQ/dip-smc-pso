...
**Deleted**:
- `D:Projectsmainacademic/` (corrupted Windows path)
...
**Moved to `.ai_workspace/`**:
- `.claude/` → `.ai_workspace/claude/`
- `.config/` → `.ai_workspace/config/`
- `.dev_tools/` → `.ai_workspace/dev_tools/`
- `.mcp_servers/` → `.ai_workspace/mcp_servers/`
- `.archive/` → `.ai_workspace/archive_temp/` (will merge later)
...
**Problem**: Both `artifacts/` and `academic/` exist
**Solution**:
- Move `artifacts/production_readiness.db` → `academic/testing/`
- Move `artifacts/testing/` → `academic/testing/`
- Delete `artifacts/` directory
- **Single source**: `academic/` only
...
**Files to move**:
- `optimize_adaptive_boundary.py` → `academic/scripts/optimize_adaptive_boundary_TIMESTAMP.py`
- `out.png` → `academic/temp/out.png`
- `LT7_conference_overleaf.pdf` → `academic/LT7_research_paper/deliverables/`
...
**Add patterns**:
```
.ai_workspace/archive/
.cache/
docs/_build/
*.tar.gz
```

**Remove old patterns** (now in .ai_workspace/):
```
.claude/
.config/
.dev_tools/
...
- Document `.ai_workspace/` consolidation
...
# View this plan
cat .ai_workspace/dev_tools/RESTRUCTURING_PLAN_2025-10-26.md
