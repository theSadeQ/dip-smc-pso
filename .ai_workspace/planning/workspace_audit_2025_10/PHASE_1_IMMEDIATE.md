...
find optimization_results -name "*.json" -type f -exec dirname {} \; | sort | uniq -c >> academic/audit_cleanup/optimization_results_analysis/analysis.txt

# Identify unique files (by content hash)
echo -e "\n=== Unique File Hashes ===" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
...
# Find duplicates
echo -e "\n=== Duplicate Files ===" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
...
cp -r optimization_results academic/audit_cleanup/optimization_results_backup_$(date +%Y%m%d_%H%M%S)

# Verify backup
du -sh optimization_results academic/audit_cleanup/optimization_results_backup_*
```
...
find optimization_results -maxdepth 1 -name "*.json" -type f >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# List phase directories (keep these)
find optimization_results -maxdepth 1 -type d | grep -v "^optimization_results$" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# List files in phase directories (keep these)
find optimization_results -maxdepth 2 -name "*.json" -type f | grep -v "optimization_results/optimization_results" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
```
...
find optimization_results -name "*.json" -type f | sort > academic/audit_cleanup/optimization_results_analysis/files_after.txt

# Compare before/after
echo -e "\n=== File Count Comparison ===" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
echo "Before: $(wc -l < academic/audit_cleanup/optimization_results_analysis/files_before.txt)" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
echo "After: $(wc -l < academic/audit_cleanup/optimization_results_analysis/files_after.txt)" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt

# Calculate space saved
du -sh academic/audit_cleanup/optimization_results_backup_* optimization_results | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt
```
...
while read hash; do
    backup_file=$(grep "$hash" academic/audit_cleanup/optimization_results_analysis/hashes.txt | head -1 | cut -d' ' -f2-)
...
done < academic/audit_cleanup/optimization_results_analysis/duplicate_hashes.txt
...
Backup Location: academic/audit_cleanup/optimization_results_backup_*
Analysis: academic/audit_cleanup/optimization_results_analysis/
...
2. `academic/` (~500KB) - Should be gitignored artifacts
...
grep "^\academic/" .gitignore  # Should find it at line 69
...
if ! grep -q "^\academic/$" .gitignore; then
    echo "academic/" >> .gitignore
fi
```
...
#### Step 2.3: Untrack academic/ Directory (10 minutes)
...
# Verify files still exist locally
ls academic/ | head -5  # Should show files
...
- academic/ (~500KB) listed in .gitignore but tracked in git
...
Verification:
- Local files preserved: ls logs/ academic/ (both work)
...
- [ ] `logs/` and `academic/` still exist on disk
- [ ] `git status` shows clean (no logs/ or academic/ changes)
...
echo "=== Comparing data/ vs data/data/ ===" > academic/audit_cleanup/data_comparison.txt
for file in data/data/*; do
    basename=$(basename "$file")
    if [ -f "data/$basename" ]; then
        if diff "data/$basename" "$file" > /dev/null; then
            echo "[IDENTICAL] $basename" >> academic/audit_cleanup/data_comparison.txt
        else
            echo "[DIFFERENT] $basename" >> academic/audit_cleanup/data_comparison.txt
        fi
    else
        echo "[UNIQUE IN data/data/] $basename" >> academic/audit_cleanup/data_comparison.txt
    fi
done

cat academic/audit_cleanup/data_comparison.txt

# If all identical, delete nested dir
if ! grep -q "\[DIFFERENT\]" academic/audit_cleanup/data_comparison.txt && \
   ! grep -q "\[UNIQUE IN data/data/\]" academic/audit_cleanup/data_comparison.txt; then
    rm -rf data/data/
...
- Comparison log: academic/audit_cleanup/data_comparison.txt
...
### Quick Win 4: Delete .ai_workspace/dev_tools/.ai_workspace/dev_tools/ Nested Dir (10 minutes)

```bash
# Verify nested duplication
ls .ai_workspace/dev_tools/.ai_workspace/dev_tools/  # Should show Switch-ClaudeAccount.ps1

# Compare with canonical version
diff .ai_workspace/dev_tools/Switch-ClaudeAccount.ps1 .ai_workspace/dev_tools/.ai_workspace/dev_tools/Switch-ClaudeAccount.ps1

# If identical, delete nested dir
if diff .ai_workspace/dev_tools/Switch-ClaudeAccount.ps1 .ai_workspace/dev_tools/.ai_workspace/dev_tools/Switch-ClaudeAccount.ps1 > /dev/null; then
    rm -rf .ai_workspace/dev_tools/.ai_workspace/dev_tools/
    echo "[OK] Deleted .ai_workspace/dev_tools/.ai_workspace/dev_tools/ (duplicate of canonical)"
else
    echo "[WARNING] Files differ. Manual review needed."
fi

# Note: .ai_workspace/dev_tools/ itself is gitignored (backward compat), so no commit needed
echo "[INFO] .ai_workspace/dev_tools/.ai_workspace/dev_tools/ removed (parent .ai_workspace/dev_tools/ is gitignored)"
```
...
mkdir -p .ai_workspace/claude/

# Check if target already exists
if [ -f .ai_workspace/claude/settings.local.json ]; then
    echo "[WARNING] Target already exists. Comparing..."
    diff .claude/settings.local.json .ai_workspace/claude/settings.local.json
    echo "[ACTION NEEDED] Manually resolve differences before proceeding."
else
    # Move file
    mv .claude/settings.local.json .ai_workspace/claude/
    echo "[OK] Moved .claude/settings.local.json to .ai_workspace/claude/"
fi
...
# Note: .ai_workspace/ may be gitignored, verify
if git check-ignore .ai_workspace/claude/settings.local.json > /dev/null; then
    echo "[INFO] .ai_workspace/claude/settings.local.json is gitignored (expected)"
else
...
- [ ] `.ai_workspace/dev_tools/.ai_workspace/dev_tools/` deleted
- [ ] `__pycache__/` at root gitignored
- [ ] 3 duplicate presentation files removed and committed
- [ ] `.claude/settings.local.json` moved to `.ai_workspace/claude/`
...
echo "=== Root Directory Count ===" > academic/audit_cleanup/phase1_validation.txt
ls -1 | wc -l >> academic/audit_cleanup/phase1_validation.txt
echo "Target: ≤19 visible items" >> academic/audit_cleanup/phase1_validation.txt

# 2. No nested optimization_results
echo -e "\n=== Nested optimization_results Check ===" >> academic/audit_cleanup/phase1_validation.txt
find optimization_results -name "optimization_results" -type d >> academic/audit_cleanup/phase1_validation.txt || echo "None found (GOOD)" >> academic/audit_cleanup/phase1_validation.txt

# 3. Gitignore violations
echo -e "\n=== Gitignore Violations ===" >> academic/audit_cleanup/phase1_validation.txt
git ls-files logs/ >> academic/audit_cleanup/phase1_validation.txt || echo "logs/ not tracked (GOOD)" >> academic/audit_cleanup/phase1_validation.txt
git ls-files academic/ >> academic/audit_cleanup/phase1_validation.txt || echo "academic/ not tracked (GOOD)" >> academic/audit_cleanup/phase1_validation.txt

# 4. Data duplication
echo -e "\n=== Data Duplication Check ===" >> academic/audit_cleanup/phase1_validation.txt
[ -d data/data ] && echo "data/data/ EXISTS (BAD)" >> academic/audit_cleanup/phase1_validation.txt || echo "data/data/ not found (GOOD)" >> academic/audit_cleanup/phase1_validation.txt

# 5. Git status clean
echo -e "\n=== Git Status ===" >> academic/audit_cleanup/phase1_validation.txt
git status --porcelain >> academic/audit_cleanup/phase1_validation.txt || echo "Working tree clean (GOOD)" >> academic/audit_cleanup/phase1_validation.txt

# Display results
cat academic/audit_cleanup/phase1_validation.txt

# Summary
echo -e "\n=== PHASE 1 COMPLETION SUMMARY ==="
echo "Validation report: academic/audit_cleanup/phase1_validation.txt"
echo "Total commits created: $(git log --oneline --since="4 hours ago" --author="Claude" | wc -l)"
echo "Estimated cleanup time: 4 hours"
echo "Next phase: .ai_workspace/planning/workspace_audit_2025_10/PHASE_2_THIS_WEEK.md"
```
...
- `.ai_workspace/planning/workspace_audit_2025_10/PHASE_2_THIS_WEEK.md`
