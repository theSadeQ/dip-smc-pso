...
**Audit Report Location:** `.ai_workspace/planning/workspace_audit_2025_10/AUDIT_SUMMARY.md`
...
echo "=== Tool Availability Check ===" > academic/audit_cleanup/tool_check.txt

# Git (CRITICAL)
if command -v git &> /dev/null; then
    git --version >> academic/audit_cleanup/tool_check.txt
...
if command -v python &> /dev/null || command -v python3 &> /dev/null; then
    python --version 2>&1 >> academic/audit_cleanup/tool_check.txt || python3 --version >> academic/audit_cleanup/tool_check.txt
...
if command -v md5sum &> /dev/null; then
    echo "md5sum: available" >> academic/audit_cleanup/tool_check.txt
    echo "[OK] md5sum available"
elif command -v md5 &> /dev/null; then
    echo "md5: available (macOS)" >> academic/audit_cleanup/tool_check.txt
...
if command -v find &> /dev/null; then
    echo "find: available" >> academic/audit_cleanup/tool_check.txt
...
if command -v tar &> /dev/null; then
    tar --version | head -1 >> academic/audit_cleanup/tool_check.txt
...
if command -v wc &> /dev/null; then
    echo "wc: available" >> academic/audit_cleanup/tool_check.txt
...
if command -v diff &> /dev/null; then
    echo "diff: available" >> academic/audit_cleanup/tool_check.txt
...
# Display summary
cat academic/audit_cleanup/tool_check.txt
...
```powershell
# Create tool check report
"=== Tool Availability Check ===" | Out-File -FilePath academic/audit_cleanup/tool_check.txt

# Git (CRITICAL)
try {
    $gitVersion = git --version
    $gitVersion | Out-File -Append -FilePath academic/audit_cleanup/tool_check.txt
...
try {
    ...
    $pythonVersion | Out-File -Append -FilePath academic/audit_cleanup/tool_check.txt
...
"Get-FileHash: available (PowerShell built-in)" | Out-File -Append -FilePath academic/audit_cleanup/tool_check.txt
...
if (Get-Command 7z -ErrorAction SilentlyContinue) {
    ...
    "7-Zip: available" | Out-File -Append -FilePath academic/audit_cleanup/tool_check.txt
} else {
    Write-Host "[INFO] Using PowerShell Compress-Archive (built-in)" -ForegroundColor Yellow
    "Compress-Archive: available (PowerShell built-in)" | Out-File -Append -FilePath academic/audit_cleanup/tool_check.txt
}

# Display summary
Get-Content academic/audit_cleanup/tool_check.txt
...
mkdir -p academic/audit_cleanup/snapshots
cp -r optimization_results academic/audit_cleanup/snapshots/optimization_results_$BACKUP_TIMESTAMP
cp -r data academic/audit_cleanup/snapshots/data_$BACKUP_TIMESTAMP
echo "[OK] Critical directories snapshot created"

# Save backup metadata
cat > academic/audit_cleanup/backup_metadata.txt <<EOF
...
Snapshot Location: academic/audit_cleanup/snapshots/

ROLLBACK INSTRUCTIONS:
1. Git rollback: git reset --hard audit-start-$BACKUP_TIMESTAMP
2. Tar restore: tar -xzf ../main_backup_$BACKUP_TIMESTAMP.tar.gz
3. Directory restore: cp -r academic/audit_cleanup/snapshots/[dir]_$BACKUP_TIMESTAMP [dir]/
EOF

cat academic/audit_cleanup/backup_metadata.txt
echo ""
...
python -m pytest tests/ -v --tb=short -m "not slow" > academic/audit_cleanup/test_results_before.txt 2>&1

# Check exit code
if [ $? -eq 0 ]; then
    echo "[OK] All tests passing before Phase 1"
    grep -E "passed|failed|error" academic/audit_cleanup/test_results_before.txt | tail -5
else
    echo "[WARNING] Some tests failing before Phase 1"
    echo "Review: academic/audit_cleanup/test_results_before.txt"
    read -p "Continue anyway? (y/n): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        exit 1
...
- [ ] Backup metadata saved to `academic/audit_cleanup/backup_metadata.txt`
...
echo ""
echo "--- Backup Status ---"
if [ -f academic/audit_cleanup/backup_metadata.txt ]; then
    echo "Backup: CREATED"
    grep "Tag:" academic/audit_cleanup/backup_metadata.txt
else
    echo "Backup: NOT CREATED YET"
fi
echo ""
echo "=================================="
echo "Ready to proceed: $([ -f academic/audit_cleanup/backup_metadata.txt ] && echo 'YES' || echo 'NO - Create backup first')"
echo "=================================="
EOFSCRIPT
...
mkdir -p academic/audit_cleanup/optimization_results_analysis

# List ALL JSON files with full paths
echo "[1/6] Listing all JSON files..."
find optimization_results -name "*.json" -type f | sort > academic/audit_cleanup/optimization_results_analysis/files_before.txt
TOTAL_FILES=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_before.txt)
echo "Found: $TOTAL_FILES JSON file references"

# Count files at each depth level
echo "[2/6] Analyzing depth distribution..."
echo "=== Files by Depth ===" > academic/audit_cleanup/optimization_results_analysis/analysis.txt
find optimization_results -name "*.json" -type f -exec dirname {} \; | sort | uniq -c | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt

# Identify unique files (by content hash)
echo "[3/6] Computing file hashes (this may take 30-60 seconds)..."
echo -e "\n=== File Hashes ===" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
find optimization_results -name "*.json" -type f -exec md5sum {} \; | sort > academic/audit_cleanup/optimization_results_analysis/hashes.txt

# Find duplicate hashes
echo "[4/6] Identifying duplicates..."
echo -e "\n=== Duplicate Analysis ===" >> academic/audit_cleanup/optimization_results_analysis/analysis.txt
cat academic/audit_cleanup/optimization_results_analysis/hashes.txt | awk '{print $1}' | sort | uniq -d > academic/audit_cleanup/optimization_results_analysis/duplicate_hashes.txt
UNIQUE_HASHES=$(cat academic/audit_cleanup/optimization_results_analysis/duplicate_hashes.txt | wc -l)
echo "Unique files with duplicates: $UNIQUE_HASHES" | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt

# Identify nested optimization_results directories
echo "[5/6] Finding nested 'optimization_results' directories..."
find optimization_results -type d -name "optimization_results" > academic/audit_cleanup/optimization_results_analysis/nested_dirs.txt
NESTED_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/nested_dirs.txt)
echo "Nested 'optimization_results' directories found: $NESTED_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt
cat academic/audit_cleanup/optimization_results_analysis/nested_dirs.txt | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt

# Calculate space usage
echo "[6/6] Calculating space usage..."
du -sh optimization_results | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt
du -sh optimization_results/optimization_results 2>/dev/null | tee -a academic/audit_cleanup/optimization_results_analysis/analysis.txt || echo "Nested dir not found (already clean?)"
...
echo "Analysis saved: academic/audit_cleanup/optimization_results_analysis/"
echo ""
...
cp -r optimization_results "academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP"

# Verify backup
echo "[2/4] Verifying backup integrity..."
ORIGINAL_FILES=$(find optimization_results -type f | wc -l)
BACKUP_FILES=$(find "academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP" -type f | wc -l)
...
BACKUP_SIZE=$(du -sh "academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP" | awk '{print $1}')
echo "Original: $ORIGINAL_SIZE | Backup: $BACKUP_SIZE"

# Create backup manifest
echo "[4/4] Creating backup manifest..."
cat > "academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP/BACKUP_MANIFEST.txt" <<EOF
=== BACKUP MANIFEST ===
Timestamp: $(date)
Backup ID: $BACKUP_TIMESTAMP
Original Path: $(pwd)/optimization_results
Backup Path: $(pwd)/academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP
File Count: $BACKUP_FILES files
Size: $BACKUP_SIZE

RESTORE COMMAND:
rm -rf optimization_results/
cp -r academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP optimization_results
...
cat "academic/audit_cleanup/optimization_results_backup_$BACKUP_TIMESTAMP/BACKUP_MANIFEST.txt"
echo ""
...
echo "=== TARGET STRUCTURE ===" > academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "Decision Rule: Keep files at shallowest depth (1-2 levels)" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# Top-level JSON files (depth 1) - KEEP ALL
echo "[1/4] Identifying top-level files (depth 1)..."
echo "--- Top-Level Files (KEEP) ---" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
find optimization_results -maxdepth 1 -name "*.json" -type f | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
TOP_LEVEL_COUNT=$(find optimization_results -maxdepth 1 -name "*.json" -type f | wc -l)
echo "Found: $TOP_LEVEL_COUNT top-level files" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# Phase directories (depth 1) - KEEP ALL
echo "[2/4] Identifying phase directories..."
echo "--- Phase Directories (KEEP) ---" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
find optimization_results -maxdepth 1 -type d | grep -v "^optimization_results$" | grep -v "optimization_results/optimization_results" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
...
echo "Found: $PHASE_DIR_COUNT phase directories" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# Files in phase directories (depth 2) - KEEP ALL
echo "[3/4] Identifying files in phase directories (depth 2)..."
echo "--- Files in Phase Directories (KEEP) ---" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
find optimization_results -maxdepth 2 -name "*.json" -type f | grep -v "optimization_results/optimization_results" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
...
echo "Found: $PHASE_FILE_COUNT files in phase directories" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# Nested optimization_results directories (depth 2+) - DELETE ALL
echo "[4/4] Identifying nested directories to DELETE..."
echo "--- Nested optimization_results Directories (DELETE) ---" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt
find optimization_results -type d -name "optimization_results" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
NESTED_DELETE_COUNT=$(find optimization_results -type d -name "optimization_results" | wc -l)
echo "Found: $NESTED_DELETE_COUNT nested directories to delete" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/target_structure.txt

# Summary
echo "=== CANONICAL STRUCTURE SUMMARY ===" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "Files to KEEP:" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "  - Top-level JSONs: $TOP_LEVEL_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "  - Phase directories: $PHASE_DIR_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "  - Files in phases: $PHASE_FILE_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "  TOTAL CANONICAL FILES: $((TOP_LEVEL_COUNT + PHASE_FILE_COUNT))" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo "Directories to DELETE: $NESTED_DELETE_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/target_structure.txt

cat academic/audit_cleanup/optimization_results_analysis/target_structure.txt
echo ""
...
find optimization_results -maxdepth 2 -name "*.json" -type f | \
    ...
    sort > academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt
...
find optimization_results -path "*/optimization_results/optimization_results/*" -name "*.json" -type f | \
    sort > academic/audit_cleanup/optimization_results_analysis/files_to_delete.txt

KEEP_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt)
DELETE_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_to_delete.txt)
...
done < academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt | \
    sort > academic/audit_cleanup/optimization_results_analysis/hashes_to_keep.txt
...
done < academic/audit_cleanup/optimization_results_analysis/files_to_delete.txt | \
    sort > academic/audit_cleanup/optimization_results_analysis/hashes_to_delete.txt

# Verify all DELETE files have a KEEP equivalent with same hash
echo "[4/5] Verifying safe deletion (matching hashes)..."
echo "=== SAFETY VERIFICATION ===" > academic/audit_cleanup/optimization_results_analysis/safety_check.txt
...
while read -r file; do
    if [ -f "$file" ]; then
        ...
        KEEP_FILE=$(grep "$BASENAME" academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt | head -1)

        if [ -n "$KEEP_FILE" ] && [ -f "$KEEP_FILE" ]; then
            KEEP_HASH=$(md5sum "$KEEP_FILE" | awk '{print $1}')

            if [ "$DELETE_HASH" == "$KEEP_HASH" ]; then
                echo "[OK] $BASENAME - Hash match ($DELETE_HASH)" | tee -a academic/audit_cleanup/optimization_results_analysis/safety_check.txt
            else
                echo "[ERROR] $BASENAME - HASH MISMATCH! DELETE: $DELETE_HASH | KEEP: $KEEP_HASH" | tee -a academic/audit_cleanup/optimization_results_analysis/safety_check.txt
                echo "[CRITICAL] Cannot proceed - file content differs!"
                exit 1
            fi
        else
            echo "[WARNING] $BASENAME - No matching file to keep found" | tee -a academic/audit_cleanup/optimization_results_analysis/safety_check.txt
        fi
    fi
done < academic/audit_cleanup/optimization_results_analysis/files_to_delete.txt
...
VERIFIED=$(grep -c "\[OK\]" academic/audit_cleanup/optimization_results_analysis/safety_check.txt || echo 0)
ERRORS=$(grep -c "\[ERROR\]" academic/audit_cleanup/optimization_results_analysis/safety_check.txt || echo 0)
WARNINGS=$(grep -c "\[WARNING\]" academic/audit_cleanup/optimization_results_analysis/safety_check.txt || echo 0)
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Review: academic/audit_cleanup/optimization_results_analysis/safety_check.txt"
    exit 1
else
    echo ""
...
2. Review mismatch details in `academic/audit_cleanup/optimization_results_analysis/safety_check.txt`
...
echo "=== DRY RUN: DIRECTORIES TO DELETE ===" > academic/audit_cleanup/optimization_results_analysis/dry_run.txt

find optimization_results -type d -name "optimization_results" | while read -r dir; do
    echo "WOULD DELETE: $dir" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
    echo "  Contents:" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
    find "$dir" -type f | wc -l | xargs echo "    - Files:" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
    du -sh "$dir" | awk '{print "    - Size: " $1}' | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
    echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
done
...
echo "Total space to free: ~${SPACE_TO_FREE}KB (~$((SPACE_TO_FREE / 1024))MB)" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt

# Show files that will remain
echo "[3/4] Showing files that will remain..."
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "=== FILES THAT WILL REMAIN ===" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
cat academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt

# Final structure preview
echo "[4/4] Showing final structure preview..."
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "=== FINAL STRUCTURE PREVIEW ===" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "optimization_results/" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "├── adaptive_boundary_gains_2024_10.json (top-level)" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "├── phase53/" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "│   ├── gains_sta_lyapunov_optimized.json" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "│   ├── optimized_gains_adaptive_smc_phase53.json" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "│   └── ... (other phase files)" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt
echo "└── [NO MORE NESTED optimization_results/]" | tee -a academic/audit_cleanup/optimization_results_analysis/dry_run.txt

cat academic/audit_cleanup/optimization_results_analysis/dry_run.txt
...
echo "=== DELETION LOG ===" > academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
echo "Timestamp: $(date)" >> academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
echo "" >> academic/audit_cleanup/optimization_results_analysis/deletion_log.txt

DELETED_COUNT=0
find optimization_results -type d -name "optimization_results" | while read -r dir; do
    if [ -d "$dir" ]; then
        echo "[DELETING] $dir" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
        rm -rf "$dir"

        if [ ! -d "$dir" ]; then
            echo "[SUCCESS] Deleted: $dir" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
            DELETED_COUNT=$((DELETED_COUNT + 1))
        else
            echo "[ERROR] Failed to delete: $dir" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
            exit 1
        fi
    fi
done

echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
echo "Total directories deleted: $DELETED_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
...
if [ "$REMAINING" -eq 0 ]; then
    echo "[SUCCESS] No nested optimization_results directories remain" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
else
    echo "[ERROR] $REMAINING nested directories still exist!" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
    find optimization_results -type d -name "optimization_results" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
    exit 1
fi
...
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
echo "=== REMAINING FILES ===" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
find optimization_results -name "*.json" -type f | sort | tee academic/audit_cleanup/optimization_results_analysis/files_after.txt | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
REMAINING_FILES=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_after.txt)
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
echo "Remaining JSON files: $REMAINING_FILES" | tee -a academic/audit_cleanup/optimization_results_analysis/deletion_log.txt
...
rm -rf optimization_results/
cp -r academic/audit_cleanup/optimization_results_backup_*/optimization_results optimization_results
...
echo "=== DATA LOSS VALIDATION ===" > academic/audit_cleanup/optimization_results_analysis/validation_report.txt

BEFORE_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_before.txt)
AFTER_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_after.txt)
EXPECTED_COUNT=$(wc -l < academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt)

echo "Files before cleanup: $BEFORE_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "Files after cleanup: $AFTER_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "Expected canonical files: $EXPECTED_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt

if [ "$AFTER_COUNT" -ne "$EXPECTED_COUNT" ]; then
    echo "[ERROR] File count mismatch! Expected: $EXPECTED_COUNT | Got: $AFTER_COUNT" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
    exit 1
else
    echo "[OK] File count matches expected: $AFTER_COUNT files" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
fi
echo ""
...
while read -r file; do
    if [ ! -f "$file" ]; then
        echo "[ERROR] MISSING: $file" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
        MISSING=$((MISSING + 1))
    fi
done < academic/audit_cleanup/optimization_results_analysis/files_to_keep.txt

if [ "$MISSING" -gt 0 ]; then
    echo "[CRITICAL] $MISSING files missing!" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
    exit 1
else
    echo "[OK] All $EXPECTED_COUNT canonical files present" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
fi
echo ""

# Re-compute hashes and verify integrity
echo "[3/5] Re-computing hashes to verify file integrity..."
find optimization_results -name "*.json" -type f -exec md5sum {} \; | sort > academic/audit_cleanup/optimization_results_analysis/hashes_after.txt
...
while read -r line; do
    ...
    AFTER_HASH=$(grep "$FILE" academic/audit_cleanup/optimization_results_analysis/hashes_after.txt | awk '{print $1}')

    if [ -z "$AFTER_HASH" ]; then
        echo "[WARNING] File not found in AFTER hashes: $FILE" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
    elif [ "$HASH" != "$AFTER_HASH" ]; then
        echo "[ERROR] Hash mismatch for $FILE: Before $HASH | After $AFTER_HASH" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
        HASH_MISMATCHES=$((HASH_MISMATCHES + 1))
    fi
done < academic/audit_cleanup/optimization_results_analysis/hashes_to_keep.txt

if [ "$HASH_MISMATCHES" -gt 0 ]; then
    echo "[CRITICAL] $HASH_MISMATCHES hash mismatches detected!" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
    exit 1
else
    echo "[OK] All file hashes match (no corruption)" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
fi
echo ""

# Calculate space savings
echo "[5/5] Calculating space savings..."
du -sh optimization_results > academic/audit_cleanup/optimization_results_analysis/size_after.txt
SIZE_AFTER=$(cat academic/audit_cleanup/optimization_results_analysis/size_after.txt | awk '{print $1}')

echo "" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "=== SPACE SAVINGS ===" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "Size after cleanup: $SIZE_AFTER" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt
echo "Expected savings: ~520KB (from nested duplicates)" | tee -a academic/audit_cleanup/optimization_results_analysis/validation_report.txt

cat academic/audit_cleanup/optimization_results_analysis/validation_report.txt
...
git status --short > academic/audit_cleanup/optimization_results_analysis/git_status.txt
cat academic/audit_cleanup/optimization_results_analysis/git_status.txt

DELETED_FILES=$(grep "^ D " academic/audit_cleanup/optimization_results_analysis/git_status.txt | wc -l)
...
git diff --cached --stat optimization_results/ | tee academic/audit_cleanup/optimization_results_analysis/git_diff_stat.txt
...
BACKUP: academic/audit_cleanup/optimization_results_backup_*
...
Currently **33 files (8.5MB)** in `logs/` and `academic/` are tracked by git despite being gitignored. This causes:
...
academic/ directory: Some files tracked (should be fully gitignored)
```
...
# Create analysis directory
mkdir -p academic/audit_cleanup/gitignore_analysis

# Find all tracked files in logs/ and academic/
echo "[1/4] Finding tracked files in logs/ and academic/..."
git ls-files logs/ > academic/audit_cleanup/gitignore_analysis/tracked_logs.txt 2>/dev/null || touch academic/audit_cleanup/gitignore_analysis/tracked_logs.txt
git ls-files academic/ > academic/audit_cleanup/gitignore_analysis/tracked_artifacts.txt 2>/dev/null || touch academic/audit_cleanup/gitignore_analysis/tracked_artifacts.txt

TRACKED_LOGS=$(wc -l < academic/audit_cleanup/gitignore_analysis/tracked_logs.txt)
TRACKED_ARTIFACTS=$(wc -l < academic/audit_cleanup/gitignore_analysis/tracked_artifacts.txt)
TOTAL_TRACKED=$((TRACKED_LOGS + TRACKED_ARTIFACTS))

echo "Tracked files in logs/: $TRACKED_LOGS"
echo "Tracked files in academic/: $TRACKED_ARTIFACTS"
echo "Total tracked files that should be ignored: $TOTAL_TRACKED"
echo ""

# Combine into single list
echo "[2/4] Creating combined list..."
cat academic/audit_cleanup/gitignore_analysis/tracked_logs.txt \
    academic/audit_cleanup/gitignore_analysis/tracked_artifacts.txt | \
    sort > academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt
...
done < academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt
...
cat > academic/audit_cleanup/gitignore_analysis/gitignore_violations_report.txt <<EOF
=== GITIGNORE VIOLATIONS REPORT ===
Generated: $(date)

SUMMARY:
- Tracked files in logs/: $TRACKED_LOGS
- Tracked files in academic/: $TRACKED_ARTIFACTS
...
cat academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt >> academic/audit_cleanup/gitignore_analysis/gitignore_violations_report.txt

cat academic/audit_cleanup/gitignore_analysis/gitignore_violations_report.txt
...
- [ ] Tracked files in academic/ identified
...
**Objective:** Ensure .gitignore has correct patterns for logs/ and academic/
...
echo "=== GITIGNORE PATTERN CHECK ===" > academic/audit_cleanup/gitignore_analysis/pattern_check.txt

# Pattern 1: logs/
if grep -q "^logs/$" .gitignore || grep -q "^logs/\*$" .gitignore; then
    echo "[OK] logs/ pattern found" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
else
    echo "[WARNING] logs/ pattern missing or incorrect" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
fi

# Pattern 2: academic/
if grep -q "^\academic/$" .gitignore || grep -q "^\academic/\*$" .gitignore || grep -q "^academic/$" .gitignore; then
    echo "[OK] academic/ pattern found" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
else
    echo "[WARNING] academic/ pattern missing or incorrect" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
fi

# Pattern 3: *.log.* (log rotations)
if grep -q "\*.log\.\*" .gitignore || grep -q "\*.log\.[0-9]" .gitignore; then
    echo "[OK] Log rotation pattern found" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
else
    echo "[WARNING] Log rotation pattern missing" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
fi

# Show current gitignore content for logs/ and academic/
echo "[3/3] Showing relevant .gitignore patterns..."
echo "" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
echo "Current .gitignore patterns (logs and artifacts related):" | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt
grep -E "(logs|artifacts|\.log)" .gitignore | tee -a academic/audit_cleanup/gitignore_analysis/pattern_check.txt || echo "No patterns found"

cat academic/audit_cleanup/gitignore_analysis/pattern_check.txt
...
# Pattern 2: academic/
if ($gitignoreContent -match "^\academic/$" -or $gitignoreContent -match "^\academic/\*$" -or $gitignoreContent -match "^academic/$") {
    $msg = "[OK] academic/ pattern found"
    Write-Host $msg -ForegroundColor Green
    $msg | Out-File -Append .artifacts\audit_cleanup\gitignore_analysis\pattern_check.txt
} else {
    $msg = "[WARNING] academic/ pattern missing or incorrect"
    Write-Host $msg -ForegroundColor Yellow
    $msg | Out-File -Append .artifacts\audit_cleanup\gitignore_analysis\pattern_check.txt
}
...
TOTAL_FILES=$(wc -l < academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt)
echo "Files to untrack: $TOTAL_FILES"
echo ""

# Dry run first
echo "[1/3] DRY RUN: Showing what will be untracked..."
echo "=== UNTRACK DRY RUN ===" > academic/audit_cleanup/gitignore_analysis/untrack_log.txt
while read -r file; do
    if [ -f "$file" ]; then
        echo "Would untrack: $file" | tee -a academic/audit_cleanup/gitignore_analysis/untrack_log.txt
    fi
done < academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt
...
while read -r file; do
    if [ -f "$file" ]; then
        git rm --cached "$file" >> academic/audit_cleanup/gitignore_analysis/untrack_log.txt 2>&1
        if [ $? -eq 0 ]; then
            echo "[OK] Untracked: $file"
            UNTRACKED=$((UNTRACKED + 1))
        else
            echo "[ERROR] Failed to untrack: $file" | tee -a academic/audit_cleanup/gitignore_analysis/untrack_log.txt
            FAILED=$((FAILED + 1))
        fi
    fi
done < academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt
...
if [ "$FAILED" -gt 0 ]; then
    echo "[WARNING] Some files failed to untrack. Review: academic/audit_cleanup/gitignore_analysis/untrack_log.txt"
fi
...
git status --short > academic/audit_cleanup/gitignore_analysis/git_status_after_untrack.txt
DELETED_COUNT=$(grep -c "^D " academic/audit_cleanup/gitignore_analysis/git_status_after_untrack.txt || echo 0)
...
echo "[1/3] Testing git status (should not show logs/ or academic/ as untracked)..."
git status --short logs/ academic/ > academic/audit_cleanup/gitignore_analysis/git_status_test.txt 2>&1

if [ -s academic/audit_cleanup/gitignore_analysis/git_status_test.txt ]; then
    echo "[WARNING] Git still showing some files:"
    cat academic/audit_cleanup/gitignore_analysis/git_status_test.txt
else
...
TEST_FILE=$(head -1 academic/audit_cleanup/gitignore_analysis/all_tracked_violations.txt)
if [ -f "$TEST_FILE" ]; then
    git add "$TEST_FILE" 2>&1 | tee academic/audit_cleanup/gitignore_analysis/git_add_test.txt

    if grep -q "ignored" academic/audit_cleanup/gitignore_analysis/git_add_test.txt; then
...
git status --short > academic/audit_cleanup/gitignore_analysis/final_git_status.txt
cat academic/audit_cleanup/gitignore_analysis/final_git_status.txt

DELETED_FILES=$(grep -c "^D " academic/audit_cleanup/gitignore_analysis/final_git_status.txt || echo 0)
echo ""
...
git add -u logs/ academic/ 2>/dev/null || git add -u

# Create commit message
echo "[3/4] Creating commit..."
COMMIT_MSG=$(cat <<'EOFMSG'
fix(workspace): Untrack gitignored files from logs/ and academic/

PROBLEM:
- 33 files (8.5MB) in logs/ and academic/ were tracked by git
...
[░░░░░░░░░░░░] QW #4:  Verify .ai_workspace/ structure compliance                  (10 min) ⏱ 0:15-0:25
...
### Quick Win #4: Verify .ai_workspace/ Structure Compliance (10 minutes)

**Problem:** Ensure all AI/dev configs are in .ai_workspace/ (not scattered in multiple dirs)

**Impact:** Workspace organization per CLAUDE.md section 14

**Bash:**
```bash
echo "[QW #4] Verifying .ai_workspace/ structure compliance..."

# Expected subdirectories in .ai_workspace/
EXPECTED_DIRS=("ai" "claude" "config" "dev_tools" "mcp_servers")

echo "[1/3] Checking .ai_workspace/ subdirectories..."
MISSING_DIRS=()
for dir in "${EXPECTED_DIRS[@]}"; do
    if [ -d ".ai_workspace/$dir" ]; then
        echo "  [OK] .ai_workspace/$dir/ exists"
    else
        echo "  [MISSING] .ai_workspace/$dir/ not found"
        MISSING_DIRS+=("$dir")
    fi
done

# Check for legacy directories that should be in .ai_workspace/
echo ""
...
for dir in "${LEGACY_DIRS[@]}"; do
    if [ -d "$dir" ]; then
        echo "  [WARNING] Found legacy directory: $dir (should be in .ai_workspace/)"
        FOUND_LEGACY+=("$dir")
    fi
done
...
if [ ${#MISSING_DIRS[@]} -eq 0 ] && [ ${#FOUND_LEGACY[@]} -eq 0 ]; then
    echo "[SUCCESS] .ai_workspace/ structure compliant"
else
...
if [ "$HIDDEN_COUNT" -le 7 ]; then
    ...
    echo "[ACTION] Review and consolidate into .ai_workspace/"
fi
...
- QW #4: Verified .ai_workspace/ structure compliance
...
**Location:** `academic/audit_cleanup/phase1_pre_validation.py`

**Usage:**
```bash
# Bash
python academic/audit_cleanup/phase1_pre_validation.py
...
class PreFlightValidator:
    ...
    def check_backup_directory(self) -> bool:
        ...
        backup_dir = Path('academic/audit_cleanup')
...
**Location:** `academic/audit_cleanup/phase1_post_validation.py`

**Usage:**
```bash
# Bash
python academic/audit_cleanup/phase1_post_validation.py
...
class PostExecutionValidator:
    ...
    def check_task2_gitignore_violations(self) -> bool:
        ...
        try:
            # Check if logs/ and academic/ are tracked
            ...
            if total_tracked > 0:
                ...
                self.issues.append(f"Task 2: {total_tracked} files still tracked in logs/academic/")
                print(f"  {RED}✗ FAIL{RESET}: {total_tracked} gitignored files still tracked")
                print(f"    - logs/: {tracked_logs} files")
                print(f"    - academic/: {tracked_artifacts} files")
                return False
            else:
...
**Location:** `academic/audit_cleanup/phase1_health_monitor.sh`

**Usage:**
```bash
# Run in separate terminal while executing Phase 1
bash academic/audit_cleanup/phase1_health_monitor.sh

# Or source it to run in background
source academic/audit_cleanup/phase1_health_monitor.sh &
```
...
get_metrics() {
    ...
    TRACKED_ARTIFACTS=$(git ls-files academic/ 2>/dev/null | wc -l)
...
display_dashboard() {
    ...
    print_metric "Tracked files in academic/" "$TRACKED_ARTIFACTS" "0" "="
...
```bash
cd D:/Projects/main
python academic/audit_cleanup/script.py
```
...
```bash
rm -rf optimization_results/
cp -r academic/audit_cleanup/optimization_results_backup_*/optimization_results/ .
...
Does .git/ directory exist?
  ...
  └─ YES → Continue
        ↓
Run: python academic/audit_cleanup/phase1_pre_validation.py
...
```
Need to restore?
  ├─ Task 1 failed → Use academic/audit_cleanup/optimization_results_backup_*
...
mkdir -p academic/audit_cleanup/phase1_metrics

# Metric 1: Root organization
echo "ROOT_ITEMS=$(ls -1 | wc -l)" > academic/audit_cleanup/phase1_metrics/metrics.txt
echo "HIDDEN_DIRS=$(find . -maxdepth 1 -type d -name ".*" ! -name ".git" | wc -l)" >> academic/audit_cleanup/phase1_metrics/metrics.txt

# Metric 2: Nested directories
echo "NESTED_DIRS=$(find optimization_results -type d -name "optimization_results" 2>/dev/null | wc -l)" >> academic/audit_cleanup/phase1_metrics/metrics.txt

# Metric 3: Gitignore violations
echo "TRACKED_VIOLATIONS=$(git ls-files logs/ academic/ 2>/dev/null | wc -l)" >> academic/audit_cleanup/phase1_metrics/metrics.txt

# Metric 4: Git repo size
echo "REPO_SIZE=$(du -sh .git | awk '{print $1}')" >> academic/audit_cleanup/phase1_metrics/metrics.txt
...
```bash
cat << 'EOF' > academic/audit_cleanup/phase1_metrics/comparison.md
...
- [ ] **Task 2:** Zero files tracked in `logs/` and `academic/`
...
- [ ] **Task 3:** `.ai_workspace/` structure compliant
...
- [ ] Backup files still accessible in `academic/audit_cleanup/`
...
5. **Archive old experiments** (`.ai_workspace/archive/`)
...
5. Check `academic/audit_cleanup/` for backups
