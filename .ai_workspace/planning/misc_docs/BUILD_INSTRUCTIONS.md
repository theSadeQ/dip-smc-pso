...
# [i] Baseline saved to academic/warning_baseline.json
```
...
```bash
# Run every Monday
python scripts/check_docs.py --all > academic/weekly-health-$(date +%Y%m%d).txt

# Compare to previous week
diff academic/weekly-health-$(date -d '7 days ago' +%Y%m%d).txt \
     academic/weekly-health-$(date +%Y%m%d).txt
```
...
echo "Build Time Report - $(date)" > academic/build-times.log
...
for project in user api dev; do
  ...
  duration=$((end - start))
  echo "docs-$project: ${duration}s" >> academic/build-times.log
done

# Alert if any build >600s (10 min)
if grep -q "[6-9][0-9][0-9]s" academic/build-times.log; then
