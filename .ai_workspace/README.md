...
The `.ai_workspace/` directory is the single source of truth for:
...
**Changes**:
1. **State Consolidation**: `.ai_workspace/config/` + `recovery/state/` -> `.ai_workspace/state/`
...
python .ai_workspace/tools/checkpoints/launch_checkpoint_task.py --task MT-6
```
...
- [ERROR] `.ai/` -> use `.ai_workspace/`
- [ERROR] `.dev_tools/` at root -> use `.ai_workspace/tools/`
- [ERROR] `recovery/state/` -> use `.ai_workspace/state/`

**Reason**: Consolidated to `.ai_workspace/` as canonical root (2025-12-18 migration)
...
2. Review `.ai_workspace/guides/` for how-tos
3. Analyze project state: `bash .ai_workspace/tools/recovery/recover_project.sh`
4. Check Git history: `git log .ai_workspace/ --oneline`
