...
# Factory Integration System Documentation

## Overview

The Double-Inverted Pendulum (DIP) SMC-PSO project implements a comprehensive factory integration system that provides robust, thread-safe controller instantiation with deep PSO optimization integration. This documentation covers the architecture, implementation patterns, and usage guidelines for the factory system that resolves GitHub issue #6.
...
The factory integration system follows enterprise-grade design patterns with clear separation of concerns:
...
└── PSO Integration Layer
    ├── Controller Wrapper System (PSO-compatible interfaces)
    ├── Gain Validation (Domain-specific validation)
    └── Batch Creation (Performance optimization)
```

### Design Principles

1. **Enterprise-Grade Quality**: Thread-safe operations with comprehensive error handling
...
Thread-safe operations using recursive locks with timeout protection:
...
The factory system resolves configuration from multiple sources with priority:
...
**Primary factory function for controller creation.**

**Parameters:**
- `controller_type` (str): Controller type identifier
  - `'classical_smc'`: Classical sliding mode controller
  - `'sta_smc'`: Super-twisting sliding mode controller
  - `'adaptive_smc'`: Adaptive sliding mode controller
  - `'hybrid_adaptive_sta_smc'`: Hybrid adaptive-STA controller
  - `'mpc_controller'`: Model predictive controller (optional)

- `config` (Optional[Any]): Configuration object containing controller parameters
- `gains` (Optional[Union[list, np.ndarray]]): Controller gain array

**Returns:**
- Configured controller instance implementing `ControllerProtocol`

**Raises:**
- `ValueError`: Invalid controller type or parameters
- `ImportError`: Missing required dependencies

**Thread Safety:** ✅ Thread-safe with timeout protection
...
**Returns:**
- `List[str]`: List of available controller type identifiers
...
#### `create_smc_for_pso(smc_type, gains, plant_config_or_model=None, **kwargs)`
...
#### `create_pso_controller_factory(smc_type, plant_config=None, **kwargs)`
...
**Returns:**
- `Callable[[Union[list, np.ndarray]], Any]`: Controller factory function with PSO attributes
...
**Returns:**
- `Tuple[List[float], List[float]]`: (lower_bounds, upper_bounds) for PSO optimization

### Controller Type Enums and Classes
...
#### `create_controller_legacy(controller_type, config=None, gains=None)`

**Legacy factory function for backward compatibility.**

#### `create_classical_smc_controller(config=None, gains=None)`
#### `create_sta_smc_controller(config=None, gains=None)`
#### `create_adaptive_smc_controller(config=None, gains=None)`

**Type-specific factory functions for backward compatibility.**
...
The factory system integrates seamlessly with the simulation engine:
...
Integration with PSO optimization workflows:
...
#### Issue: `ValueError: Unknown controller type`
...
2. **Use correct controller names:**
   - ✅ `'classical_smc'` (correct)
   - ❌ `'classical'` (incorrect)
   - ✅ `'sta_smc'` (correct)
   - ❌ `'super_twisting'` (incorrect)

3. **Check controller aliases:**
   ```python
...
#### Issue: `ImportError: Controller class not available`
...
#### Issue: Gain validation failures
...
- Controller-specific validation failures

**Solutions:**
1. **Check required gain count:**
   ```python
...
3. **Use controller-specific validation:**
   ```python
...
2. **Update deprecated parameters:**
   ```python
   # Deprecated -> Current
   'use_equivalent' -> 'enable_equivalent_control'
   'k_gain' -> 'switching_gain'
...
3. **Provide required parameters:**
   ```python
   # Classical SMC required parameters:
   config_params = {
       'gains': [20.0, 15.0, 12.0, 8.0, 35.0, 5.0],
       'max_force': 150.0,
       'boundary_layer': 0.02,
...
**Solutions:**
1. **Avoid excessive concurrent calls:**
   ```python
...
3. **Use pre-created controllers for PSO:**
   ```python
...
The factory system is optimized for performance-critical applications:
...
This comprehensive documentation provides complete coverage of the factory integration system, addressing all aspects of implementation, usage, and troubleshooting. The system successfully resolves GitHub issue #6 by providing robust, thread-safe controller instantiation with deep PSO optimization integration.