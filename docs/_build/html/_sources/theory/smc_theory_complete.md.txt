...
Sliding Mode Control (SMC) is a robust control methodology that provides finite-time convergence and inherent disturbance rejection capabilities {cite}`smc_utkin_1993_sliding_mode_control_design`. The fundamental principle is to constrain the system trajectory to evolve on a lower-dimensional sliding surface where desired dynamics are enforced. ### Fundamental Concepts **Definition 1 (Sliding Surface)**: A sliding surface $\mathcal{S}$ is a subset of the state space defined by: ```{math}
