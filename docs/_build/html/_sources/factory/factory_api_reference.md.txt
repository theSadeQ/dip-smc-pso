...
## GitHub Issue #6 Enhanced Controller Factory System

### Overview

This comprehensive API reference documents the enhanced controller factory system implemented as part of GitHub Issue #6 resolution. The factory provides thread-safe, type-safe controller instantiation with advanced validation and PSO optimization support.
...
| Controller Type | Aliases | Gain Count | Description |
|----------------|---------|------------|-------------|
...
| `'mpc_controller'` | - | 0 | Model predictive controller (if available) |
...
**Returns list of available controller types.**
...
- **Gains**: `[k1, k2, λ1, λ2, K, kd]`
  - `k1, k2`: Proportional gains for pendulum 1 and 2
  - `λ1, λ2`: Sliding surface coefficients
  - `K`: Switching gain magnitude
  - `kd`: Derivative gain for chattering reduction

- **Sliding Surface**: `s = λ₁e₁ + λ₂e₂ + ė₁ + ė₂`
- **Control Law**: `u = -K·sign(s) + u_eq`
...
- **Gains**: `[K1, K2, k1, k2, λ1, λ2]`
  - `K1, K2`: Super-twisting algorithmic gains
  - `k1, k2`: Surface proportional gains
  - `λ1, λ2`: Sliding surface coefficients
...
- **Adaptation Law**: `K̇ = γ·|s| - σ·K` (with leak rate σ)
- **Control Law**: `u = -K(t)·sign(s) + u_eq`
...
## PSO Integration APIs

### `create_pso_controller_factory(smc_type, **kwargs)`

**Creates PSO-optimized controller factory function.**
...
#### Returns
Factory function compatible with PSO optimization
...
| Controller | Lower Bounds | Upper Bounds |
|-----------|--------------|--------------|
...
**Validates gains for controller type.**
...
## SMC Enumeration Types

### `SMCType`

**Enumeration of SMC controller types.**
...
**General SMC configuration dataclass.**
...
**Checks and migrates deprecated configuration parameters.**
...
**Returns migration guide for controller type.**
...
The factory system provides comprehensive thread safety:

1. **Reentrant Locks**: All factory operations use `threading.RLock()`
...
1. **Legacy Functions**: Deprecated functions remain available with warnings
...
Planned enhancements maintain backward compatibility:
...
This API reference provides comprehensive documentation for the enhanced factory system, enabling efficient and reliable controller creation for advanced control systems research.