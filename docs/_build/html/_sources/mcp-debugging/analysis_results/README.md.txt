...
This directory contains comprehensive code quality analysis results from the **mcp-analyzer** MCP server.
...
### 1. RUFF Findings Report
**Filename Pattern**: `RUFF_FINDINGS_YYYYMMDD_HHMMSS.md`

Contains comprehensive RUFF linting analysis:
...
**Example**: `RUFF_FINDINGS_20251006_143000.md`

### 2. VULTURE Findings Report
**Filename Pattern**: `VULTURE_FINDINGS_YYYYMMDD_HHMMSS.md`

Contains comprehensive dead code detection:
...
- Phase-by-phase breakdown
- Recommended actions for each confidence level

**Example**: `VULTURE_FINDINGS_20251006_143000.md`

### 3. Summary Report (JSON)
**Filename Pattern**: `SUMMARY_REPORT_YYYYMMDD_HHMMSS.json`

Machine-readable comprehensive summary:
...
**Example**: `SUMMARY_REPORT_20251006_143000.json`

**Usage**: CI integration, historical comparison, automated quality gates

## 🔍 How to Read Reports

### Understanding RUFF Severity Codes

| Code Prefix | Severity | Description | Examples |
|-------------|----------|-------------|----------|
...
| **N** | Naming | Naming convention violations | N802 (function name) |

### Understanding VULTURE Confidence Scores

| Range | Confidence Level | Action | Description |
|-------|------------------|--------|-------------|
| **90-100%** | High | Safe to remove | Very likely dead code |
| **70-89%** | Medium | Manual review | Probably dead, verify first |
| **0-69%** | Low | Document only | Likely false positive or intentional |

## 📈 Analysis Timeline

| Date | Timestamp | Scope | RUFF Issues | VULTURE Items | Status |
|------|-----------|-------|-------------|---------------|--------|
| 2025-10-06 | 143000 | Full codebase | TBD | TBD | ⏳ Pending |

*(This table will be updated as analyses are completed)*

## 🎯 Acceptance Criteria

All analysis runs must meet these criteria:

- ✅ **Coverage**: 100% of Python files analyzed (604/604)
- ✅ **Completeness**: Both RUFF and VULTURE run on all files
- ✅ **Quality**: Zero files skipped due to unrecoverable errors
- ✅ **Reporting**: All three report types generated successfully
...
4. Document retention rationale for VULTURE false positives

### For Code Reviews
1. Check SUMMARY_REPORT for overall quality metrics
2. Verify no new high-severity RUFF issues introduced
3. Review VULTURE findings for deleted/refactored code
...
4. Generate quality badges from metrics
...
python scripts/mcp_validation/compare_analysis.py \
    docs/mcp-debugging/analysis_results/SUMMARY_REPORT_20251006_100000.json \
    docs/mcp-debugging/analysis_results/SUMMARY_REPORT_20251006_150000.json
```

*(Comparison script to be created if needed)*
...
- **Automation Script**: `scripts/mcp_validation/run_complete_code_quality_analysis.py`
...
cat docs/mcp-debugging/workflows/CODE_QUALITY_ANALYSIS_PLAN.md

# 2. Run full analysis (7-12 hours)
python scripts/mcp_validation/run_complete_code_quality_analysis.py
...
- Safe to interrupt (resume with `--resume` flag)
