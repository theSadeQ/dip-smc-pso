...
> Comprehensive guide to the physics models, mathematical foundations, and implementation architecture for the double inverted pendulum plant system.
...
- **Comprehensive Monitoring**: Energy conservation tracking and diagnostics
...
1. **Cart** (mass $m_0$): Moves horizontally on a track with position $x$
...
5. **Type Safety**: Comprehensive type hints for compile-time error detection
...
Dynamics computations return a structured result:
...
**Source:** [`src/plant/models/lowrank/dynamics.py`](../../src/plant/models/lowrank/dynamics.py)
...
Each model uses a type-safe dataclass-based configuration with comprehensive validation.
...
The DIP system is derived using Lagrangian mechanics. The Lagrangian $\mathcal{L} = T - V$ where:
...
This provides a validation criterion:
...
Enable fast mode for **10-100× speedup** in repeated evaluations:
...
Enable matrix caching for repeated evaluations at similar states:
