...
|------------|-------------------|-------------------|--------|
| Bounded disturbances | $\|d_u(t)\| \leq \bar{d}$ | d_bar ≈ 0.15 N (FDI threshold) |  Implicit |
| Switching gain | $K > \bar{d}$ | K = 0.5 > 0.15 |  PASS |
| Controllability | $\mathbf{L}\mathbf{M}^{-1}\mathbf{B} > \epsilon_0$ | Not validated at runtime |  Assumed |
...
```
 **Order:** `[k1, k2, lambda1, lambda2, K, kd]` (docstring line 114 confirms)
 **Config documentation:** Missing/unclear (Critical Finding #1)
...
| Assumption | Proof Requirement | Config/Code Value | Status |
|------------|-------------------|-------------------|--------|
| Lipschitz disturbance | $\|\dot{d}_u(t)\| \leq L$ | L ≈ 10 N/s (estimated) |  Not explicit |
...
| Finite reset freq | At most $N_{reset}$ per unit time | Runtime check needed |  Pending |
| Positive gains | All > 0 | All positive |  PASS |
...
-  Documentation mismatches create usability risk
-  Missing runtime validation (controllability, Lyapunov functions)
