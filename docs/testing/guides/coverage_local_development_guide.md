...
**Issue #9 - Local Development Workflow for Coverage Quality Gates**

> **🎯 Mission**: Streamlined local development integration for coverage validation and quality gate enforcement
...
#### PyCharm Integration
1. **Settings → Tools → Python Integrated Tools → Testing**
   - Default test runner: pytest
   - Additional arguments: `--cov=src --cov-report=html`
2. **Run/Debug Configurations**
   - Add pytest configuration with coverage enabled

---

## 📊 Local Coverage Workflows
...
# Develop with continuous coverage feedback
python -m pytest tests/test_my_module.py --cov=src/my_module --cov-report=term-missing -v

# Iterative improvement
while [[ $(python -m pytest tests/test_my_module.py --cov=src/my_module --cov-report=json -q && python -c "import json; print(json.load(open('coverage.json'))['totals']['percent_covered'])") < 95 ]]; do
...
python scripts/coverage_validator.py --coverage-xml coverage.xml --verbose --fail-below-threshold

# Generate PR-ready coverage report
...
python -m pytest tests/test_controllers/ --cov=src/controllers --cov-report=html:controller_coverage/

# Safety-critical controller validation (requires 100%)
...
python -m pytest tests/test_optimization/ --cov=src/optimizer --cov-report=html:optimization_coverage/
...
```bash
# Comprehensive safety-critical validation
python -m pytest tests/test_controllers/smc/core/ tests/test_controllers/base/ \
    --cov=src/controllers/smc/core --cov=src/controllers/base \
    --cov-fail-under=100 \
    --cov-report=html:safety_critical_coverage/ \
    --cov-report=json:safety_coverage.json

# Validate 100% achievement
python -c "
import json
with open('safety_coverage.json') as f:
    data = json.load(f)
    coverage = data['totals']['percent_covered']
    print(f'Safety-critical coverage: {coverage}%')
    exit(0 if coverage == 100.0 else 1)
"
```
...
for component in "${CRITICAL_COMPONENTS[@]}"; do
    ...
    python -m pytest "$component" --cov="${component/tests\/test_/src/}" --cov-fail-under=95 -q
done
```
...
echo "📊 Infrastructure Health: $([ $? -eq 0 ] && echo '✅ PASS' || echo '❌ FAIL')"
...
echo "📈 Overall Coverage: $OVERALL_COV% $([ $(echo "$OVERALL_COV >= 85" | bc -l) -eq 1 ] && echo '✅ PASS' || echo '❌ FAIL')"

# Safety-Critical
python -m pytest tests/test_controllers/smc/core/ --cov=src/controllers/smc/core --cov-fail-under=100 -q > /dev/null 2>&1
echo "🔴 Safety-Critical: $([ $? -eq 0 ] && echo '✅ PASS (100%)' || echo '❌ FAIL (<100%)')"
...
echo "🟠 Critical Components: $([ $? -eq 0 ] && echo '✅ PASS (≥95%)' || echo '❌ FAIL (<95%)')"
...
## ⚡ Performance-Optimized Local Testing

### Fast Coverage Collection
```bash
# Minimal coverage for rapid feedback (< 10 seconds)
python -m pytest tests/test_specific_module.py --cov=src/specific_module --cov-report=term -x

# Parallel execution (if tests are thread-safe)
python -m pytest tests/ --cov=src -n auto --cov-report=term-missing
...
coverage run -m pytest tests/ && coverage report --format=json > feature_coverage.json
...
python -m pytest tests/test_specific.py --cov=src/specific --cov-report=term -v

# Validate source file detection
python -c "
import coverage
...
python -m pytest tests/ --cov=src --cov-report=annotate:annotate_coverage/

# View annotate files with line-by-line coverage
...
python -m pytest --collect-only -v 2>&1 | grep -E "(ERROR|FAILED|ImportError)"
...
python scripts/coverage_validator.py --coverage-xml coverage.xml --verbose | grep -A 5 "FAIL"
...
python -m pytest tests/ --cov=src --cov-report=missing > missing_coverage.txt
...
print('Lowest coverage files (priority for testing):')
...
python -m pytest tests/test_analysis/test_stability.py -v --cov=src/controllers --cov-report=term-missing
...
python -m pytest tests/ --cov=src --cov-branch --cov-report=html:branch_coverage/
...
python -m pytest tests/ --cov=src --cov-report=html --cov-report=term-missing -k "error or exception or invalid"
...
echo "🔍 Pre-commit coverage validation..."
...
if [ $COVERAGE_STATUS -ne 0 ]; then
    ...
    echo "📊 Run: python -m pytest tests/ --cov=src --cov-report=term-missing"
    exit 1
fi
...
echo "📈 Post-merge coverage analysis..."

# Generate coverage baseline
python -m pytest tests/ --cov=src --cov-report=json:baseline_coverage.json -q
...
files = sorted(glob.glob('.coverage_history/coverage_*.json'))
for file in files[-7:]:  # Last 7 coverage snapshots
    with open(file) as f:
        data = json.load(f)
    timestamp = os.path.basename(file).split('_')[1:3]
    coverage = data['totals']['percent_covered']
    print(f'{timestamp[0]}_{timestamp[1]}: {coverage:.1f}%')
"
```
...
time python -m pytest tests/ --cov=src --cov-report=term-missing

# Coverage development velocity
git log --oneline --since="1 week ago" | grep -i "test\|coverage" | wc -l
```

---

**🎯 Issue #9 Local Development Integration**: This guide provides streamlined local development workflows for coverage quality gate enforcement, enabling efficient development cycles while maintaining the sophisticated 85%/95%/100% coverage framework.
