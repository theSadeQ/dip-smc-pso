# Phase 2 Execution Guide - HIGH & MEDIUM Batch Research
...
This guide provides step-by-step instructions to complete Phase 2 AI Research by processing all 497 remaining claims (459 HIGH + 38 MEDIUM) across 9 batches.
...
- **Total remaining:** 497 claims

---

## Tooling (Completed ✅)

All required tools have been created and are ready to use:

### 1. Batch Creation Tool
**Location:** `.dev_tools/research/create_batch.py`

**Purpose:** Extract claims by module/priority from inventory
...
### 2. Batch Merge Tool
**Location:** `.dev_tools/research/merge_all_batches.py`

**Purpose:** Merge all batch results and deduplicate BibTeX entries
...
python .dev_tools/research/merge_all_batches.py \
  --results artifacts/research/batch_*.json \
  --output artifacts/research/phase2_complete.json \
  --bibtex artifacts/research/enhanced_bibliography_phase2.bib
```

### 3. Report Generator
**Location:** `.dev_tools/research/generate_phase2_report.py`

**Purpose:** Generate comprehensive Phase 2 completion report with validation

**Usage:**
```bash
python .dev_tools/research/generate_phase2_report.py \
  --results artifacts/research/phase2_complete.json \
  --bibtex artifacts/research/enhanced_bibliography_phase2.bib \
...
All 9 batch input files have been created and validated:

| Batch | Module(s) | Priority | Claims | Time Estimate | Output File |
|-------|-----------|----------|--------|---------------|-------------|
...
| **Total** | | | **497** | **12-15 hours** | |

**Location:** `.artifacts/batch_*.json`

---

## Execution Workflow

### Phase 1: Batch Execution (12-15 hours total)

Execute each batch sequentially using the research pipeline:

#### Batch 1: Optimization (111 claims, 2.5-3 hours)

```bash
python .dev_tools/research/research_pipeline.py \
  --claims .artifacts/batch_01_optimization_high.json \
  --output artifacts/research/batch_01_results.json \
  --bibtex artifacts/research/batch_01_bibliography.bib \
  --log logs/research_batch_01.log
```
...
- Checkpoints in `artifacts/checkpoints/` (auto-saved every 50 claims)

**Validation (after completion):**
```bash
python .dev_tools/research/validate_results.py \
  --results artifacts/research/batch_01_results.json \
  --bibtex artifacts/research/batch_01_bibliography.bib
```

#### Batch 2-9: Repeat Pattern

Use the same command structure, replacing batch numbers:

```bash
# Batch 2 (Controllers)
python .dev_tools/research/research_pipeline.py \
  --claims .artifacts/batch_02_controllers_high.json \
  --output artifacts/research/batch_02_results.json \
  --bibtex artifacts/research/batch_02_bibliography.bib \
  --log logs/research_batch_02.log

# Batch 3 (Analysis)
python .dev_tools/research/research_pipeline.py \
  --claims .artifacts/batch_03_analysis_high.json \
  --output artifacts/research/batch_03_results.json \
  --bibtex artifacts/research/batch_03_bibliography.bib \
  --log logs/research_batch_03.log

# ... and so on for batches 4-9
```
...
### Phase 2: Merge & Validation (1-2 hours)

After all 9 batches are complete:
...
- `artifacts/research/enhanced_bibliography_phase2.bib` - Deduplicated BibTeX (400-600 entries estimated)
...
# Expected: Total claims: 497

# Count unique BibTeX entries
...
- `docs/plans/citation_system/phase2_completion_report.md` - Human-readable report
...
| **DOI/URL Accessibility** | ≥90% accessible | DOI or ArXiv/URL required |
...
| **DOI Coverage** | ≥95% with DOIs | Higher than minimum URL requirement |

---

## Risk Mitigation

### API Rate Limiting (High Probability)

**Risk:** Semantic Scholar API limits (100 req/5min) will be exceeded
...
- Consider splitting Batch 1 (111 claims) into 2 sub-batches if rate limiting severe
...
### Low Relevance Citations (Medium Probability)

**Risk:** Implementation claims yield less relevant academic papers than theorems

**Mitigation:**
- Query generator includes implementation-specific keywords
- Accept lower citation counts for utility/visualization claims
- Manual curation of top 50 most-cited implementation claims (optional)
...
for result in data['results'][:5]:  # Check first 5 claims
    print(f\"Claim: {result['claim_id']}\")
    for paper in result.get('selected_citations', []):
        print(f\"  - {paper.get('fields', {}).get('title', 'N/A')}\")
    print()
"
```

### Time Budget Overrun (Low-Medium Probability)

**Risk:** 12-15 hour estimate exceeded due to API delays

**Mitigation:**
- Process highest-priority batches first (1-2: optimization, controllers)
- Allow flexible completion timeline (Week 4-5)
- Use parallel sessions if multiple accounts available
...
- [ ] **Batch 1:** Optimization (111 claims) - Est. 2.5-3 hours
...
- [ ] **Generate Report:** Create completion report - Est. 30 min

### Validation Checklist

After merging:

- [ ] All 497 claims processed (check `phase2_complete.json`)
...
1. ✅ All 497 claims researched (9 batches executed)
...
6. ✅ Validation report generated with all gates PASS
...
- **Phase 2 completion:** ~942 total citations (22 CRITICAL + 920 HIGH/MEDIUM)

---

## Troubleshooting

### Problem: Batch execution fails with "API Error"
...
python .dev_tools/research/research_pipeline.py \
  --claims .artifacts/batch_XX.json \
  --output artifacts/research/batch_XX_results.json \
  --resume
```

### Problem: BibTeX has duplicate entries
...
### Problem: Validation report shows < 75% coverage
...
for claim in no_cites[:10]:  # Show first 10
    print(f\"  - {claim['claim_id']}: {claim.get('claim_text', '')[:60]}...\")
"
...
1. **Archive batch artifacts:**
   ```bash
   mkdir -p .archive/phase2_batches
   mv .artifacts/batch_*.json .archive/phase2_batches/
   ```
...
3. **Begin Phase 3: Manual Curation & Quality Review**
   - Review top 50 most-cited claims
   - Curate references for accuracy
   - Add missing citations manually if needed
...
- [high_batch_execution_plan.json](../../../artifacts/research/high_batch_execution_plan.json) - Original HIGH batch strategy
