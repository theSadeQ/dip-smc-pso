...
-------
list of dict, dict A list containing the raw metrics for each trial and a dictionary mapping metric names to tuples ``(mean, ci)`` where ``ci`` is half the width of the 95 % confidence interval. #### Source Code ```{literalinclude} ../../../src/analysis/validation/statistical_benchmarks.py
:language: python
:pyobject: run_trials
:linenos:
```
...
-------
dict comparison results #### Source Code ```{literalinclude} ../../../src/analysis/validation/statistical_benchmarks.py
:language: python
