...
:linenos:
``` #### Methods (3) ##### `__init__(self, time_constant)` Initialize transition filter. [View full source →](#method-transitionfilter-__init__) ##### `filter(self, new_input, dt)` Apply exponential smoothing filter. [View full source →](#method-transitionfilter-filter) ##### `reset(self, initial_value)` Reset filter state. [View full source →](#method-transitionfilter-reset) --- ### `ModularHybridSMC` Modular Hybrid SMC using intelligent switching between multiple controllers. Provides optimal performance by selecting the most appropriate SMC algorithm
