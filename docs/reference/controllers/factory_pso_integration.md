...
:linenos:
``` #### Methods (2) ##### `compute_control(self, state)` Compute control output for given state. [View full source →](#method-psooptimizable-compute_control) ##### `max_force(self)` Maximum control force limit. [View full source →](#method-psooptimizable-max_force) --- ### `PSOPerformanceMetrics` Performance metrics for PSO controller evaluation. #### Source Code ```{literalinclude} ../../../src/controllers/factory/pso_integration.py
:language: python
